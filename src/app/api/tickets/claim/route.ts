...
export function createTicketClaimPostHandler(deps: TicketClaimDeps) {
  return async function ticketClaimPost(request: Request) {
    const limited = await deps.enforceRateLimit(request, "ticket-claim", { limit: 10, windowMs: 60_000 });
    if (limited) {
      return limited;
    }
