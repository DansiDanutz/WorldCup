...
export async function GET(request: Request) {
  ...
  const summary = {
    ...
    noEntry: accounts.filter((account) => (account.entryStatus as AuditEntryStatus) === "none").length,
    ...
    incompletePicks: accounts.filter(
      (account) => (account.entryStatus as AuditEntryStatus) !== "none" && account.pickCount > 0 && account.pickCount < 3,
    ).length,
