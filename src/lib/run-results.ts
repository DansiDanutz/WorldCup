// Shared "fetch results → apply points → advance bracket" sweep.
//
// Used by the daily cron (/api/cron/results) and the admin "Run results now"
// button (/api/admin/run-results). Each due match is isolated: a single
// provider/DB failure is recorded and the batch continues, so one bad match
// never blocks the rest. Only FINISHED matches from the results provider are
// written; points apply via worldcup_apply_match_points (idempotent).
...
export type ResultsSweepSummary = {
  processed: ResultsSweepEntry[];
...
export async function runResultsSweep(): Promise<ResultsSweepSummary> {
  ...
  const processed: ResultsSweepEntry[] = [];
  ...
  return { processed, bracketAdvanced };
}
