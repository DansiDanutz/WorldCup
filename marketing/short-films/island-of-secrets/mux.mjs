// Final assembly for the Match 22 video:
...
// -> WorldCup26_Match22_IRQ_NOR.mp4 (H.264/AAC, 1920x1080)
...
const OUT = process.env.OUTFILE || 'The_Island_of_Secrets.mp4';
...
// Music cues + ambient SFX beds share the looped/faded/ducked-under-VO bus.
const cues = [...(music?.cues || []), ...(sfx?.beds || [])].filter(c => fs.existsSync(c.src));
for (const c of cues) {
...
execFileSync(ffmpegPath, [
  ...
  '-i', 'audio_master.m4a',
  '-filter_complex', `[0:v]crop=1080:1920:0:0,fps=${FPS},format=yuv420p[vout]`,
  '-map', '[vout]', '-map', '1:a',
