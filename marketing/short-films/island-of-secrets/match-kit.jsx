...
// CLIP-BASED episode: frame-exact <video> playback. The renderer drives
// window.__seek(t) while paused; each VideoSprite seeks its source to the exact
// frame and decrements the global counter so render.mjs waits before screenshot.
...
function VideoSprite({ src, start, dur, fit = 'cover', style = {}, dim = 0, rate = 1 }) {
  ...
  React.useEffect(() => {
    ...
    // Start 0.2s into the source: skips any dark intro frame AND guarantees a
    // non-zero seek on a clip's first visible frame (forces a real decode/paint,
    // so cuts never flash black). Clips are slowed via `rate`, so this loses nothing.
    const target = Math.min(0.2 + (local * rate), clipDur - 0.07);
    if (!v.paused) v.pause();
    if (Math.abs(v.currentTime - target) > 1 / 60) {
      ...
      const onSeeked = () => {
        if (v.requestVideoFrameCallback) { v.requestVideoFrameCallback(() => settle()); setTimeout(settle, 90); }
  ...
  return (
    <video ref={ref} src={src} muted playsInline preload="auto" style={{
      ...
      objectFit: fit, filter: dim ? `brightness(${1 - dim})` : 'none', ...style,
    }} />
  );
}
...
function TransitionLayer() {
  ...
  return (<>
    {dip > 0 && <div style={{ position: 'absolute', inset: 0, zIndex: 40, background: '#000', opacity: clamp(dip * 0.6, 0, 1) }} />}
