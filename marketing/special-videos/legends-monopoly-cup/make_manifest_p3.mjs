...
const seq = JSON.parse(readFileSync(DIR + 'clips_p3.json', 'utf8')).sequence;
const nar = JSON.parse(readFileSync(DIR + 'narration_p3.json', 'utf8'));
const vod = {}; let missing = [];
for (const l of nar.lines) {
  if (l.card || !l.text || !l.text.trim()) continue;
  const f = `${DIR}audio_p3/line_${String(l.id).padStart(2,'0')}.mp3`;
  if (!existsSync(f)) { missing.push(l.id); continue; }
  vod[l.id] = dur(f);
}
if (missing.length) { console.error(`\n  ⛔ Missing VO for lines: ${missing.join(', ')}\n  -> node gen_brian.mjs narration_p3.json audio_p3  (needs ElevenLabs quota)\n`); process.exit(1); }
...
seq.forEach((c,i)=>{
  ...
  let t=cs+lead;
  c.lines.forEach(id=>{vo.push({file:`audio_p3/line_${String(id).padStart(2,'0')}.mp3`,at:Math.round(t*100)/100,vol:1.0});t+=vod[id]+GAP;});
...
const sfx=[
  ...
  S("goal",at("n12")+0.3,0.5),S("braam",at("n14")+0.2,0.55),S("goal",at("n16")+0.3,0.5),
  S("whoosh",at("n19")+0.1,0.4),S("goal",at("n25")+0.3,0.6),S("mystic",at("n30")+0.2,0.45),
  S("whoosh",at("n31")+0.2,0.5),S("braam",at("n34")+0.15,0.6),S("mystic",at("n36")+0.2,0.4),
  S("braam",at("lb")+0.2,0.55),S("whoosh",at("n47")+0.1,0.4),S("braam",at("n50")+0.2,0.5),S("braam",at("cta"),0.5)
];
const music=[
  {file:"music/score-mystery.m4a",at:0,dur:at("n11")+2,vol:0.26,fadeIn:0.5,fadeOut:3},
  {file:"music/score-anthem.m4a",at:at("n11")-1,dur:(at("n18")-at("n11"))+3,vol:0.22,fadeIn:2,fadeOut:3.5},
  {file:"music/score-jail.m4a",at:at("n18")-1,dur:(at("n25")-at("n18"))+3,vol:0.25,fadeIn:1.5,fadeOut:3.5},
  {file:"music/score-build.m4a",at:at("n25")-1,dur:(at("n40")-at("n25"))+3,vol:0.24,fadeIn:2,fadeOut:4},
  {file:"music/score-finale.m4a",at:at("n40")-1,dur:(total-at("n40"))+1,vol:0.25,fadeIn:1.5,fadeOut:6}
];
const manifest={W:1920,H:1080,fps:30,xfade:0.4,outfile:"LegendsGame_Round3.mp4",clips,vo,music,sfx};
writeFileSync(DIR+'manifest.json',JSON.stringify(manifest,null,2));
console.log(`P3 manifest: ${clips.length} clips, ${vo.length} VO, ${music.length} music, ${sfx.length} sfx`);
