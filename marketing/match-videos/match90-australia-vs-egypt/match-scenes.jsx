// match-scenes.jsx — Ep90 Australia vs Egypt (R32) — PHOTOREAL + NO-REPEAT/NO-LOOP.
// "THE SPHINX AND THE SOUTHERN CROSS". OUR PREDICTION Egypt 2-1 Australia (Australia lead early;
// Salah equalise; Marmoush late winner). Mystic (#21): EGYPT = the Pharaohs, guarded by the
// eternal Sphinx -> Legend 090 = the Sphinx, Guardian of the Pharaohs; AUSTRALIA = the Socceroos
// under the Southern Cross. Both squads generated FRESH with LIKENESS prompts + real numbers (#29).
// Beat captions over dimmed footage (#27). Nation-correct only (#28). #23 name<->image SYNC.

const EG_RED = '#ce1126', EG_WHITE = '#f3f4f7', EG_GOLD = '#c8a13a';
const AU_GOLD = '#ffcd00', AU_GREEN = '#00843d', AU_BLUE = '#00247d';
const ACC = '#d4af37';
const GRADE = { filter: 'saturate(1.06) contrast(1.04)' };

function FlagEG({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ flex: 1, background: EG_RED }} />
      <div style={{ flex: 1, background: '#fff' }} />
      <div style={{ flex: 1, background: '#000' }} />
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: h * 0.28, height: h * 0.28, borderRadius: '50%', background: EG_GOLD, boxShadow: '0 0 6px rgba(0,0,0,0.3)' }} />
    </div>
  );
}
function FlagAU({ w = 120 }) {
  const h = w * 2 / 3;
  const star = (x, y, s) => <div style={{ position: 'absolute', left: `${x}%`, top: `${y}%`, transform: 'translate(-50%,-50%)', color: '#fff', fontSize: s, lineHeight: 1 }}>★</div>;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, overflow: 'hidden', position: 'relative', background: AU_BLUE, boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', left: 0, top: 0, width: '42%', height: '50%', background: `linear-gradient(${AU_BLUE},${AU_BLUE})` }}>
        <div style={{ position: 'absolute', inset: 0, background: 'conic-gradient(from 0deg at 50% 50%, transparent 0, transparent 100%)' }} />
        <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: '90%', height: 0, borderTop: '2px solid rgba(255,255,255,0.7)' }} />
        <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%) rotate(90deg)', width: '90%', height: 0, borderTop: '2px solid rgba(255,255,255,0.7)' }} />
      </div>
      {star(21, 78, h * 0.16)}
      {star(70, 24, h * 0.13)}{star(84, 44, h * 0.13)}{star(72, 66, h * 0.13)}{star(60, 50, h * 0.09)}{star(88, 74, h * 0.1)}
    </div>
  );
}

function FS({ id, style }) { return <ClipSprite id={id} fit="cover" style={{ ...GRADE, ...(style || {}) }} />; }
// Sphinx card art (slow Ken-Burns) — Egypt's cold-open symbol.
function SphinxStill({ start = 0, dur = 5 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', opacity: fade, zIndex: 1 }}>
      <img data-seq src="assets/legend-090-landscape.png" alt="" style={{ position: 'absolute', left: '50%', top: '50%', width: '108%', transform: `translate(-50%,-50%) scale(${scale})`, filter: 'saturate(1.05) contrast(1.03)' }} />
...
function NightField({ o = 0.6 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 50% 42%, rgba(212,175,55,${(0.13 * pulse * o).toFixed(3)}) 0%, transparent 55%)` }} />
    </div>
  );
}

function ScoreBug({ start, eg = 2, au = 1, note = "88'", badge = 'OUR PREDICTION' }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ ...cell, background: EG_RED }}>EGY</div>
        <div style={{ ...cell, fontSize: 38, color: ACC }}>{eg} — {au}</div>
        <div style={{ ...cell, background: AU_GREEN }}>AUS</div>
        {note && <div style={{ ...cell, fontSize: 24, color: ACC, borderLeft: `1px solid ${MV.line}` }}>{note}</div>}
      </div>
      <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 16, color: ACC, letterSpacing: '0.22em', background: 'rgba(212,175,55,0.14)', border: '1px solid rgba(212,175,55,0.5)', borderRadius: 999, padding: '4px 16px' }}>{badge}</div>
    </div>
  );
}
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <SphinxStill start={0} dur={5} />
      <FS id="egy-sphinx" /><FS id="aus-outback" /><FS id="texture-australia" />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at center, rgba(212,175,55,${(0.20 * beat).toFixed(3)}) 0%, transparent 60%)` }} />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f0dca6', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE SPHINX MEETS THE STARS.</div>
        </div>
      )}
      {lt > 19.2 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, opacity: titleP, zIndex: 22 }}>
          <Kicker size={32} color={ACC}>The Sphinx and The Southern Cross</Kicker>
          <TitleReveal text="THE SPHINX" start={20.0} size={84} color={ACC} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 38 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 90 · Last 32</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 56, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagEG w={210} /></Waving><BigTitle size={64} glow={EG_RED}>EGYPT</BigTitle></div>
          <BigTitle size={108} color={ACC}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagAU w={210} /></Waving><BigTitle size={56} glow={AU_GOLD}>AUSTRALIA</BigTitle></div>
        </div>
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['KINGS OF AFRICA', 'EGYPT'], ['THE SOCCEROOS', 'AUSTRALIA'], ['THE SPHINX', 'vs THE STARS'], ['LAST 32', 'WIN OR GO HOME']].map(([v, l], i) => (
...
function SceneEgypt() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={44} end={49}><div style={{ position: 'absolute', inset: 0 }}><FS id="egy-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagEG w={64} />} label="THE PHARAOHS · EGYPT" accent={EG_RED} /></div></Sprite>
      <Sprite start={49} end={54}><div style={{ position: 'absolute', inset: 0 }}><FS id="egy-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <Sprite start={54} end={55.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="egy-surge" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(206,17,38,0.30), transparent 55%)' }} /></div></Sprite>
      <PlayerShowcase clipId="egy-salah" name="MOHAMED SALAH" role="CAPTAIN · THE PHARAOH · 10" accent={EG_RED} start={55.5} end={60.5} />
      <PlayerShowcase clipId="egy-marmoush" name="OMAR MARMOUSH" role="THE YOUNG FIRE · 9" accent={ACC} start={60.5} end={63.74} />
      <PlayerShowcase clipId="egy-trezeguet" name="MAHMOUD TREZEGUET" role="THE DESERT WIND · 7" accent={EG_RED} start={63.74} end={68.0} />
      <PlayerShowcase clipId="egy-elneny" name="MOHAMED ELNENY" role="THE IRON MIDDLE · 17" accent={ACC} start={68.0} end={70.99} />
      <PlayerShowcase clipId="egy-cb" name="THE WALL OF EGYPT" role="THE ANCIENT STONE · 6" accent={EG_RED} start={70.99} end={79.5} />
    </div>
  );
}
...
function SceneAustralia() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={79.5} end={84.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="aus-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagAU w={64} />} label="THE SOCCEROOS · AUSTRALIA" accent={AU_GOLD} /></div></Sprite>
      <Sprite start={84.5} end={90.6}><div style={{ position: 'absolute', inset: 0 }}><FS id="aus-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <PlayerShowcase clipId="aus-mooy" name="AARON MOOY" role="THE MAESTRO · 13" accent={AU_GOLD} start={90.6} end={96.8} />
      <PlayerShowcase clipId="aus-irvine" name="JACKSON IRVINE" role="CAPTAIN · THE ENGINE · 22" accent={AU_GREEN} start={96.8} end={99.95} />
      <PlayerShowcase clipId="aus-goodwin" name="CRAIG GOODWIN" role="THE LEFT CANNON · 23" accent={AU_GOLD} start={99.95} end={103.0} />
      <PlayerShowcase clipId="aus-duke" name="MITCHELL DUKE" role="THE WARRIOR · 9" accent={AU_GREEN} start={103.0} end={105.71} />
      <PlayerShowcase clipId="aus-mcgree" name="RILEY McGREE" role="THE SPARK · 15" accent={AU_GOLD} start={105.71} end={113.0} />
    </div>
  );
}
...
function SceneRiddle() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={113} end={118}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.24)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE SPHINX vs THE STARS</div></div></Sprite>
      ...
      <Sprite start={123} end={132}><BeatCard clipId="bg-riddle" start={123} end={132} text={<>DOES THE SPHINX GUARD THE CROWN,<br />OR DO THE SOCCEROOS STEAL THE NIGHT?</>} accent={ACC} big={48} /></Sprite>
    </div>
  );
}
...
function PredictionCard({ start }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(212,175,55,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${EG_RED}44 0%, #070b14 38%, #070b14 62%, ${AU_GOLD}33 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${EG_RED} 0%, ${ACC} 50%, ${AU_GOLD} 100%)`, padding: '13px 0', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '36px 70px 28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 46 }}>
            <Badge flag={<FlagEG w={104} />} name="EGYPT" accent={EG_RED} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 116, color: '#fff', lineHeight: 1 }}>
                <span style={{ textShadow: `0 0 36px ${EG_RED}cc` }}>2</span>
                <span style={{ color: ACC, fontSize: 58, transform: 'translateY(-6px)' }}>—</span>
                <span style={{ textShadow: `0 0 36px ${AU_GOLD}cc` }}>1</span>
              </div>
            ...
            <Badge flag={<FlagAU w={104} />} name="AUSTRALIA" accent={AU_GOLD} />
          </div>
          <div style={{ textAlign: 'center', marginTop: 16, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 32, color: ACC, letterSpacing: '0.05em', textShadow: `0 0 26px ${ACC}66` }}>THE SPHINX GUARDS THE CROWN · 2–1</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 16, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#f3e6c4', background: 'rgba(212,175,55,0.16)', border: '1px solid rgba(212,175,55,0.45)', borderRadius: 999, padding: '7px 18px' }}>👑 MARMOUSH — THE LATE WINNER</span>
          </div>
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <Sprite start={132} end={137}><BeatCard clipId="bg-firing" start={132} end={137} text={<>AUSTRALIA COME<br />OUT FLYING</>} accent={AU_GOLD} big={62} /></Sprite>
      <Sprite start={137} end={142}><div style={{ position: 'absolute', inset: 0 }}><FS id="aus-surge" /><ChanceTag start={137.5} end={142} text="THE SOCCEROOS PRESS" sub="ALL RUNNING, ALL HEART" accent={AU_GREEN} /></div></Sprite>
      <Sprite start={142} end={152.21}><BeatCard clipId="texture-egypt" start={142} end={152.21} text={<>THE ANCIENT KINGDOM<br />IS ROCKED</>} accent="#fff" big={60} /></Sprite>
      <Sprite start={152.21} end={159}><div style={{ position: 'absolute', inset: 0 }}><FS id="aus-goal-1" /><GoalFlash at={153.6} /><ChanceTag start={154} end={159} text="AUSTRALIA LEAD!" sub="THE SOUTHERN CROSS BURNS" accent={AU_GOLD} /></div></Sprite>
      <Sprite start={159} end={172}><BeatCard clipId="bg-rise" start={159} end={172} text={<>BUT THE SPHINX DOES NOT FALL</>} sub="EGYPT STEADY · THE TEMPLE WAKES" accent={ACC} big={54} /></Sprite>
      <Sprite start={172} end={178.46}><BeatCard clipId="bg-believe" start={172} end={178.46} text={<>THE BALL<br />FINDS SALAH</>} accent="#fff" big={68} /></Sprite>
      <Sprite start={178.46} end={185}><div style={{ position: 'absolute', inset: 0 }}><FS id="egy-goal-1" /><GoalFlash at={179.9} /><ChanceTag start={180.3} end={185} text="SALAH — ONE-ONE!" sub="THE PHARAOH SPEAKS" accent={EG_RED} /></div></Sprite>
      <Sprite start={185} end={189.58}><BeatCard clipId="bg-roar" start={185} end={189.58} text={<>AND THEN —<br />THE DAGGER</>} accent={ACC} big={68} /></Sprite>
      <Sprite start={189.58} end={203.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="egy-goal-2" /><GoalFlash at={191.0} /><ChanceTag start={191.4} end={196} text="MARMOUSH — 2-1 EGYPT!" sub="THE CROWN HOLDS" accent={ACC} /><ScoreBug start={196.3} eg={2} au={1} note="88'" badge="OUR PREDICTION" /><PredictionCard start={196.9} /></div></Sprite>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={203.32} end={208.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="vd-handshake" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.25) 0%, transparent 45%, rgba(2,3,8,0.55) 100%)' }} /><div style={{ position: 'absolute', left: 0, right: 0, top: 120, textAlign: 'center', zIndex: 24 }}><Kicker size={28} color={ACC}>The Sphinx Guards the Crown</Kicker></div></div></Sprite>
      <Sprite start={208.32} end={213.32}><BeatCard clipId="bg-verdict" start={208.32} end={213.32} text={<>AUSTRALIA RAN THE KINGS —<br />THE SPHINX HELD</>} accent="#fff" big={50} /></Sprite>
      <Sprite start={213.32} end={218.32}><BeatCard clipId="bg-winner" start={213.32} end={218.32} text={<>MARMOUSH</>} sub="THE LATE WINNER" accent={ACC} big={64} /></Sprite>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
            <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 900, backdropFilter: 'blur(6px)', opacity: panelP, transform: `translateY(${(1 - panelP) * 24}px)` }}>
              ...
              <div style={{ marginTop: 24 }}>
                <StatLine start={219.5} delay={0.0} label="EGYPT" value="THE PHARAOHS · KINGS OF AFRICA" accent={EG_RED} />
                <StatLine start={219.5} delay={0.25} label="AUSTRALIA" value="THE SOCCEROOS · THE STARS" accent={AU_GOLD} />
                <StatLine start={219.5} delay={0.5} label="OUR PREDICTION" value="EGY 2 — 1 AUS" accent="#fff" />
                <StatLine start={219.5} delay={0.75} label="88' MARMOUSH" value="THE LATE WINNER" accent={ACC} />
              </div>
            </div>
          </div>
        </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT EGYPT', sub: 'THE PHARAOHS', flag: <FlagEG w={76} />, accent: EG_RED },
    { label: 'COMMENT AUSTRALIA', sub: 'THE SOCCEROOS', flag: <FlagAU w={76} />, accent: AU_GOLD },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 140, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: headP }}><Kicker size={30}>The Sphinx, Or The Stars?</Kicker></div>
...
function MiniStrip() {
  const prev = ['legend-085-portrait', 'legend-086-portrait', 'legend-087-portrait', 'legend-088-portrait', 'legend-089-portrait'];
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
      ...
      <div style={{ width: 78, height: 104, borderRadius: 9, overflow: 'hidden', border: `2px solid ${ACC}`, boxShadow: `0 10px 28px rgba(0,0,0,0.6), 0 0 26px ${ACC}66` }}>
        <img data-seq alt="" src="assets/legend-090-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      </div>
    </div>
  );
}
...
function SceneMystery() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 50% 44%, rgba(212,175,55,${(0.32 * glow).toFixed(3)}) 0%, transparent 55%)` }} />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: 90, textAlign: 'center', opacity: teaseP, zIndex: 20 }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f3e0b8', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.7)' }}>GUARDIAN OF THE PHARAOHS</div>
        </div>
      )}
      {lt > 1.6 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 22, opacity: clamp(cardP, 0, 1) }}>
          <div style={{ position: 'relative', transform: `perspective(1500px) rotateY(${tilt}deg) scale(${0.92 + 0.08 * Math.min(cardP, 1)})`, marginTop: -34 }}>
            <img data-seq src="assets/legend-090-portrait.png" alt="" style={{ height: 640, display: 'block', borderRadius: 16, boxShadow: '0 50px 130px rgba(0,0,0,0.85), 0 0 80px rgba(212,175,55,0.5)' }} />
            ...
            <div style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', background: 'rgba(212,175,55,0.95)', color: '#1a1306', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 17, letterSpacing: '0.14em', padding: '5px 16px', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.5)' }}>Nº 090 · ✦✦✦ ULTRA RARE</div>
          </div>
        </div>
      )}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, zIndex: 24 }}>
        <div style={{ textAlign: 'center', opacity: txtP, transform: `translateY(${(1 - txtP) * 16}px)` }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 44, color: ACC, letterSpacing: '0.03em', textShadow: '0 2px 24px rgba(212,175,55,0.5)' }}>THE SPHINX · GUARDIAN OF THE PHARAOHS</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 19, color: '#ecd9b0', letterSpacing: '0.18em', marginTop: 6 }}>EGYPT · LEGEND 090</div>
        </div>
...
function PhoneCollect({ start }) {
  ...
  const filled = ['legend-086-portrait', 'legend-087-portrait', 'legend-088-portrait', 'legend-089-portrait'];
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 26, opacity: inP }}>
      <div style={{ position: 'relative', width: 300, height: 600, borderRadius: 40, background: 'linear-gradient(160deg,#23262e,#0c0e13)', border: '3px solid #2b2f38', boxShadow: '0 40px 110px rgba(0,0,0,0.8), 0 0 50px rgba(212,175,55,0.25)', padding: 12 }}>
        ...
        <div style={{ width: '100%', height: '100%', borderRadius: 30, background: 'radial-gradient(ellipse at 50% 20%, #1a2030 0%, #06080c 70%)', overflow: 'hidden', position: 'relative', padding: '34px 18px 18px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            ...
            <div style={{ height: 104, borderRadius: 6, overflow: 'hidden', border: snapped ? `2px solid ${ACC}` : '1px dashed rgba(212,175,55,0.6)', position: 'relative', boxShadow: flash > 0 ? `0 0 ${20 * flash}px ${ACC}` : 'none' }}>
              {snapped && <img data-seq alt="" src="assets/legend-090-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 92, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: p }}>
        <Kicker size={30} color={ACC}>Claim the Sphinx</Kicker>
...
function SceneCTA() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={303.05} end={308.05}><FS id="egy-cta" /></Sprite>
