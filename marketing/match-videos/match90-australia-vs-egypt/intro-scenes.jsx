// intro-scenes.jsx — Ep90 · 15s MYSTIC INTRO (rule #20). Wordless, atmospheric
// face-off: the sphinx and the southern cross, the two flags rising from shadow, an igniting "VS",
// the WorldCup26 Legends wordmark, then a whoosh-fade into the cold open.
// Reuses FlagEG/FlagAU from match-scenes.jsx. Concatenated to the FRONT of the body → ~333s.
...
function SceneIntro() {
  ...
  const sideUnit = (flag, name, x, accent) => (
    <div style={{ position: 'absolute', left: '50%', top: '46%', transform: `translate(-50%,-50%) translateX(${x}px)`, opacity: flagP, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 22, zIndex: 12 }}>
      ...
      <div style={{ opacity: namesP, transform: `translateY(${(1 - namesP) * 16}px)`, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 52, letterSpacing: '0.05em', color: '#fff', textShadow: `0 4px 26px rgba(0,0,0,0.9), 0 0 26px ${accent}66` }}>{name}</div>
    </div>
  );
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      {sideUnit(<FlagEG w={300} />, 'EGYPT', leftX, '#ce1126')}
      {sideUnit(<FlagAU w={300} />, 'AUSTRALIA', rightX, '#ffcd00')}
      ...
      <div style={{ position: 'absolute', bottom: 120, left: 0, right: 0, textAlign: 'center', zIndex: 20, opacity: themeP * (1 - fadeOut) }}>
        <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 28, letterSpacing: '0.30em', color: '#ffe9a0', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE SPHINX AND THE SOUTHERN CROSS</div>
      </div>
