// match-scenes.jsx — Ep64 Senegal vs Iraq — ANIMATION-FIRST + NO-REPEAT (318s).
// "The Lion Meets the Lions of Mesopotamia" · Group I. OUR PREDICTION SEN 1–0 IRQ.
// Rule #11: every clip used EXACTLY ONCE. Rule #15: no banned hype wording.
// Rule #16: the Legend (The Griot) is TEASED in the cold open (cg-tease + riddle
// "WHO KEEPS THE ROAR ALIVE?"), echoed mid-episode, and the reveal STATES the
// spine↔legend connection — the griot is "the keeper of the roar" (a people's
// story sung through every hardship so the pride never forgets how to roar).
// Senegal strike first ~40'; Koulibaly's 55' header rattles the crossbar; 1-0 holds.

const SEN = '#00853f', SEN_YEL = '#fdef42', SEN_RED = '#e31b23', IRQ = '#007a3d', IRQ_RED = '#ce1126';
...
function FlagSEN({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', display: 'flex', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <div style={{ flex: 1, background: '#fdef42', position: 'relative' }}><div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', color: '#00853f', fontSize: h * 0.34, lineHeight: 1 }}>★</div></div>
...
function FlagIRQ({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <div style={{ position: 'absolute', top: '33.33%', left: 0, right: 0, height: '33.34%', background: '#fff' }} />
      <div style={{ position: 'absolute', top: '66.67%', left: 0, right: 0, height: '33.33%', background: '#111' }} />
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', color: '#007a3d', fontSize: h * 0.18, fontWeight: 900, whiteSpace: 'nowrap', letterSpacing: '0.05em' }}>الله أكبر</div>
    </div>
  );
}

function ScoreBug({ start, sen = 0, irq = 0, minute, badge = "OUR PREDICTION", note }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ ...cell, background: SEN }}>SEN</div>
        <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{sen} — {irq}</div>
        <div style={{ ...cell, background: IRQ }}>IRQ</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        {note && <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 16, color: '#fff', letterSpacing: '0.18em', background: SEN, border: `1px solid ${SEN}`, borderRadius: 999, padding: '4px 16px' }}>{note}</div>}
      </div>
    </div>
  );
}
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <FS id="cg-senfan" /><FS id="cg-irqfan" /><FS id="cg-tease" /><FS id="cg-stadaerial" />
      ...
      {teaseLabelP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseLabelP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#e8c97a', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>WHO KEEPS THE ROAR ALIVE?</div>
        </div>
      )}
      {lt > 18.0 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 30, opacity: titleP, zIndex: 22 }}>
          <Kicker size={34}>Two Prides · One Pitch</Kicker>
          <TitleReveal text="TWO LIONS" start={19.8} size={112} color={MV.gold} />
        </div>
      )}
      <Letterbox />
    </div>
...
function SceneTitle() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, #0a0f1c 0%, #0a140d 55%, #0a0f1c 100%)' }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 30%, rgba(0,133,63,0.16) 0%, transparent 55%)' }} />
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 40 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 64</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 64, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagSEN w={220} /></Waving><BigTitle size={80} glow={SEN}>SENEGAL</BigTitle></div>
          <BigTitle size={116} color={MV.gold}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagIRQ w={220} /></Waving><BigTitle size={80} glow="#9ad6a0">IRAQ</BigTitle></div>
        </div>
        <div style={{ opacity: p3, fontFamily: '"Inter",sans-serif', fontWeight: 600, fontSize: 32, color: MV.muted, letterSpacing: '0.08em' }}>GROUP I · TWO PRIDES OF LIONS</div>
      </div>
      <Letterbox />
    </div>
  );
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['GROUP I', 'TWO LIONS'], ['TERANGA', 'vs RAFIDAIN'], ['FIRE', 'vs RESILIENCE'], ['FOOTBALL', 'AS RESISTANCE']].map(([v, l], i) => (
...
function SceneSenegal() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={44} end={55.6}><div style={{ position: 'absolute', inset: 0 }}><FS id="sen-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.32) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagSEN w={66} />} label="LIONS OF TERANGA · SENEGAL" accent={SEN} /></div></Sprite>
      <PlayerShowcase clipId="s-mane" name="SADIO MANÉ" role="THE BIG BROTHER" accent={SEN} start={55.5} end={61.1} />
      <PlayerShowcase clipId="s-jackson" name="NICOLAS JACKSON" role="THE YOUNG GUN" accent={SEN_RED} start={61.1} end={65.1} />
      <PlayerShowcase clipId="s-koulibaly" name="KALIDOU KOULIBALY" role="THE CAPTAIN" accent={SEN} start={65.1} end={68.1} />
      <PlayerShowcase clipId="s-camara" name="LAMINE CAMARA" role="THE FUTURE" accent={SEN_YEL} start={68.1} end={73.6} />
      <PlayerShowcase clipId="s-sarr" name="PAPE MATAR SARR" role="THE ENGINE" accent={SEN} start={73.6} end={79.5} />
...
function SceneIraq() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={79.5} end={90.7}><div style={{ position: 'absolute', inset: 0 }}><FS id="irq-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.32) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagIRQ w={66} />} label="LIONS OF MESOPOTAMIA · IRAQ" accent={IRQ} /></div></Sprite>
      <PlayerShowcase clipId="s-iqbal" name="ZIDANE IQBAL" role="THE ANCHOR" accent={IRQ} start={90.6} end={95.2} />
      <PlayerShowcase clipId="s-hussein" name="AYMEN HUSSEIN" role="THE STRIKER" accent={IRQ_RED} start={95.2} end={98.2} />
      <PlayerShowcase clipId="s-alhamadi" name="ALI AL-HAMADI" role="THE SPARK" accent={IRQ} start={98.2} end={100.8} />
      <PlayerShowcase clipId="s-alammari" name="AMIR AL-AMMARI" role="THE DRIVER" accent={IRQ_RED} start={100.8} end={103} />
      <Vignette strength={0.35} />
      <Letterbox />
    </div>
  );
...
function SceneDuel() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={103} end={113}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-sen" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(0,133,63,0.4), transparent 55%)' }} /><div style={{ position: 'absolute', left: 80, bottom: 130, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE FIRE<div style={{ fontSize: 27, fontWeight: 700, color: MV.gold, letterSpacing: '0.2em', marginTop: 8 }}>RHYTHM OF TERANGA</div></div></div></Sprite>
      <Sprite start={113} end={122.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.25)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 58, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>FOOTBALL AS RESISTANCE</div></div></Sprite>
      <Sprite start={122.5} end={132}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-irq" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(270deg, rgba(0,122,61,0.42), transparent 55%)' }} /><div style={{ position: 'absolute', right: 80, bottom: 130, textAlign: 'right', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE RESILIENCE<div style={{ fontSize: 27, fontWeight: 700, color: MV.gold, letterSpacing: '0.2em', marginTop: 8 }}>WILL OF RAFIDAIN</div></div></div></Sprite>
      <Letterbox />
    </div>
  );
}
...
function SceneDrama() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <FS id="dr-sen-attack" /><FS id="dr-goal" /><FS id="dr-sen-celeb" /><FS id="dr-irq-attack" /><FS id="dr-bar" /><FS id="dr-keeper" /><FS id="dr-defiance" />
      ...
      {/* Senegal strike — SEN 1-0, 40' */}
      <GoalFlash at={S + 19.0} />
      <Sprite start={152.0} end={189.0}><ScoreBug start={S + 20.5} sen={1} irq={0} minute="40'" badge="OUR PREDICTION" /></Sprite>
      {/* Koulibaly header hits the crossbar 55' — no goal, 1-0 holds */}
      <Sprite start={189.0} end={203.32}><ScoreBug start={S + 58.0} sen={1} irq={0} minute="55'" badge="OUR PREDICTION" note="OFF THE BAR" /></Sprite>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.55)', opacity: p }}>
      <div style={{ width: 980, borderRadius: 26, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 30px 120px rgba(0,0,0,0.75)', position: 'relative', border: `1px solid ${MV.line}` }}>
        <div style={{ background: `linear-gradient(100deg, ${SEN} 0%, #1a1f2e 50%, ${IRQ} 100%)`, padding: '14px 0', textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 24, letterSpacing: '0.3em', color: '#fff' }}>FULL-TIME · OUR PREDICTION</div>
        <div style={{ background: 'linear-gradient(160deg, #0d1424 0%, #070b14 100%)', padding: '44px 70px 38px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 48 }}>
            <Badge flag={<FlagSEN w={104} />} name="SENEGAL" accent={SEN} />
            ...
            <Badge flag={<FlagIRQ w={104} />} name="IRAQ" accent="#9ad6a0" />
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 30, paddingTop: 22, borderTop: `1px solid ${MV.line}` }}>
            <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 24, color: '#5fd07f' }}>⚽ 40' &nbsp;THE FIRE STRIKES</div>
            <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 24, color: SEN_YEL }}>55' &nbsp;OFF THE BAR 🪵</div>
          </div>
          <div style={{ textAlign: 'center', marginTop: 24, fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 22, color: MV.muted, letterSpacing: '0.16em' }}>BOTH LIONS ROAR ON</div>
        </div>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
        <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 880, backdropFilter: 'blur(6px)', opacity: clamp((lt - 11) / 1.0, 0, 1) }}>
          ...
          <div style={{ marginTop: 24 }}>
            <StatLine start={S + 11.5} delay={0.0} label="SENEGAL" value="LIONS OF TERANGA" accent={SEN} />
            <StatLine start={S + 11.5} delay={0.25} label="IRAQ" value="LIONS OF MESOPOTAMIA" accent="#9ad6a0" />
            <StatLine start={S + 11.5} delay={0.5} label="OUR PREDICTION" value="SEN 1 — 0 IRQ" accent="#fff" />
            <StatLine start={S + 11.5} delay={0.75} label="40' THE FIRE · 55' OFF THE BAR" value="BOTH LIONS ROAR ON" accent={MV.gold} />
          </div>
        </div>
      </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT TERANGA', sub: "SENEGAL'S FIRE", flag: <FlagSEN w={78} />, accent: SEN },
    { label: 'COMMENT RAFIDAIN', sub: "IRAQ'S LIONS", flag: <FlagIRQ w={78} />, accent: '#9ad6a0' },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <FS id="en-crowd" />
...
// cold-open "WHO KEEPS THE ROAR ALIVE?" tease — the Griot, keeper of the roar.
function SceneMystery() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <ClipSprite id="my-griot" fit="cover" style={{ filter: 'brightness(0.7) saturate(1.12)' }} />
      ...
      <div style={{ position: 'absolute', top: 70, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: inP }}><Kicker size={26} color="#e8c97a">The Keeper Of The Roar · Legend No. 064</Kicker></div>
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: '50%', top: '50%', zIndex: 24, pointerEvents: 'none', transform: `translate(-50%,-50%) scale(${0.8 + teasePulse * 0.12})`, opacity: teaseP, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
          ...
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 30, color: '#ffe9a0', letterSpacing: '0.2em', textShadow: '0 2px 18px rgba(0,0,0,0.8)' }}>THE ONE WHO CARRIES THE ROAR</div>
        </div>
      )}
      ...
      {cardP > 0 && (
        <div style={{ position: 'absolute', left: '50%', top: '49%', zIndex: 25, opacity: clamp(cardP, 0, 1), perspective: 1600, transform: `translate(-50%,-50%) translateY(${(1 - cardP) * 40 + floatY}px) scale(${0.9 + 0.1 * cardP})` }}>
          <div style={{ transformStyle: 'preserve-3d', transform: `rotateY(${flipDeg}deg) rotate(${floatR}deg)`, borderRadius: 22, boxShadow: `0 30px 120px rgba(0,0,0,0.8), 0 0 ${50 + settle * 40}px rgba(245,208,22,${0.2 + settle * 0.3})` }}>
            <img data-seq="" src="assets/legend-064-portrait.png" alt="" style={{ height: 820, width: 'auto', display: 'block', borderRadius: 18 }} />
          </div>
        </div>
      )}
      {settle > 0.2 && (
        <div style={{ position: 'absolute', left: '50%', bottom: 46, zIndex: 26, transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', gap: 16, opacity: settle }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 12, background: 'linear-gradient(100deg, #16a34a, #0e8a3c)', borderRadius: 999, padding: '14px 32px', transform: `scale(${1 + pulse * 0.04})`, boxShadow: `0 8px 30px rgba(22,163,74,${0.35 + pulse * 0.4})` }}><span style={{ fontSize: 22 }}>⚡</span><span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 24, color: '#fff', letterSpacing: '0.03em' }}>SIGN UP FREE — UNLOCK LEGEND 064</span></div>
        </div>
      )}
...
function SceneApp() {
  ...
  const pulse = 0.5 + 0.5 * Math.sin(lt * 3.4);
  const cards = [
    { name: 'SENEGAL', flag: <FlagSEN w={84} /> },
    { name: 'IRAQ', flag: <FlagIRQ w={84} /> },
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(160deg, #0a1a10 0%, #07090f 70%)` }}>
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 50% 20%, rgba(0,133,63,0.4) 0%, rgba(7,9,15,0.92) 62%)` }} />
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 42, opacity: inP }}>
        <Kicker color="#9fe0b3" size={30}>The Prediction Game</Kicker>
        <BigTitle size={100} color="#fff" glow={SEN}>worldcup26.world</BigTitle>
        <Kicker color="#cfeede" size={30}>Sign Up · Pick 3 Of 48</Kicker>
        ...
        <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 27, color: MV.gold, opacity: clamp(btnP, 0, 1) }}>Unlock Legend 064 the moment you sign up · free · no prizes</div>
      </div>
      <Letterbox />
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', left: 0, right: 0, bottom: 140, display: 'flex', justifyContent: 'center', zIndex: 26, opacity: p, transform: `translateY(${(1 - p) * 24}px)` }}>
      <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 999, padding: '18px 50px', display: 'flex', alignItems: 'center', gap: 20 }}>
        ...
        <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 29, color: MV.gold }}>⚡ EP65 · COLLECT LEGEND 064 · worldcup26.world</span>
      </div>
    </div>
  );
}
