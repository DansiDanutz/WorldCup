...
function FlagCAN({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.04, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)', display: 'flex' }}>
      ...
      <div style={{ width: '50%', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ fontSize: h * 0.62, lineHeight: 1, color: '#d52b1e' }}>🍁</span></div>
      <div style={{ width: '25%', background: '#d52b1e' }} />
    </div>
  );
}
...
function ScoreBug({ start, aut = 0, jor = 0, minute }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ ...cell, background: MV.aut, color: '#fff' }}>CAN</div>
      <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{aut} — {jor}</div>
      <div style={{ ...cell, background: MV.jor, color: '#fff' }}>QAT</div>
