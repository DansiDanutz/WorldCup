// match-scenes.jsx — Ep88 Portugal vs Croatia (R32) — PHOTOREAL + NO-REPEAT/NO-LOOP.
// "THE NAVIGATOR AND THE FIRE". OUR PREDICTION Portugal 2-1 Croatia (Kramarić early; Ronaldo
// equalise; Bruno late winner). Mystic (#21): PORTUGAL = the Navigators (Os Navegadores);
// CROATIA = the Vatreni, the checkered Fire -> Legend 088 = the Navigator, Master of the Unknown.
// Portugal REUSED from Ep71 (same nation, Rule #26); Croatia generated fresh. Beat captions over
// ANONYMOUS atmosphere backdrops (#27). Nation-correct clips only (#28). #23 name<->image SYNC.

const PT_RED = '#a4161a', PT_GREEN = '#046a38', PT_YELLOW = '#ffd700';
const HR_RED = '#c8102e', HR_WHITE = '#f3f4f7', HR_BLUE = '#171796';
const ACC = '#e0a226';           // gold accent
const GRADE = { filter: 'saturate(1.06) contrast(1.04)' };

function FlagPT({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', display: 'flex', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ width: '40%', background: '#046a38' }} />
      <div style={{ width: '60%', background: '#da291c' }} />
      <div style={{ position: 'absolute', left: '40%', top: '50%', transform: 'translate(-50%,-50%)', width: h * 0.42, height: h * 0.42, borderRadius: '50%', background: '#ffd700', border: `${Math.max(2, w * 0.02)}px solid #b38600`, opacity: 0.95 }} />
    </div>
  );
}
function FlagHR({ w = 120 }) {
  const h = w * 2 / 3;
  const sq = w * 0.055;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <div style={{ position: 'absolute', left: '50%', top: '46%', transform: 'translate(-50%,-50%)', width: sq * 5, height: sq * 5.6, borderRadius: 3, overflow: 'hidden', border: '1px solid rgba(0,0,0,0.25)', background: `repeating-conic-gradient(#c8102e 0% 25%, #ffffff 0% 50%) 0 0 / ${sq * 2}px ${sq * 2}px` }} />
    </div>
  );
}

function FS({ id, style }) { return <ClipSprite id={id} fit="cover" style={{ ...GRADE, ...(style || {}) }} />; }
// Navigator motif from the Legend 088 card art (slow Ken-Burns) — Portugal's cold-open symbol.
function NavigatorStill({ start = 0, dur = 5 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', opacity: fade, zIndex: 1 }}>
      <img data-seq src="assets/legend-088-landscape.png" alt="" style={{ position: 'absolute', left: '50%', top: '50%', width: '108%', transform: `translate(-50%,-50%) scale(${scale})`, filter: 'saturate(1.05) contrast(1.03)' }} />
...
function ScoreBug({ start, pt = 2, hr = 1, note = "BRUNO 88'", badge = 'OUR PREDICTION' }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ ...cell, background: PT_RED }}>POR</div>
        <div style={{ ...cell, fontSize: 38, color: ACC }}>{pt} — {hr}</div>
        <div style={{ ...cell, background: HR_WHITE, color: '#06121a' }}>CRO</div>
...
// BeatCard (#27): a caption over DIMMED anonymous footage — never bare text on dark.
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <NavigatorStill start={0} dur={5} />
      <FS id="portugal-fog" /><FS id="cro-fire" /><FS id="texture-croatia" />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f0dca6', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE NAVIGATOR MEETS THE FIRE.</div>
        </div>
      )}
      {lt > 19.2 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, opacity: titleP, zIndex: 22 }}>
          <Kicker size={32} color={ACC}>The Navigator and The Fire</Kicker>
          <TitleReveal text="THE NAVIGATOR" start={20.0} size={82} color={ACC} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 38 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 88 · Last 32</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 56, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagPT w={210} /></Waving><BigTitle size={62} glow={PT_RED}>PORTUGAL</BigTitle></div>
          <BigTitle size={108} color={ACC}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagHR w={210} /></Waving><BigTitle size={64} glow={HR_RED}>CROATIA</BigTitle></div>
        </div>
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['THE NAVIGATORS', 'PORTUGAL'], ['THE VATRENI', 'CROATIA'], ['THE NAVIGATOR', 'vs THE FIRE'], ['LAST 32', 'WIN OR GO HOME']].map(([v, l], i) => (
...
function ScenePortugal() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={44} end={49}><div style={{ position: 'absolute', inset: 0 }}><FS id="por-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagPT w={64} />} label="OS NAVEGADORES · PORTUGAL" accent={PT_RED} /></div></Sprite>
      <Sprite start={49} end={54}><div style={{ position: 'absolute', inset: 0 }}><FS id="por-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <Sprite start={54} end={55.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="por-surge" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(164,22,26,0.30), transparent 55%)' }} /></div></Sprite>
      <PlayerShowcase clipId="por-ronaldo" name="CRISTIANO RONALDO" role="CAPTAIN · THE ETERNAL · 7" accent={PT_RED} start={55.5} end={60.84} />
      <PlayerShowcase clipId="por-bruno" name="BRUNO FERNANDES" role="THE CONDUCTOR · 8" accent={PT_GREEN} start={60.84} end={64.75} />
      <PlayerShowcase clipId="por-leao" name="RAFAEL LEÃO" role="THE SILK · 11" accent={PT_RED} start={64.75} end={68.0} />
      <PlayerShowcase clipId="por-dias" name="RÚBEN DIAS" role="THE FORTRESS · 4" accent={PT_GREEN} start={68.0} end={70.82} />
      <PlayerShowcase clipId="por-bernardo" name="BERNARDO SILVA" role="THE WEAVER · 10" accent={PT_RED} start={70.82} end={79.5} />
    </div>
  );
}
...
function SceneCroatia() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={79.5} end={84.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="cro-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagHR w={64} />} label="VATRENI · CROATIA" accent={HR_RED} /></div></Sprite>
      <Sprite start={84.5} end={90.6}><div style={{ position: 'absolute', inset: 0 }}><FS id="cro-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <PlayerShowcase clipId="cro-modric" name="LUKA MODRIĆ" role="CAPTAIN · THE MAESTRO · 10" accent={HR_RED} start={90.6} end={95.1} />
      <PlayerShowcase clipId="cro-gvardiol" name="JOŠKO GVARDIOL" role="THE MOUNTAIN · 20" accent={HR_BLUE} start={95.1} end={98.16} />
      <PlayerShowcase clipId="cro-kramaric" name="ANDREJ KRAMARIĆ" role="THE FOX · 9" accent={HR_RED} start={98.16} end={103.0} />
      <PlayerShowcase clipId="cro-kovacic" name="MATEO KOVAČIĆ" role="THE ENGINE · 8" accent={HR_BLUE} start={103.0} end={106.2} />
      <PlayerShowcase clipId="cro-perisic" name="IVAN PERIŠIĆ" role="THE WARRIOR · 4" accent={HR_RED} start={106.2} end={113.0} />
    </div>
  );
}
...
function SceneRiddle() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={113} end={118}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.24)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE NAVIGATOR vs THE FIRE</div></div></Sprite>
      ...
      <Sprite start={123} end={132}><BeatCard clipId="bg-riddle" start={123} end={132} text={<>DOES THE NAVIGATOR CHART GLORY,<br />OR THE FIRE BURN THE MAP?</>} accent={ACC} big={54} /></Sprite>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(224,162,38,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${PT_RED}44 0%, #070b14 38%, #070b14 62%, ${HR_RED}33 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${PT_GREEN} 0%, ${ACC} 50%, ${HR_RED} 100%)`, padding: '13px 0', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '36px 70px 28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 46 }}>
            <Badge flag={<FlagPT w={104} />} name="PORTUGAL" accent={PT_RED} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 116, color: '#fff', lineHeight: 1 }}>
                <span style={{ textShadow: `0 0 36px ${PT_RED}cc` }}>2</span>
                <span style={{ color: ACC, fontSize: 58, transform: 'translateY(-6px)' }}>—</span>
                <span style={{ textShadow: `0 0 36px ${HR_RED}cc` }}>1</span>
              </div>
            ...
            <Badge flag={<FlagHR w={104} />} name="CROATIA" accent={HR_RED} />
          </div>
          <div style={{ textAlign: 'center', marginTop: 16, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 32, color: ACC, letterSpacing: '0.05em', textShadow: `0 0 26px ${ACC}66` }}>THE NAVIGATOR SAILS ON · 2–1</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 16, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#f3e6c4', background: 'rgba(224,162,38,0.16)', border: '1px solid rgba(224,162,38,0.45)', borderRadius: 999, padding: '7px 18px' }}>⛵ BRUNO — THE LATE WINNER</span>
          </div>
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <Sprite start={132} end={137}><BeatCard clipId="bg-firing" start={132} end={137} text={<>CROATIA COME<br />OUT BURNING</>} accent="#ff9a9a" big={62} /></Sprite>
      <Sprite start={137} end={142}><div style={{ position: 'absolute', inset: 0 }}><FS id="keeper-save" /><ChanceTag start={137.5} end={142} text="PORTUGAL RATTLED EARLY" sub="THE FIRE SPREADS" accent={HR_RED} /></div></Sprite>
      <Sprite start={142} end={152.21}><BeatCard clipId="bg-shaken" start={142} end={152.21} text={<>THE NAVIGATORS<br />ARE SHAKEN</>} accent="#fff" big={66} /></Sprite>
      <Sprite start={152.21} end={159}><div style={{ position: 'absolute', inset: 0 }}><FS id="cro-goal" /><GoalFlash at={153.6} /><ChanceTag start={154} end={159} text="KRAMARIĆ — CROATIA LEAD!" sub="THE FIRE DRAWS BLOOD" accent={HR_RED} /></div></Sprite>
      <Sprite start={159} end={172}><BeatCard clipId="bg-rise" start={159} end={172} text={<>BUT THE NAVIGATORS HOLD COURSE</>} sub="PORTUGAL STEADY · THE MAP UNFOLDS" accent={ACC} big={56} /></Sprite>
      <Sprite start={172} end={178.46}><BeatCard clipId="bg-believe" start={172} end={178.46} text={<>THE PRESSURE<br />BUILDS</>} accent="#fff" big={68} /></Sprite>
      <Sprite start={178.46} end={185}><div style={{ position: 'absolute', inset: 0 }}><FS id="por-goal-1" /><GoalFlash at={179.9} /><ChanceTag start={180.3} end={185} text="RONALDO — ONE-ONE!" sub="THE NAVIGATOR FINDS THE WIND" accent={PT_RED} /></div></Sprite>
      <Sprite start={185} end={189.58}><BeatCard clipId="bg-roar" start={185} end={189.58} text={<>AND THEN —<br />THE CONVICTION</>} accent={ACC} big={68} /></Sprite>
      <Sprite start={189.58} end={203.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="por-goal-2" /><GoalFlash at={191.0} /><ChanceTag start={191.4} end={196} text="BRUNO — 2-1 PORTUGAL!" sub="THE NAVIGATOR TAKES THE HORIZON" accent={ACC} /><ScoreBug start={196.3} pt={2} hr={1} note="BRUNO 88'" badge="OUR PREDICTION" /><PredictionCard start={196.9} /></div></Sprite>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={203.32} end={208.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="vd-handshake" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.25) 0%, transparent 45%, rgba(2,3,8,0.55) 100%)' }} /><div style={{ position: 'absolute', left: 0, right: 0, top: 120, textAlign: 'center', zIndex: 24 }}><Kicker size={28} color={ACC}>The Navigator Sails On</Kicker></div></div></Sprite>
      <Sprite start={208.32} end={213.32}><BeatCard clipId="bg-verdict" start={208.32} end={213.32} text={<>CROATIA SCORCHED A GIANT —<br />THE NAVIGATOR FOUND A WAY</>} accent="#fff" big={52} /></Sprite>
      <Sprite start={213.32} end={218.32}><BeatCard clipId="bg-winner" start={213.32} end={218.32} text={<>BRUNO</>} sub="THE WINNER · LATE" accent={ACC} big={64} /></Sprite>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
            <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 900, backdropFilter: 'blur(6px)', opacity: panelP, transform: `translateY(${(1 - panelP) * 24}px)` }}>
              ...
              <div style={{ marginTop: 24 }}>
                <StatLine start={219.5} delay={0.0} label="PORTUGAL" value="OS NAVEGADORES" accent={PT_RED} />
                <StatLine start={219.5} delay={0.25} label="CROATIA" value="THE VATRENI · THE FIRE" accent={HR_RED} />
                <StatLine start={219.5} delay={0.5} label="OUR PREDICTION" value="POR 2 — 1 CRO" accent="#fff" />
                <StatLine start={219.5} delay={0.75} label="88' BRUNO" value="THE LATE WINNER" accent={ACC} />
              </div>
            </div>
          </div>
        </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT PORTUGAL', sub: 'THE NAVIGATORS', flag: <FlagPT w={76} />, accent: PT_RED },
    { label: 'COMMENT CROATIA', sub: 'THE VATRENI', flag: <FlagHR w={76} />, accent: HR_RED },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 140, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: headP }}><Kicker size={30}>The Navigator, Or The Fire?</Kicker></div>
...
function MiniStrip() {
  const prev = ['legend-083-portrait', 'legend-084-portrait', 'legend-085-portrait', 'legend-086-portrait', 'legend-087-portrait'];
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
      ...
      <div style={{ width: 78, height: 104, borderRadius: 9, overflow: 'hidden', border: `2px solid ${ACC}`, boxShadow: `0 10px 28px rgba(0,0,0,0.6), 0 0 26px ${ACC}66` }}>
        <img data-seq alt="" src="assets/legend-088-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      </div>
    </div>
  );
}
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: 90, textAlign: 'center', opacity: teaseP, zIndex: 20 }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f3e0b8', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.7)' }}>THE MASTER OF THE UNKNOWN</div>
        </div>
      )}
      {lt > 1.6 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 22, opacity: clamp(cardP, 0, 1) }}>
          <div style={{ position: 'relative', transform: `perspective(1500px) rotateY(${tilt}deg) scale(${0.92 + 0.08 * Math.min(cardP, 1)})`, marginTop: -34 }}>
            <img data-seq src="assets/legend-088-portrait.png" alt="" style={{ height: 640, display: 'block', borderRadius: 16, boxShadow: '0 50px 130px rgba(0,0,0,0.85), 0 0 80px rgba(224,162,38,0.5)' }} />
            ...
            <div style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', background: 'rgba(224,162,38,0.95)', color: '#1a1306', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 17, letterSpacing: '0.14em', padding: '5px 16px', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.5)' }}>Nº 088 · ✦✦✦ ULTRA RARE</div>
          </div>
        </div>
      )}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, zIndex: 24 }}>
        <div style={{ textAlign: 'center', opacity: txtP, transform: `translateY(${(1 - txtP) * 16}px)` }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 44, color: ACC, letterSpacing: '0.03em', textShadow: '0 2px 24px rgba(224,162,38,0.5)' }}>THE NAVIGATOR · MASTER OF THE UNKNOWN</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 19, color: '#ecd9b0', letterSpacing: '0.18em', marginTop: 6 }}>PORTUGAL · LEGEND 088</div>
        </div>
...
function PhoneCollect({ start }) {
  ...
  const filled = ['legend-084-portrait', 'legend-085-portrait', 'legend-086-portrait', 'legend-087-portrait'];
        ...
        <div style={{ width: '100%', height: '100%', borderRadius: 30, background: 'radial-gradient(ellipse at 50% 20%, #1a2030 0%, #06080c 70%)', overflow: 'hidden', position: 'relative', padding: '34px 18px 18px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            ...
            <div style={{ height: 104, borderRadius: 6, overflow: 'hidden', border: snapped ? `2px solid ${ACC}` : '1px dashed rgba(224,162,38,0.6)', position: 'relative', boxShadow: flash > 0 ? `0 0 ${20 * flash}px ${ACC}` : 'none' }}>
              {snapped && <img data-seq alt="" src="assets/legend-088-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 92, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: p }}>
        <Kicker size={30} color={ACC}>Claim the Navigator</Kicker>
