// match-scenes.jsx — Ep63 Norway vs France — ANIMATION-FIRST + NO-REPEAT (318s).
// "The Titans Collide" · Group I. OUR PREDICTION NOR 2–1 FRA.
// Rule #11: every clip used EXACTLY ONCE. Rule #15: no banned hype wording.
// Rule #16: the Legend (The Boatbuilder) is TEASED in the cold open (cg-tease +
// riddle), echoed mid-episode, and the reveal STATES the spine↔legend connection —
// "greatness is the will to build what will not stop" (the machine's inevitability).
// Mbappé (artist) strikes first, Haaland (machine) equalizes, then his 72' screamer wins it.

const NOR = '#ba0c2f', NOR_DK = '#00205b', FRA = '#0055a4', FRA_RED = '#ef4135';
...
function FlagNOR({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#ba0c2f', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', top: 0, bottom: 0, left: '31%', width: '22%', background: '#fff' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '39%', height: '22%', background: '#fff' }} />
      <div style={{ position: 'absolute', top: 0, bottom: 0, left: '36%', width: '12%', background: '#00205b' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '44%', height: '12%', background: '#00205b' }} />
    </div>
  );
}
...
function ScoreBug({ start, nor = 0, fra = 0, minute, badge = "OUR PREDICTION", note }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        ...
        <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{nor} — {fra}</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        {note && <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 16, color: '#fff', letterSpacing: '0.18em', background: NOR, border: `1px solid ${NOR}`, borderRadius: 999, padding: '4px 16px' }}>{note}</div>}
      </div>
    </div>
  );
}
...
function SceneColdOpen() {
  ...
  // Rule #16: a faint "the hidden hand" riddle label over the Boatbuilder tease clip (10-15s).
  const teaseLabelP = clamp((lt - 10.4) / 0.8, 0, 1) * clamp((15.0 - lt) / 0.6, 0, 1);
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <FS id="cg-norfan" /><FS id="cg-frafan" /><FS id="cg-tease" /><FS id="cg-stadaerial" />
      ...
      {teaseLabelP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseLabelP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#e8c97a', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>WHO REALLY CARRIES NORWAY?</div>
        </div>
      )}
      {lt > 18.0 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 30, opacity: titleP, zIndex: 22 }}>
          <Kicker size={34}>The Artist Meets The Machine</Kicker>
          <TitleReveal text="TITANS COLLIDE" start={19.8} size={104} color={MV.gold} />
        </div>
      )}
      <Letterbox />
    </div>
...
function SceneTitle() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, #0a0f1c 0%, #0b1020 55%, #0a0f1c 100%)' }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 30%, rgba(186,12,47,0.14) 0%, transparent 55%)' }} />
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 40 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 63</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 64, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagNOR w={220} /></Waving><BigTitle size={84} glow={NOR}>NORWAY</BigTitle></div>
          <BigTitle size={116} color={MV.gold}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagFRA w={220} /></Waving><BigTitle size={84} glow="#5b8dff">FRANCE</BigTitle></div>
        </div>
        <div style={{ opacity: p3, fontFamily: '"Inter",sans-serif', fontWeight: 600, fontSize: 32, color: MV.muted, letterSpacing: '0.08em' }}>GROUP I · THE TITANS COLLIDE</div>
      </div>
      <Letterbox />
    </div>
  );
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['GROUP I', 'THE BIG ONE'], ['MBAPPÉ', 'vs HAALAND'], ['ARTIST', 'vs MACHINE'], ['SPEED', 'vs CERTAINTY']].map(([v, l], i) => (
...
function SceneNorway() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={44} end={55.6}><div style={{ position: 'absolute', inset: 0 }}><FS id="nor-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.32) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagNOR w={66} />} label="THE VIKINGS · NORWAY" accent={NOR} /></div></Sprite>
      <PlayerShowcase clipId="s-haaland" name="ERLING HAALAND" role="THE GOAL MACHINE" accent={NOR} start={55.5} end={61.1} />
      <PlayerShowcase clipId="s-odegaard" name="MARTIN ØDEGAARD" role="THE CAPTAIN" accent={NOR_DK} start={61.1} end={65.1} />
      <PlayerShowcase clipId="s-sorloth" name="ALEXANDER SØRLOTH" role="POWER & MOVEMENT" accent={NOR} start={65.1} end={68.1} />
      <PlayerShowcase clipId="s-berge" name="SANDER BERGE" role="THE ENGINE" accent={NOR_DK} start={68.1} end={73.6} />
      <PlayerShowcase clipId="s-ryerson" name="BIRGER RYERSON" role="THE FLANK" accent={NOR} start={73.6} end={79.5} />
...
function SceneFrance() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={79.5} end={90.7}><div style={{ position: 'absolute', inset: 0 }}><FS id="fra-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.32) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagFRA w={66} />} label="LES BLEUS · FRANCE" accent="#5b8dff" /></div></Sprite>
      <PlayerShowcase clipId="s-mbappe" name="KYLIAN MBAPPÉ" role="THE ARTIST" accent={FRA} start={90.6} end={95.2} />
      <PlayerShowcase clipId="s-saliba" name="WILLIAM SALIBA" role="THE WALL" accent={FRA_RED} start={95.2} end={98.2} />
      <PlayerShowcase clipId="s-tchouameni" name="AURÉLIEN TCHOUAMÉNI" role="THE POWER" accent={FRA} start={98.2} end={100.8} />
      <PlayerShowcase clipId="s-dembele" name="OUSMANE DEMBÉLÉ" role="THE TRICKSTER" accent={FRA_RED} start={100.8} end={103} />
      <Vignette strength={0.35} />
      <Letterbox />
    </div>
  );
...
function SceneDuel() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={103} end={113}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-nor" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(186,12,47,0.4), transparent 55%)' }} /><div style={{ position: 'absolute', left: 80, bottom: 130, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE MACHINE<div style={{ fontSize: 27, fontWeight: 700, color: MV.gold, letterSpacing: '0.2em', marginTop: 8 }}>COLD CERTAINTY</div></div></div></Sprite>
      <Sprite start={113} end={122.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.25)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 58, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>NOTHING ELSE MATTERS</div></div></Sprite>
      <Sprite start={122.5} end={132}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-fra" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(270deg, rgba(0,85,164,0.42), transparent 55%)' }} /><div style={{ position: 'absolute', right: 80, bottom: 130, textAlign: 'right', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE ARTIST<div style={{ fontSize: 27, fontWeight: 700, color: MV.gold, letterSpacing: '0.2em', marginTop: 8 }}>BLINDING SPEED</div></div></div></Sprite>
      <Letterbox />
    </div>
  );
}
...
function SceneDrama() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <FS id="dr-fra-attack" /><FS id="dr-mbappe-goal" /><FS id="dr-nor-equalize" /><FS id="dr-endtoend" /><FS id="dr-haaland-run" /><FS id="dr-haaland-goal" /><FS id="dr-eruption" />
      ...
      {/* Mbappé strikes first (FRA), Haaland equalizes (1-1), then Haaland's 72' winner (NOR 2-1) */}
      <GoalFlash at={S + 16.5} />
      <GoalFlash at={S + 27.5} />
      <Sprite start={162.0} end={189.0}><ScoreBug start={S + 30.5} nor={1} fra={1} minute="60'" badge="OUR PREDICTION" /></Sprite>
      <GoalFlash at={S + 59.3} />
      <Sprite start={191.0} end={203.32}><ScoreBug start={S + 59.8} nor={2} fra={1} minute="72'" badge="OUR PREDICTION" note="HAALAND" /></Sprite>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.55)', opacity: p }}>
      <div style={{ width: 980, borderRadius: 26, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 30px 120px rgba(0,0,0,0.75)', position: 'relative', border: `1px solid ${MV.line}` }}>
        <div style={{ background: `linear-gradient(100deg, ${NOR} 0%, #1a1f2e 50%, ${FRA} 100%)`, padding: '14px 0', textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 24, letterSpacing: '0.3em', color: '#fff' }}>FULL-TIME · OUR PREDICTION</div>
        <div style={{ background: 'linear-gradient(160deg, #0d1424 0%, #070b14 100%)', padding: '44px 70px 38px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 48 }}>
            <Badge flag={<FlagNOR w={104} />} name="NORWAY" accent={NOR} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 130, color: '#fff', lineHeight: 1, textShadow: '0 0 40px rgba(255,210,74,0.3)' }}>2<span style={{ color: MV.gold, margin: '0 18px' }}>–</span>1</div>
            </div>
            <Badge flag={<FlagFRA w={104} />} name="FRANCE" accent="#5b8dff" />
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 30, paddingTop: 22, borderTop: `1px solid ${MV.line}` }}>
            <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 24, color: '#f0565d' }}>⚽ HAALAND ×2 · 72' WINNER</div>
            <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 24, color: '#5b8dff' }}>40' MBAPPÉ ⚽</div>
          </div>
          <div style={{ textAlign: 'center', marginTop: 24, fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 22, color: MV.muted, letterSpacing: '0.16em' }}>THE MACHINE HAD THE LAST WORD</div>
        </div>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
        <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 880, backdropFilter: 'blur(6px)', opacity: clamp((lt - 11) / 1.0, 0, 1) }}>
          ...
          <div style={{ marginTop: 24 }}>
            <StatLine start={S + 11.5} delay={0.0} label="NORWAY" value="THE VIKINGS" accent={NOR} />
            <StatLine start={S + 11.5} delay={0.25} label="FRANCE" value="LES BLEUS" accent="#5b8dff" />
            <StatLine start={S + 11.5} delay={0.5} label="OUR PREDICTION" value="NOR 2 — 1 FRA" accent="#fff" />
            <StatLine start={S + 11.5} delay={0.75} label="72' HAALAND WINNER" value="THE MACHINE'S LAST WORD" accent={MV.gold} />
          </div>
        </div>
      </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT MBAPPÉ', sub: "THE ARTIST", flag: <FlagFRA w={78} />, accent: '#5b8dff' },
    { label: 'COMMENT HAALAND', sub: "THE MACHINE", flag: <FlagNOR w={78} />, accent: '#f0565d' },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <FS id="en-crowd" />
...
// Rule #16: the cold-open "hidden hand" riddle pays off here, with the spine line.
function SceneMystery() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <ClipSprite id="my-boat" fit="cover" style={{ filter: 'brightness(0.7) saturate(1.12)' }} />
      ...
      <div style={{ position: 'absolute', top: 70, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: inP }}><Kicker size={26} color="#e8c97a">The Hidden Hand · Legend No. 063</Kicker></div>
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: '50%', top: '50%', zIndex: 24, pointerEvents: 'none', transform: `translate(-50%,-50%) scale(${0.8 + teasePulse * 0.12})`, opacity: teaseP, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
          ...
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 30, color: '#ffe9a0', letterSpacing: '0.2em', textShadow: '0 2px 18px rgba(0,0,0,0.8)' }}>THE ONE WHO CARRIES NORWAY</div>
        </div>
      )}
      ...
      {cardP > 0 && (
        <div style={{ position: 'absolute', left: '50%', top: '49%', zIndex: 25, opacity: clamp(cardP, 0, 1), perspective: 1600, transform: `translate(-50%,-50%) translateY(${(1 - cardP) * 40 + floatY}px) scale(${0.9 + 0.1 * cardP})` }}>
          <div style={{ transformStyle: 'preserve-3d', transform: `rotateY(${flipDeg}deg) rotate(${floatR}deg)`, borderRadius: 22, boxShadow: `0 30px 120px rgba(0,0,0,0.8), 0 0 ${50 + settle * 40}px rgba(245,208,22,${0.2 + settle * 0.3})` }}>
            <img data-seq="" src="assets/legend-063-portrait.png" alt="" style={{ height: 820, width: 'auto', display: 'block', borderRadius: 18 }} />
          </div>
        </div>
      )}
      {settle > 0.2 && (
        <div style={{ position: 'absolute', left: '50%', bottom: 46, zIndex: 26, transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', gap: 16, opacity: settle }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 12, background: 'linear-gradient(100deg, #16a34a, #0e8a3c)', borderRadius: 999, padding: '14px 32px', transform: `scale(${1 + pulse * 0.04})`, boxShadow: `0 8px 30px rgba(22,163,74,${0.35 + pulse * 0.4})` }}><span style={{ fontSize: 22 }}>⚡</span><span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 24, color: '#fff', letterSpacing: '0.03em' }}>SIGN UP FREE — UNLOCK LEGEND 063</span></div>
        </div>
      )}
...
function SceneApp() {
  ...
  const pulse = 0.5 + 0.5 * Math.sin(lt * 3.4);
  const cards = [
    { name: 'NORWAY', flag: <FlagNOR w={84} /> },
    { name: 'FRANCE', flag: <FlagFRA w={84} /> },
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(160deg, #0d1020 0%, #07090f 70%)` }}>
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 50% 20%, rgba(40,60,160,0.4) 0%, rgba(7,9,15,0.92) 62%)` }} />
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 42, opacity: inP }}>
        ...
        <BigTitle size={100} color="#fff" glow={FRA}>worldcup26.world</BigTitle>
        ...
        <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 27, color: MV.gold, opacity: clamp(btnP, 0, 1) }}>Unlock Legend 063 the moment you sign up · free · no prizes</div>
      </div>
      <Letterbox />
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', left: 0, right: 0, bottom: 140, display: 'flex', justifyContent: 'center', zIndex: 26, opacity: p, transform: `translateY(${(1 - p) * 24}px)` }}>
      <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 999, padding: '18px 50px', display: 'flex', alignItems: 'center', gap: 20 }}>
        ...
        <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 29, color: MV.gold }}>⚡ EP64 · COLLECT LEGEND 063 · worldcup26.world</span>
      </div>
    </div>
  );
}
