// match-kit.jsx — shared primitives for the Match 20 video (loads after animations.jsx)
// Dark cinematic broadcast theme. Episode 20 is IMAGE-BASED: Ken-Burns motion on
...
const MV = {
  ...
  irn: '#ce1126',
  irnGreen: '#239f40',
  // New Zealand — All Whites: black / white (silver accent reads on a dark bg)
  nzl: '#e9edf2',
  nzlInk: '#1f1f1f',
  nzlSilver: '#aab4c2',
  green: '#106b4f',
...
// Iran: horizontal green / white / red, with a stylised central emblem token.
function FlagIRN({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', left: 0, right: 0, top: 0, height: '33.34%', background: '#239f40' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '33.33%', height: '33.34%', background: '#fff' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '66.66%', height: '33.34%', background: '#ce1126' }} />
      {/* central emblem token (tulip/Allah abstraction) */}
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: h * 0.30, height: h * 0.30, borderRadius: '50% 50% 50% 50% / 60% 60% 40% 40%', background: '#ce1126' }} />
    </div>
  );
}

// New Zealand: dark-blue field, Union-Jack canton (top-left), four red stars at right.
function FlagNZL({ w = 120 }) {
  const h = w * 2 / 3;
  const star = (top, left, s) => (
    <div style={{ position: 'absolute', top, left, width: s, height: s, background: '#cc142b',
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#00247d', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, width: '40%', height: '50%', background: '#00247d', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '40%', left: 0, right: 0, height: '20%', background: '#fff' }} />
        <div style={{ position: 'absolute', top: 0, bottom: 0, left: '40%', width: '20%', background: '#fff' }} />
        <div style={{ position: 'absolute', top: '44%', left: 0, right: 0, height: '12%', background: '#cf142b' }} />
        <div style={{ position: 'absolute', top: 0, bottom: 0, left: '44%', width: '12%', background: '#cf142b' }} />
      </div>
      {/* four red stars (Southern Cross) on the fly half */}
      {star('20%', '74%', h * 0.14)}
      {star('46%', '60%', h * 0.16)}
      {star('50%', '86%', h * 0.13)}
      {star('74%', '72%', h * 0.16)}
...
function LowerThird({ start, name, role, line, accent = MV.irn }) {
  const t = useTime();
  const local = t - start;
...
// Match scoreboard chip (top center). IRN — NZL.
function ScoreBug({ start, irn = 0, nzl = 0, minute }) {
  const t = useTime();
  const local = t - start;
  if (local < 0) return null;
  ...
  return (
    <div style={{
      ...
      <div style={{ ...cell, background: MV.irn, color: '#fff' }}>IRN</div>
      <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{irn} — {nzl}</div>
      <div style={{ ...cell, background: MV.nzlInk, color: '#fff' }}>NZL</div>
...
function Confetti({ start, dur, count = 90, zIndex = 24, colors }) {
  ...
  if (local < 0 || local > dur) return null;
  const W = 1920, H = 1080;
  const pal = colors || [MV.gold, '#fff', MV.irn, MV.irnGreen];
  const pieces = [];
