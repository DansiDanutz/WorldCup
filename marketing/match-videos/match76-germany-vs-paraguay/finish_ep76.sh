#!/usr/bin/env bash
# Ep76 finish (steps 3-5): body_s.mp4 (crf25, already made) + frames_intro (already rendered)
# -> intro audio -> intro.mp4 -> concat -> final <100MiB.
set -e
cd "$(dirname "$0")"
...
echo "== [4/5] encode intro.mp4 (crf25 grade to match body_s) =="
"$FF" -y -framerate 30 -start_number 0 -i frames_intro/f_%05d.jpg -i intro_audio.m4a \
  ...
  -map "[v]" -map 1:a -c:v libx264 -pix_fmt yuv420p -crf 25 -preset medium -r 30 -c:a aac -b:a 160k -t 15 -movflags +faststart intro.mp4 >/tmp/introenc.log 2>&1
echo "  intro.mp4: $(ls -la intro.mp4 | awk '{print $5}') bytes"

echo "== [5/5] concat intro + body_s -> final =="
printf "file 'intro.mp4'\nfile 'body_s.mp4'\n" > concat_list.txt
OUT="WorldCup26_Ep76_Germany_Paraguay.mp4"
"$FF" -y -f concat -safe 0 -i concat_list.txt -c copy -movflags +faststart "$OUT" >/tmp/concat.log 2>&1 || {
  ...
  "$FF" -y -f concat -safe 0 -i concat_list.txt -c:v libx264 -pix_fmt yuv420p -crf 25 -preset medium -c:a aac -b:a 160k -movflags +faststart "$OUT" >/tmp/concat.log 2>&1; }
echo "  FINAL: $OUT  $(ls -la "$OUT" | awk '{print $5/1048576" MiB"}')"
