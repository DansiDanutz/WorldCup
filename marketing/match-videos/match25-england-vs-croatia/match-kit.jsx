// match-kit.jsx — shared primitives for the Match 25 video (loads after animations.jsx)
// Dark cinematic broadcast theme. Episode 25 is IMAGE-BASED: Ken-Burns motion on
// still PNGs (no generated video clips). SOCCER ONLY — round-neck shirts, a pitch
// with goals; never gridiron.
// NOTE: the historical scene/flag/colour token names are kept as `civ`/`ecu` from
// the Ep15 template to minimise churn, but they now mean ENGLAND (`civ`/home) and
// CROATIA (`ecu`/away). England = white + navy + red; Croatia = red + white check.
...
const MV = {
  ...
  civ: '#1b3a8f',       // England navy (primary accent)
  civGreen: '#cf1020',  // England red (St George's cross / secondary)
  // Croatia — Vatreni: red / white (checkerboard)
  ecu: '#d52b1e',       // Croatia red (primary accent)
  ecuBlue: '#1b3a8f',   // a deep blue used in the kit/crest
  ecuRed: '#d52b1e',
  green: '#106b4f',
...
// England — St George's cross: white field, red cross. (FlagCIV = home/England.)
function FlagCIV({ w = 120 }) {
  const h = w * 2 / 3;
  const bar = Math.max(6, w * 0.13);
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#fff', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <div style={{ position: 'absolute', left: '50%', top: 0, bottom: 0, width: bar, marginLeft: -bar / 2, background: '#cf1020' }} />
      <div style={{ position: 'absolute', top: '50%', left: 0, right: 0, height: bar, marginTop: -bar / 2, background: '#cf1020' }} />
    </div>
  );
}

// Croatia — Vatreni: red / white / blue tricolour with a central checkerboard crest.
function FlagECU({ w = 120 }) {
  const h = w * 2 / 3;
  const sq = h * 0.085; // checker square size
  const cols = 5, rows = 3;
  const checks = [];
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      checks.push(
        <div key={`${r}-${c}`} style={{ position: 'absolute', left: c * sq, top: r * sq, width: sq, height: sq, background: (r + c) % 2 ? '#fff' : '#d52b1e' }} />
      );
    }
  }
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', left: 0, right: 0, top: 0, height: '33.33%', background: '#d52b1e' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '33.33%', height: '33.33%', background: '#fff' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '66.66%', height: '33.34%', background: '#1b3a8f' }} />
      {/* central checkerboard crest (Croatia's šahovnica) */}
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: cols * sq, height: rows * sq, overflow: 'hidden', border: '1.5px solid rgba(255,255,255,0.85)', boxShadow: '0 1px 4px rgba(0,0,0,0.4)' }}>
        {checks}
      </div>
    </div>
  );
...
// Match scoreboard chip (top center). ENG — CRO.
function ScoreBug({ start, civ = 0, ecu = 0, minute }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ ...cell, background: MV.civ, color: '#fff' }}>ENG</div>
      <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{civ} — {ecu}</div>
      <div style={{ ...cell, background: MV.ecu, color: '#fff' }}>CRO</div>
