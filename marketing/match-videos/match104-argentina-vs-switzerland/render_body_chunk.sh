...
export OUT=/dev/shm/ep104/frames FPS=30 DURATION=355.73 QUALITY=90 START=$S END=$E
mkdir -p "$OUT" chunks
node render_local.mjs > /tmp/r104_$N.log 2>&1
tail -1 /tmp/r104_$N.log
CNT=$((E-S))
"$FF" -y -start_number $S -framerate 30 -i "$OUT/f_%05d.jpg" -frames:v $CNT \
  ...
  -c:v libx264 -pix_fmt yuv420p -crf 26 -preset medium -r 30 "chunks/chunk_$(printf %02d $N).mp4" > /tmp/e104_$N.log 2>&1
