// match-scenes.jsx — the twelve scenes of Episode 27, Uzbekistan vs Colombia (300s timeline).
...
// Story spine: Uzbekistan's first-ever World Cup (the White Wolves) set against
// Colombia's golden generation (Los Cafeteros, the Dancers — James, Luis Díaz).
// Legend 027 = the White Wolf. Palette: Uzbekistan blue/white; Colombia yellow
// + blue + red.
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#000' }}>
      {/* a Colombia star — Luis Díaz as the symbol of the moment (dark, dramatic) */}
    ...
    <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(180deg, #0a0f1c 0%, #11182b 55%, #0a0f1c 100%)` }}>
      ...
      {recapP > 0.01 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 22, opacity: recapP }}>
          ...
          <BigTitle size={62} color={MV.text} glow={MV.gold} style={{ maxWidth: 1500 }}>GHANA 2 — 1 PANAMA</BigTitle>
      ...
      {lt >= 9.5 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 40 }}>
          <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}>
            <Kicker color={MV.aut}>WorldCup26 Legends · Episode 27</Kicker>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 70, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20 }}>
              <Waving><FlagUZB w={230} /></Waving>
              <BigTitle size={62} glow={MV.aut}>UZBEKISTAN</BigTitle>
            </div>
            <BigTitle size={120} color={MV.gold}>VS</BigTitle>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20 }}>
              <Waving speed={1.9}><FlagCOL w={230} /></Waving>
              <BigTitle size={62} glow={MV.jor}>COLOMBIA</BigTitle>
            </div>
          </div>
          <div style={{ opacity: p3, transform: `translateY(${(1 - p3) * 26}px)`, fontFamily: '"Inter",sans-serif', fontWeight: 600, fontSize: 32, color: MV.muted, letterSpacing: '0.08em' }}>
            FIRST EVER · 2026 vs THE DANCERS
          </div>
        </div>
      )}
      <Letterbox />
...
// ── 4. History (46.5–150): Uzbekistan's first-ever WC + Colombia's golden era ─
...
function SceneHistory() {
  const S = 46.5;
  const t = useTime();
  // Background switches from an Uzbekistan-tinted plate (White Wolves) to a
  // Colombia-tinted one when the narration pivots to Los Cafeteros (~lt 54 → t≈101).
  const austriaBg = t < 101;
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#05060a' }}>
      {austriaBg ? (
        <VideoSprite src="assets/eldor-shomurodov-act.mp4" start={S} dur={54.5} from={1.1} to={1.24} panX={-20}
          dim={0.72} style={{ filter: 'brightness(0.24) saturate(0.7) contrast(1.1) grayscale(0.35)' }} />
      ) : (
        <VideoSprite src="assets/james-rodriguez-act.mp4" start={101} dur={49} from={1.1} to={1.24} panX={20}
      ...
      <div style={{ position: 'absolute', top: 116, left: 0, right: 0, textAlign: 'center', zIndex: 26 }}>
        ...
        <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 50, color: MV.text, letterSpacing: '0.04em', marginTop: 16, textShadow: '0 4px 22px rgba(0,0,0,0.8)' }}>
          {austriaBg ? 'THE WHITE WOLVES' : 'LOS CAFETEROS'}
        </div>
      </div>
      ...
      <div style={{ position: 'absolute', bottom: 124, left: 0, right: 0, display: 'flex', justifyContent: 'center', zIndex: 26, opacity: 0.95 }}>
        {austriaBg ? <FlagUZB w={120} /> : <FlagCOL w={120} />}
      </div>
      ...
      <HistoryPlate start={S + 65.5} end={S + 76.5} year="2014 · GOLDEN BOOT" venue="THE STAGE THEY OWNED" score="JAMES" accent={MV.jorGreen} stamp="GOLDEN" />
      <HistoryPlate start={S + 76.5} end={S + 87.0} year="LOS CAFETEROS" venue="THE COFFEE GROWERS" score="HISTORY" accent={MV.gold} />
      <HistoryPlate start={S + 87.0} end={S + 103.5} year="2026 · THE DEBUT" venue="FIRST EVER · 2026" score="UZB × COL" accent={MV.jor} stamp="FIRST EVER" />
      <Vignette strength={0.5} />
      <Letterbox />
    </div>
  );
...
// ── 5. Uzbekistan (150–178): the White Wolves ────────────────────────────────
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#0a0c14' }}>
      ...
      <SquadGrid start={S + 0.4} end={S + 7.0} accent={MV.aut} players={[
        { clip: 'assets/eldor-shomurodov.mp4', img: 'assets/squad/uzb-1-Shomurodov.png', name: 'SHOMURODOV', role: 'CAPTAIN · STRIKER' },
        { clip: 'assets/abdukodir-khusanov.mp4', img: 'assets/squad/uzb-2-Khusanov.png', name: 'KHUSANOV', role: 'THE WALL' },
        { clip: 'assets/abbosbek-fayzullaev.mp4', img: 'assets/squad/uzb-3-Fayzullaev.png', name: 'FAYZULLAEV', role: 'THE SPARK' },
        { clip: 'assets/jaloliddin-masharipov.mp4', img: 'assets/squad/uzb-4-Masharipov.png', name: 'MASHARIPOV', role: 'THE CREATOR' },
        { clip: 'assets/dostonbek-khamdamov.mp4', img: 'assets/squad/uzb-5-Khamdamov.png', name: 'KHAMDAMOV', role: 'THE WINGER' },
      ]} />
      {/* line beats: 150 Shomurodov, 160 Fayzullaev/Khusanov, 170 Masharipov */}
      <Sprite start={150.0} end={160.0}>
        <VideoSprite src="assets/eldor-shomurodov-cel.mp4" start={150} dur={10} from={1.04} to={1.14} dim={0.18} style={{ zIndex: 10 }} />
        <LowerThird start={150.4} name="SHOMURODOV" role="Captain · Striker" accent={MV.aut} />
      </Sprite>
      <Sprite start={160.0} end={170.0}>
        <VideoSprite src="assets/abbosbek-fayzullaev-act.mp4" start={160} dur={10} from={1.04} to={1.16} panX={20} dim={0.18} style={{ zIndex: 10 }} />
        <LowerThird start={160.4} name="FAYZULLAEV · KHUSANOV" role="The Midfield" accent={MV.aut} />
      </Sprite>
      <Sprite start={170.0} end={178.0}>
        <VideoSprite src="assets/jaloliddin-masharipov-act.mp4" start={170} dur={8} from={1.04} to={1.14} dim={0.18} style={{ zIndex: 10 }} />
        <LowerThird start={170.3} name="MASHARIPOV" role="The Creator · Midfield" accent={MV.autSoft} />
      </Sprite>
...
// ── 6. Colombia (178–195): Luis Díaz & James Rodríguez ───────────────────────
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#0a0c14' }}>
      ...
      <div style={{ position: 'absolute', top: 108, left: 0, right: 0, display: 'flex', justifyContent: 'center', zIndex: 26, opacity: headerP, transform: `translateY(${(1 - headerP) * -24}px)` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 26, background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 999, padding: '16px 44px' }}>
          <FlagCOL w={74} />
          <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 40, color: MV.text, letterSpacing: '0.10em' }}>COLOMBIA · LOS CAFETEROS · THE DANCERS</span>
        </div>
      </div>
      <SquadGrid start={S + 0.4} end={S + 8.0} accent={MV.jor} players={[
        { clip: 'assets/luis-diaz.mp4', img: 'assets/squad/col-1-Diaz.png', name: 'LUIS DÍAZ', role: 'THE STAR' },
        { clip: 'assets/james-rodriguez.mp4', img: 'assets/squad/col-2-James.png', name: 'JAMES RODRÍGUEZ', role: 'THE MAESTRO' },
        { clip: 'assets/jhon-duran.mp4', img: 'assets/squad/col-3-Duran.png', name: 'JHON DURÁN', role: 'THE POWER' },
        { clip: 'assets/jhon-arias.mp4', img: 'assets/squad/col-4-Arias.png', name: 'JHON ARIAS', role: 'THE ENGINE' },
        { clip: 'assets/davinson-sanchez.mp4', img: 'assets/squad/col-5-Sanchez.png', name: 'DAVINSON SÁNCHEZ', role: 'THE WALL' },
      ]} />
      {/* line beats: 178 Luis Díaz, 187 James Rodríguez */}
      <Sprite start={178.0} end={187.0}>
        ...
        <LowerThird start={178.3} name="LUIS DÍAZ" role="The Star · Winger" accent={MV.jor} />
      </Sprite>
      <Sprite start={187.0} end={195.0}>
        <VideoSprite src="assets/james-rodriguez-cel.mp4" start={187} dur={8} from={1.04} to={1.14} dim={0.2} style={{ zIndex: 10 }} />
        <LowerThird start={187.3} name="JAMES RODRÍGUEZ" role="The Maestro · Playmaker" accent={MV.jorGreen} />
      </Sprite>
      <Vignette strength={0.42} />
      <Letterbox />
    </div>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#05060a', transform: `translate(${shake}px, ${-shake}px)` }}>
      <div style={{ position: 'absolute', top: 0, bottom: 0, left: 0, width: '50%', overflow: 'hidden', transform: `translateX(${(1 - slideP) * -100}%)` }}>
        <VideoSprite src="assets/abdukodir-khusanov-act.mp4" start={195} dur={18.5} fit="cover" />
        ...
        <div style={{ position: 'absolute', bottom: 150, left: 90, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 58, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>
          THE LONG WAIT
          <div style={{ fontSize: 27, fontWeight: 700, color: MV.gold, letterSpacing: '0.2em', marginTop: 8 }}>UZBEKISTAN · FIRST EVER WORLD CUP</div>
        </div>
      </div>
      <div style={{ position: 'absolute', top: 0, bottom: 0, right: 0, width: '50%', overflow: 'hidden', transform: `translateX(${(1 - slideP) * 100}%)` }}>
        <VideoSprite src="assets/fans-uzb-anx.mp4" start={195} dur={18.5} fit="cover" />
        ...
        <div style={{ position: 'absolute', bottom: 150, right: 90, textAlign: 'right', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 58, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>
          THE DANCERS
          <div style={{ fontSize: 27, fontWeight: 700, color: MV.gold, letterSpacing: '0.2em', marginTop: 8 }}>COLOMBIA · PURE JOY</div>
        </div>
      </div>
...
function SceneDrama() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#000' }}>
      <VideoSprite src="assets/fans-uzb-joy.mp4" start={S} dur={19} from={1.1} to={1.28} panX={-30} dim={0.18} />
      <Sprite start={224} end={236}>
        <VideoSprite src="assets/luis-diaz-act.mp4" start={224} dur={12} from={1.08} to={1.24} panX={20} dim={0.16} />
      </Sprite>
      <Sprite start={236} end={247}>
        <VideoSprite src="assets/dostonbek-khamdamov-act.mp4" start={236} dur={11} from={1.08} to={1.22} panX={-18} dim={0.16} />
      </Sprite>
      ...
      <GoalFlash at={S + 8.0} color={MV.jor} />
      <Confetti start={S + 8.2} dur={11} colors={[MV.jor, '#fff', MV.jorGreen]} />
      <Sprite start={221.0} end={232.5}>
        <ScoreBug start={S + 8.0} aut={0} jor={1} minute="DÍAZ" />
      </Sprite>
      {/* Uzbekistan equalise (~232.5, lt 19.5) */}
      <GoalFlash at={S + 19.5} color={MV.aut} />
      <Confetti start={S + 19.7} dur={11} colors={[MV.aut, '#fff', MV.autSoft]} />
      <Sprite start={232.5} end={244.0}>
        <ScoreBug start={S + 19.5} aut={1} jor={1} minute="SHOMURODOV" />
      </Sprite>
      {/* Colombia winner (~244, lt 31) */}
      <GoalFlash at={S + 31.0} color={MV.jor} />
      <Confetti start={S + 31.2} dur={11} colors={[MV.jor, '#fff', MV.jorGreen]} />
      <Sprite start={244.0} end={250.0}>
        <ScoreBug start={S + 31.0} aut={1} jor={2} minute="DURÁN" />
      </Sprite>
...
// ── 9. Verdict (256–266): do you agree? WHITE WOLVES / CAFETEROS ──────────────────
    ...
    <div style={{ position: 'absolute', inset: 0, background: '#0a0f1c' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 70, zIndex: 26, marginTop: 60 }}>
        <div style={{ opacity: clamp(a, 0, 1), transform: `translateY(${(1 - a) * 50}px) scale(${0.85 + 0.15 * a})`, background: 'rgba(237,41,57,0.12)', border: `2px solid ${MV.aut}`, borderRadius: 22, padding: '34px 60px', textAlign: 'center', minWidth: 420 }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 52, color: MV.autSoft, letterSpacing: '0.06em' }}>“WHITE WOLVES”</div>
        ...
        <div style={{ opacity: clamp(b, 0, 1), transform: `translateY(${(1 - b) * 50}px) scale(${0.85 + 0.15 * b})`, background: 'rgba(0,122,61,0.18)', border: `2px solid ${MV.jorGreen}`, borderRadius: 22, padding: '34px 60px', textAlign: 'center', minWidth: 420 }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 52, color: '#3ddc8a', letterSpacing: '0.06em' }}>“CAFETEROS”</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 600, fontSize: 26, color: MV.muted, marginTop: 12 }}></div>
        </div>
      </div>
      <Vignette strength={0.5} />
...
// ── 10. Mystery Supporter (266–284): Legend 027 — the the White Wolf ─────
function SceneMystery() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#02030a' }}>
      <VideoSprite src="assets/davinson-sanchez-cel.mp4" start={S} dur={18} from={1.16} to={1.3} panY={-20}
      ...
      <div style={{ position: 'absolute', top: 118, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: inP }}>
        <Kicker size={26} color="#f4c98a">The Mystery Supporter · Legend No. 027</Kicker>
      </div>
      {plateP > 0 && (
        <div style={{
          ...
          <div style={{ background: 'rgba(14,12,8,0.9)', border: '1px solid rgba(255,200,120,0.4)', borderRadius: 18, padding: '28px 44px', backdropFilter: 'blur(6px)' }}>
            <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff3e2', letterSpacing: '0.02em' }}>THE WHITE WOLF</div>
            <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 24, color: '#f4c98a', letterSpacing: '0.14em', marginTop: 8, maxWidth: 820 }}>FIRST EVER · 2026</div>
...
function SceneApp() {
  ...
  const cards = [
    { name: 'UZBEKISTAN', coef: 'x2.10', pts: '+2.10', flag: <FlagUZB w={86} />, hot: true },
    { name: 'COLOMBIA', coef: 'x3.00', pts: 'UNDERDOG', flag: <FlagCOL w={86} />, hot: true },
