// match-scenes.jsx — Ep99 Argentina vs Egypt (Play-Offs) — PHOTOREAL + NO-REPEAT/NO-LOOP.
// "THE GENIUS AND THE PHARAOH". OUR PREDICTION Argentina 2-1 Egypt (Argentina strike first through
// their young striker; Salah — the Pharaoh — levels it; late, Messi — the genius — strikes the
// winner). Mystic (#21): EGYPT this episode is guarded by Anubis, Warden of the Scales, the ancient
// jackal-headed god who weighs every heart at the gates of the afterlife (a real, documented
// ancient-Egyptian myth, distinct from Legend 090's Sphinx). ARGENTINA = La Albiceleste, carried by
// the Sun of May (already carded as Legend 091 — referenced here as flavor text only, NOT re-carded).
// Argentina REUSED from Ep91 (Rule #26) with a likeness-audit correction: Álvarez re-generated after
// a broadcast-graphic ("LIVE" tag) hallucination was found baked into the Ep91 original;
// Messi/Enzo Fernández/Mac Allister/Lautaro Martínez reused as-is (audited correct). Egypt has NO
// recoverable prior generation (Ep90/Ep67 job URLs are lost) so every Egypt clip is fresh this
// episode. #23 name<->image SYNC (measured VO onsets). Argentina runs 5 named showcases; Egypt runs
// 4 (Elneny cut on a squad-accuracy confidence check, same standard as Ep94/96).

const AR_BLUE = '#74ACDF', AR_WHITE = '#f3f4f7', AR_DARK = '#1c3f6e';
const EG_RED = '#ce1126', EG_BLACK = '#0a0a0a', EG_WHITE = '#f3f4f7', EG_GOLD = '#c09a3e';
const ACC = '#e0a226';           // desert-gold accent (Anubis / the scales)
...
function FlagARG({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <div style={{ flex: 1, background: AR_WHITE, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: h * 0.34, height: h * 0.34, borderRadius: '50%', background: '#f0c419', border: '2px solid #caa000', boxShadow: '0 0 0 1px rgba(0,0,0,0.1)' }} />
      </div>
      <div style={{ flex: 1, background: AR_BLUE }} />
    </div>
  );
}
function FlagEGY({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ flex: 1, background: EG_RED }} />
      <div style={{ flex: 1, background: EG_WHITE, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: h * 0.3, height: h * 0.3, borderRadius: '20%', background: EG_GOLD, transform: 'rotate(45deg)' }} />
      </div>
      <div style={{ flex: 1, background: EG_BLACK }} />
    </div>
  );
}

function FS({ id, style }) { return <ClipSprite id={id} fit="cover" style={{ ...GRADE, ...(style || {}) }} />; }
// Anubis card art (slow Ken-Burns) — Egypt's cold-open symbol this episode.
function GoldStill({ start = 0, dur = 5 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', opacity: fade, zIndex: 1 }}>
      <img data-seq src="assets/legend-099-landscape.png" alt="" style={{ position: 'absolute', left: '50%', top: '50%', width: '108%', transform: `translate(-50%,-50%) scale(${scale})`, filter: 'saturate(1.05) contrast(1.03)' }} />
...
function ScoreBug({ start, arg = 1, egy = 1, note = "88'", badge = 'OUR PREDICTION' }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ ...cell, background: AR_DARK }}>ARG</div>
        <div style={{ ...cell, fontSize: 38, color: ACC }}>{arg} — {egy}</div>
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <FS id="arg-tango" /><FS id="egy-nile" />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f0dca6', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE GENIUS MEETS THE PHARAOH.</div>
        </div>
      )}
      {lt > 15.3 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, opacity: titleP, zIndex: 22 }}>
          <Kicker size={32} color={ACC}>The Genius and The Pharaoh</Kicker>
          <TitleReveal text="ANUBIS" start={16.2} size={64} color={ACC} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 38 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 99 · Play-Offs</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 52, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagARG w={210} /></Waving><BigTitle size={62} glow={AR_BLUE}>ARGENTINA</BigTitle></div>
          <BigTitle size={104} color={ACC}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagEGY w={210} /></Waving><BigTitle size={72} glow={EG_RED}>EGYPT</BigTitle></div>
        </div>
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['LA ALBICELESTE', 'ARGENTINA'], ['THE PHARAOHS', 'EGYPT'], ['THE GENIUS', 'vs THE PHARAOH'], ['PLAY-OFFS', 'WIN OR GO HOME']].map(([v, l], i) => (
...
function SceneArgentina() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={44} end={49}><div style={{ position: 'absolute', inset: 0 }}><FS id="arg-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagARG w={64} />} label="LA ALBICELESTE · ARGENTINA" accent={AR_BLUE} /></div></Sprite>
      ...
      <Sprite start={54} end={55.5}><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(116,172,223,0.18), #05060c 65%)' }} /></Sprite>
      <PlayerShowcase clipId="arg-messi" name="LIONEL MESSI" role="CAPTAIN · THE GENIUS · 10" accent={AR_BLUE} start={55.5} end={60.3} />
      <PlayerShowcase clipId="arg-alvarez" name="JULIÁN ÁLVAREZ" role="THE SPIDER · 9" accent={ACC} start={60.3} end={65.1} />
      <PlayerShowcase clipId="arg-enzo" name="ENZO FERNÁNDEZ" role="THE MAESTRO · 24" accent={AR_BLUE} start={65.1} end={69.9} />
      <PlayerShowcase clipId="arg-macallister" name="MAC ALLISTER" role="THE GLIDER · 20" accent={ACC} start={69.9} end={74.7} />
      <PlayerShowcase clipId="arg-lautaro" name="LAUTARO MARTÍNEZ" role="THE BULL · 22" accent={AR_BLUE} start={74.7} end={79.5} />
    </div>
  );
}
...
function SceneEgypt() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={79.5} end={84.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="egy-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagEGY w={64} />} label="THE PHARAOHS · EGYPT" accent={EG_RED} /></div></Sprite>
      <Sprite start={84.5} end={92.3}><BeatCard start={84.5} end={92.3} text={<>THE PHARAOHS<br />ARRIVE</>} sub="ONE CONTINENT, ONE HOPE" accent={ACC} big={54} /></Sprite>
      <Sprite start={92.3} end={93.8}><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(206,17,38,0.18), #05060c 65%)' }} /></Sprite>
      <PlayerShowcase clipId="egy-salah" name="MOHAMED SALAH" role="CAPTAIN · THE PHARAOH · 10" accent={EG_RED} start={93.8} end={98.6} />
      <PlayerShowcase clipId="egy-marmoush" name="OMAR MARMOUSH" role="THE YOUNG FIRE · 9" accent={ACC} start={98.6} end={103.4} />
      <PlayerShowcase clipId="egy-trezeguet" name="TREZEGUET" role="THE DESERT WIND · 7" accent={EG_RED} start={103.4} end={108.2} />
      <PlayerShowcase clipId="egy-cb" name="THE WALL OF EGYPT" role="THE ANCIENT STONE · 6" accent={ACC} start={108.2} end={113.0} />
    </div>
  );
}
...
function SceneRiddle() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={113} end={118}><div style={{ position: 'absolute', inset: 0 }}><FS id="stadium-aerial" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.24)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE GENIUS vs THE PHARAOH</div></div></Sprite>
      <Sprite start={118} end={123}><NightField o={0.75} /></Sprite>
      <Sprite start={123} end={132}><BeatCard start={123} end={132} text={<>DOES THE GENIUS ADD ONE MORE CHAPTER,<br />OR DOES THE PHARAOH'S CONTINENT GET ITS NIGHT?</>} accent={ACC} big={44} /></Sprite>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(224,162,38,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${AR_BLUE}33 0%, #070b14 38%, #070b14 62%, ${EG_RED}44 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${AR_BLUE} 0%, ${ACC} 50%, ${EG_RED} 100%)`, padding: '13px 0', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '36px 70px 28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 46 }}>
            <Badge flag={<FlagARG w={104} />} name="ARGENTINA" accent={AR_BLUE} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 116, color: '#fff', lineHeight: 1 }}>
                ...
                <span style={{ textShadow: `0 0 36px ${EG_RED}cc` }}>1</span>
              </div>
            ...
            <Badge flag={<FlagEGY w={104} />} name="EGYPT" accent={EG_RED} />
          </div>
          <div style={{ textAlign: 'center', marginTop: 16, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 32, color: ACC, letterSpacing: '0.05em', textShadow: `0 0 26px ${ACC}66` }}>THE GENIUS DELIVERS ONE MORE · 2–1</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 16, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#f3e6c4', background: 'rgba(224,162,38,0.16)', border: '1px solid rgba(224,162,38,0.45)', borderRadius: 999, padding: '7px 18px' }}>⚽ MESSI — THE LATE WINNER</span>
          </div>
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <Sprite start={132} end={138}><BeatCard start={132} end={138} text={<>ARGENTINA COME<br />OUT SHARP</>} accent={AR_BLUE} big={62} /></Sprite>
      <Sprite start={138} end={148.21}><div style={{ position: 'absolute', inset: 0 }}><FS id="arg-attack" /><ChanceTag start={143} end={148.21} text="LA ALBICELESTE SURGE!" sub="THE PHARAOHS' LINE IS STRETCHED THIN" accent={AR_BLUE} /></div></Sprite>
      <Sprite start={148.21} end={160}><div style={{ position: 'absolute', inset: 0 }}><FS id="alvarez-goal" /><GoalFlash at={148.7} /><ChanceTag start={149.2} end={155} text="ARGENTINA LEAD!" sub="ÁLVAREZ STRIKES FIRST" accent={AR_BLUE} /></div></Sprite>
      <Sprite start={160} end={170}><BeatCard clipId="egy-attack" start={160} end={170} text={<>SALAH GOES<br />HUNTING</>} sub="EGYPT NEED AN ANSWER" accent={ACC} big={60} /></Sprite>
      <Sprite start={170} end={182}><div style={{ position: 'absolute', inset: 0 }}><FS id="egy-goal-1" /><GoalFlash at={170.5} /><ChanceTag start={171} end={177} text="SALAH — ONE-ONE!" sub="THE PHARAOH ANSWERS" accent={EG_RED} /></div></Sprite>
      <Sprite start={182} end={192}><BeatCard clipId="messi-magic" start={182} end={192} text={<>THE GENIUS<br />DRIFTS INSIDE</>} accent="#fff" big={58} /></Sprite>
      <Sprite start={192} end={203.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="messi-goal" /><GoalFlash at={192.4} /><ChanceTag start={192.9} end={197.5} text="MESSI — 2-1 ARGENTINA!" sub="THE GENIUS DELIVERS" accent={ACC} /><ScoreBug start={197.6} arg={2} egy={1} note="MESSI 88'" badge="OUR PREDICTION" /><PredictionCard start={198.1} /></div></Sprite>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={203.32} end={213.4}><div style={{ position: 'absolute', inset: 0 }}><FS id="vd-handshake" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.25) 0%, transparent 45%, rgba(2,3,8,0.55) 100%)' }} /><div style={{ position: 'absolute', left: 0, right: 0, top: 120, textAlign: 'center', zIndex: 24 }}><Kicker size={28} color={ACC}>The Genius Delivers One More</Kicker></div></div></Sprite>
      <Sprite start={213.4} end={219}><BeatCard start={213.4} end={219} text={<>EGYPT LEAVE WITH<br />THEIR HEADS HIGH</>} sub="THE PHARAOHS MADE THEM WORK" accent="#fff" big={54} /></Sprite>
      <Sprite start={219} end={225}><BeatCard start={219} end={225} text={<>ARGENTINA<br />MARCH ON</>} sub="LA ALBICELESTE STILL RISING" accent={ACC} big={58} /></Sprite>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
            <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 900, backdropFilter: 'blur(6px)', opacity: panelP, transform: `translateY(${(1 - panelP) * 24}px)` }}>
              ...
              <div style={{ marginTop: 24 }}>
                <StatLine start={225.5} delay={0.0} label="ARGENTINA" value="LA ALBICELESTE" accent={AR_BLUE} />
                <StatLine start={225.5} delay={0.25} label="EGYPT" value="THE PHARAOHS" accent={EG_RED} />
                <StatLine start={225.5} delay={0.5} label="OUR PREDICTION" value="ARG 2 — 1 EGY" accent="#fff" />
                <StatLine start={225.5} delay={0.75} label="88' MESSI" value="THE LATE WINNER" accent={ACC} />
              </div>
            </div>
          </div>
        </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT ARGENTINA', sub: 'LA ALBICELESTE', flag: <FlagARG w={76} />, accent: AR_BLUE },
    { label: 'COMMENT EGYPT', sub: 'THE PHARAOHS', flag: <FlagEGY w={76} />, accent: EG_RED },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      ...
      <div style={{ position: 'absolute', top: 140, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: headP }}><Kicker size={30}>The Genius, Or The Pharaoh?</Kicker></div>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
      ...
      <div style={{ width: 78, height: 104, borderRadius: 9, overflow: 'hidden', border: `2px solid ${ACC}`, boxShadow: `0 10px 28px rgba(0,0,0,0.6), 0 0 26px ${ACC}66` }}>
        <img data-seq alt="" src="assets/legend-099-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      </div>
    </div>
  );
}
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: 90, textAlign: 'center', opacity: teaseP, zIndex: 20 }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f3e0b8', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.7)' }}>ANUBIS</div>
        </div>
      )}
      {lt > 1.6 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 22, opacity: clamp(cardP, 0, 1) }}>
          <div style={{ position: 'relative', transform: `perspective(1500px) rotateY(${tilt}deg) scale(${0.92 + 0.08 * Math.min(cardP, 1)})`, marginTop: -34 }}>
            <img data-seq src="assets/legend-099-portrait.png" alt="" style={{ height: 640, display: 'block', borderRadius: 16, boxShadow: '0 50px 130px rgba(0,0,0,0.85), 0 0 80px rgba(224,162,38,0.5)' }} />
            ...
            <div style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', background: 'rgba(224,162,38,0.95)', color: '#1a1306', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 17, letterSpacing: '0.14em', padding: '5px 16px', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.5)' }}>Nº 099 · ✦✦✦ ULTRA RARE</div>
          </div>
        </div>
      )}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, zIndex: 24 }}>
        <div style={{ textAlign: 'center', opacity: txtP, transform: `translateY(${(1 - txtP) * 16}px)` }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 40, color: ACC, letterSpacing: '0.03em', textShadow: '0 2px 24px rgba(224,162,38,0.5)' }}>ANUBIS · WARDEN OF THE SCALES</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 19, color: '#ecd9b0', letterSpacing: '0.18em', marginTop: 6 }}>EGYPT · LEGEND 099</div>
        </div>
        ...
        <div style={{ width: '100%', height: '100%', borderRadius: 30, background: 'radial-gradient(ellipse at 50% 20%, #1a2030 0%, #06080c 70%)', overflow: 'hidden', position: 'relative', padding: '34px 18px 18px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            ...
            <div style={{ height: 104, borderRadius: 6, overflow: 'hidden', border: snapped ? `2px solid ${ACC}` : '1px dashed rgba(224,162,38,0.6)', position: 'relative', boxShadow: flash > 0 ? `0 0 ${20 * flash}px ${ACC}` : 'none' }}>
              {snapped && <img data-seq alt="" src="assets/legend-099-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
