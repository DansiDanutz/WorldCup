// match-scenes.jsx — Ep69 Croatia vs Ghana — ANIMATION-FIRST + NO-REPEAT (318s).
// "The Warriors and the Black Stars" · Group L. OUR PREDICTION CRO 0–0 GHA (a
// goalless war of will: Livaković a wall, Williams denied, Modrić's 75' turn slides
// Kramarić through, the shot flashes WIDE; neither will bends an inch). Rule #11
// no-repeat · #15 no banned wording · #16 story-woven mystery (Stonemason teased
// "WHAT MAKES A SMALL NATION UNBREAKABLE?" → paid off "the will made solid").
// Rule #17 — animated HOLO COLLECTIBLE reveal. Rule #18 — PREMIUM prediction card.

const CRO = '#ff0000', CRO_DK = '#171796', GHA = '#0a8a4f', GHA_GOLD = '#fcd116', GHA_RED = '#ce1126';
const HERO = { filter: 'brightness(1.05) saturate(1.14) contrast(1.05)' };

function Checkers({ w = 120, rows = 5, cols = 5 }) {
  const cells = [];
  for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++)
    cells.push(<div key={`${r}-${c}`} style={{ position: 'absolute', left: `${c * 100 / cols}%`, top: `${r * 100 / rows}%`, width: `${100 / cols}%`, height: `${100 / rows}%`, background: (r + c) % 2 === 0 ? '#ff0000' : '#fff' }} />);
  return <div style={{ position: 'relative', width: w, height: w * rows / cols, overflow: 'hidden' }}>{cells}</div>;
}
function FlagCRO({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '33.33%', background: '#ff0000' }} />
      <div style={{ position: 'absolute', top: '33.33%', left: 0, right: 0, height: '33.33%', background: '#fff' }} />
      <div style={{ position: 'absolute', top: '66.66%', left: 0, right: 0, height: '33.34%', background: '#171796' }} />
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', border: '1px solid #fff' }}><Checkers w={h * 0.36} rows={5} cols={5} /></div>
    </div>
  );
}
function FlagGHA({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '33.33%', background: '#ce1126' }} />
      <div style={{ position: 'absolute', top: '33.33%', left: 0, right: 0, height: '33.33%', background: '#fcd116' }} />
      <div style={{ position: 'absolute', top: '66.66%', left: 0, right: 0, height: '33.34%', background: '#006b3f' }} />
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', color: '#000', fontSize: h * 0.30, lineHeight: 1 }}>★</div>
    </div>
  );
}

function ScoreBug({ start, cro = 0, gha = 0, minute, badge = "OUR PREDICTION", note }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ ...cell, background: CRO }}>CRO</div>
        <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{cro} — {gha}</div>
        <div style={{ ...cell, background: GHA }}>GHA</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        {note && <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 16, color: '#fff', letterSpacing: '0.18em', background: CRO, border: `1px solid ${CRO}`, borderRadius: 999, padding: '4px 16px' }}>{note}</div>}
      </div>
    </div>
  );
}
...
function Backdrop() { return <div style={{ position: 'absolute', inset: 0, zIndex: 0, background: 'radial-gradient(ellipse at 50% 45%, #14100c 0%, #05060c 100%)' }} />; }
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <FS id="cg-crofan" /><FS id="cg-ghafan" /><FS id="cg-tease" /><FS id="cg-stadaerial" />
      ...
      {teaseLabelP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseLabelP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 28, color: '#e8c97a', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>WHAT MAKES A SMALL NATION UNBREAKABLE?</div>
        </div>
      )}
      {lt > 18.0 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 30, opacity: titleP, zIndex: 22 }}>
          <Kicker size={34}>Two Warrior Nations</Kicker>
          <TitleReveal text="THE WARRIORS" start={19.8} size={112} color={MV.gold} />
        </div>
      )}
      <Letterbox />
    </div>
...
function SceneTitle() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, #14100c 0%, #1c1410 55%, #14100c 100%)' }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 30%, rgba(255,0,0,0.12) 0%, transparent 55%)' }} />
      <AmbientParticles start={23.48} dur={9.5} count={30} color="252,209,22" />
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 40 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 69</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 64, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagCRO w={220} /></Waving><BigTitle size={80} glow={CRO}>CROATIA</BigTitle></div>
          <BigTitle size={116} color={MV.gold}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagGHA w={220} /></Waving><BigTitle size={88} glow={GHA_GOLD}>GHANA</BigTitle></div>
        </div>
        <div style={{ opacity: p3, fontFamily: '"Inter",sans-serif', fontWeight: 600, fontSize: 32, color: MV.muted, letterSpacing: '0.08em' }}>GROUP L · A WAR OF WILL</div>
      </div>
      <Letterbox />
    </div>
  );
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['GROUP L', 'MATCHDAY'], ['VATRENI', 'vs BLACK STARS'], ['MODRIĆ', 'vs PARTEY'], ['4M', 'vs 30M']].map(([v, l], i) => (
...
function SceneCroatia() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={44} end={55.6}><div style={{ position: 'absolute', inset: 0 }}><FS id="cro-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.32) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagCRO w={66} />} label="VATRENI · CROATIA" accent={CRO} /></div></Sprite>
      <PlayerShowcase clipId="s-modric" name="LUKA MODRIĆ" role="THE MAESTRO" accent={CRO} start={55.5} end={61.1} />
      <PlayerShowcase clipId="s-kovacic" name="MATEO KOVAČIĆ" role="THE GLIDER" accent={CRO_DK} start={61.1} end={65.1} />
      <PlayerShowcase clipId="s-gvardiol" name="JOŠKO GVARDIOL" role="THE WALL" accent={CRO} start={65.1} end={68.1} />
      <PlayerShowcase clipId="s-kramaric" name="ANDREJ KRAMARIĆ" role="THE FOX IN THE BOX" accent={CRO_DK} start={68.1} end={73.6} />
      <PlayerShowcase clipId="s-livakovic" name="DOMINIK LIVAKOVIĆ" role="THE WALL IN GOAL" accent={CRO} start={73.6} end={79.5} />
...
function SceneGhana() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={79.5} end={90.7}><div style={{ position: 'absolute', inset: 0 }}><FS id="gha-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.32) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagGHA w={66} />} label="BLACK STARS · GHANA" accent={GHA_GOLD} /></div></Sprite>
      <PlayerShowcase clipId="s-partey" name="THOMAS PARTEY" role="THE GENERAL" accent={GHA} start={90.6} end={95.2} />
      <PlayerShowcase clipId="s-williams" name="IÑAKI WILLIAMS" role="THE FLYER" accent={GHA_GOLD} start={95.2} end={98.2} />
      <PlayerShowcase clipId="s-issahaku" name="FATAWU ISSAHAKU" role="THE FEARLESS" accent={GHA} start={98.2} end={100.8} />
      <PlayerShowcase clipId="s-kudus" name="MOHAMMED KUDUS" role="THE MAGICIAN" accent={GHA_GOLD} start={100.8} end={103} />
      <Vignette strength={0.35} />
      <Letterbox />
    </div>
  );
...
function SceneDuel() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={103} end={113}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-cro" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(255,0,0,0.4), transparent 55%)' }} /><div style={{ position: 'absolute', left: 80, bottom: 130, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE MAESTRO<div style={{ fontSize: 27, fontWeight: 700, color: MV.gold, letterSpacing: '0.2em', marginTop: 8 }}>MODRIĆ AT 40</div></div></div></Sprite>
      <Sprite start={113} end={122.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.25)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 58, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>WILL AGAINST WILL</div></div></Sprite>
      <Sprite start={122.5} end={132}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-gha" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(270deg, rgba(10,138,79,0.44), transparent 55%)' }} /><div style={{ position: 'absolute', right: 80, bottom: 130, textAlign: 'right', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE GENERAL<div style={{ fontSize: 27, fontWeight: 700, color: MV.gold, letterSpacing: '0.2em', marginTop: 8 }}>PARTEY'S STEEL</div></div></div></Sprite>
      <Letterbox />
    </div>
  );
}
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <FS id="dr-cro-attack" /><FS id="dr-gha-williams" /><FS id="dr-livakovic-save" /><FS id="dr-modric-turn" /><FS id="dr-kramaric-wide" /><FS id="dr-gha-chance" /><FS id="dr-end-level" />
      ...
      {/* a goalless war: chances both ways, Modric's 75' pass, Kramaric WIDE, the wall holds */}
      <ChanceTag start={154.0} end={162.0} text="LIVAKOVIĆ — THE WALL" sub="A SAVE FROM NOWHERE" accent="#ff5a5a" />
      <ChanceTag start={164.0} end={172.0} text="75' — MODRIĆ TURNS BACK TIME" sub="THE PASS IS ON" accent={MV.gold} />
      <ChanceTag start={173.0} end={182.0} text="KRAMARIĆ... WIDE!" sub="BY AN INCH" accent="#ff5a5a" />
      <ChanceTag start={184.0} end={192.0} text="OFF THE LINE!" sub="GHANA DENIED" accent="#7be0a7" />
      <Sprite start={193.0} end={203.32}><ScoreBug start={194.0} cro={0} gha={0} minute="FT" badge="OUR PREDICTION" note="WILL UNBROKEN" /></Sprite>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.6)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(255,210,74,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${CRO}26 0%, #070b14 38%, #070b14 62%, ${GHA}33 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${CRO} 0%, #11151f 50%, ${GHA} 100%)`, padding: '13px 0', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '40px 70px 30px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 50 }}>
            <Badge flag={<FlagCRO w={104} />} name="CROATIA" accent={CRO} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 132, color: '#fff', lineHeight: 1 }}>
                <span style={{ textShadow: `0 0 36px ${CRO}99` }}>0</span>
                <span style={{ color: MV.gold, fontSize: 64, transform: 'translateY(-6px)' }}>—</span>
                <span style={{ textShadow: `0 0 36px ${GHA}cc` }}>0</span>
              </div>
            ...
            <Badge flag={<FlagGHA w={104} />} name="GHANA" accent={GHA_GOLD} />
          </div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 26, paddingTop: 22, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#ffd4d8', background: 'rgba(255,0,0,0.16)', border: '1px solid rgba(255,0,0,0.4)', borderRadius: 999, padding: '7px 18px' }}>🧤 LIVAKOVIĆ: THE WALL</span>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#fff0b0', background: 'rgba(252,209,22,0.16)', border: '1px solid rgba(252,209,22,0.45)', borderRadius: 999, padding: '7px 18px' }}>⚽ 75' KRAMARIĆ — WIDE BY AN INCH</span>
          </div>
          <div style={{ textAlign: 'center', marginTop: 22, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 23, color: MV.gold, letterSpacing: '0.16em' }}>★ NEITHER WILL BENT AN INCH ★</div>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, zIndex: 24, background: `linear-gradient(160deg, #14100c 0%, #05060c 100%)`, opacity: clamp((lt - 10.5) / 1.0, 0, 1) }} />
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
        <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 880, backdropFilter: 'blur(6px)', opacity: clamp((lt - 11) / 1.0, 0, 1) }}>
          ...
          <div style={{ marginTop: 24 }}>
            <StatLine start={S + 11.5} delay={0.0} label="CROATIA" value="VATRENI" accent={CRO} />
            <StatLine start={S + 11.5} delay={0.25} label="GHANA" value="THE BLACK STARS" accent={GHA_GOLD} />
            <StatLine start={S + 11.5} delay={0.5} label="OUR PREDICTION" value="CRO 0 — 0 GHA" accent="#fff" />
            <StatLine start={S + 11.5} delay={0.75} label="75' KRAMARIĆ WIDE · LIVAKOVIĆ THE WALL" value="NEITHER WILL BENT AN INCH" accent={MV.gold} />
          </div>
        </div>
      </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT HRVATSKA', sub: "THE MAESTRO", flag: <FlagCRO w={78} />, accent: CRO },
    { label: 'COMMENT GHANA', sub: "THE BLACK STARS", flag: <FlagGHA w={78} />, accent: GHA_GOLD },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <FS id="en-crowd" />
...
function SceneMystery() {
  ...
  const recent = ['legend-064', 'legend-065', 'legend-066', 'legend-067', 'legend-068'];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <ClipSprite id="my-stonemason" fit="cover" style={{ filter: 'brightness(0.62) saturate(1.12)' }} />
      ...
      <div style={{ position: 'absolute', top: 56, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: inP }}><Kicker size={26} color="#e8c97a">The Will Made Solid · Legend No. 069</Kicker></div>
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: '50%', top: '46%', zIndex: 24, pointerEvents: 'none', transform: `translate(-50%,-50%) scale(${0.8 + teasePulse * 0.12})`, opacity: teaseP, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
          ...
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 30, color: '#ffe9a0', letterSpacing: '0.2em', textShadow: '0 2px 18px rgba(0,0,0,0.8)' }}>WALLS THAT NEVER FELL</div>
        </div>
      )}
        ...
        <div style={{ position: 'absolute', left: '50%', top: '44%', zIndex: 25, opacity: clamp(cardP, 0, 1), perspective: 1700, transform: `translate(-50%,-50%) translateY(${(1 - cardP) * 40 + floatY}px) scale(${0.86 + 0.1 * cardP})` }}>
          {unlockP > 0 && (
            <div style={{ position: 'absolute', top: -64, left: '50%', transform: `translateX(-50%) scale(${unlockP})`, zIndex: 28, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, whiteSpace: 'nowrap' }}>
              ...
              <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 17, letterSpacing: '0.3em', color: '#f5d016' }}>Nº 069 · ✦✦✦ ULTRA RARE</div>
            </div>
          )}
          <div style={{ position: 'relative', transformStyle: 'preserve-3d', transform: `rotateY(${flipDeg + tiltX}deg)`, borderRadius: 20, overflow: 'hidden', boxShadow: `0 30px 120px rgba(0,0,0,0.8), 0 0 ${50 + settle * 50}px rgba(245,208,22,${0.22 + settle * 0.33})` }}>
            <img data-seq="" src="assets/legend-069-portrait.png" alt="" style={{ height: 760, width: 'auto', display: 'block' }} />
        ...
        <div style={{ position: 'absolute', left: '50%', bottom: 120, zIndex: 26, transform: `translateX(-50%) translateY(${(1 - stripP) * 30}px)`, opacity: stripP, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
            ...
            <div style={{ width: 96, height: 134, borderRadius: 9, overflow: 'hidden', border: `2px solid ${MV.gold}`, boxShadow: `0 8px 26px rgba(0,0,0,0.6), 0 0 ${16 + pulse * 14}px rgba(245,208,22,0.7)`, transform: `scale(${1 + pulse * 0.03})` }}>
              <img data-seq="" src="assets/legend-069-portrait.png" alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </div>
          </div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 22, color: '#fff', letterSpacing: '0.14em' }}>LEGEND <span style={{ color: MV.gold }}>069</span> OF 66 · <span style={{ color: MV.gold }}>COLLECT THEM ALL</span></div>
        </div>
      )}
      {settle > 0.2 && (
        <div style={{ position: 'absolute', left: '50%', bottom: 46, zIndex: 26, transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', gap: 16, opacity: settle }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 12, background: 'linear-gradient(100deg, #16a34a, #0e8a3c)', borderRadius: 999, padding: '13px 30px', transform: `scale(${1 + pulse * 0.04})`, boxShadow: `0 8px 30px rgba(22,163,74,${0.35 + pulse * 0.4})` }}><span style={{ fontSize: 20 }}>⚡</span><span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 22, color: '#fff', letterSpacing: '0.03em' }}>SIGN UP FREE — CLAIM LEGEND 069</span></div>
        </div>
      )}
...
function SceneApp() {
  ...
  const pulse = 0.5 + 0.5 * Math.sin(lt * 3.4);
  const cards = [
    { name: 'CROATIA', flag: <FlagCRO w={84} /> },
    { name: 'GHANA', flag: <FlagGHA w={84} /> },
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(160deg, #1c1410 0%, #07090f 70%)` }}>
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 50% 20%, rgba(180,40,40,0.4) 0%, rgba(7,9,15,0.92) 62%)` }} />
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 42, opacity: inP }}>
        ...
        <BigTitle size={100} color="#fff" glow={CRO}>worldcup26.world</BigTitle>
        ...
        <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 27, color: MV.gold, opacity: clamp(btnP, 0, 1) }}>Unlock Legend 069 the moment you sign up · free · no prizes</div>
      </div>
      <Letterbox />
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', left: 0, right: 0, bottom: 140, display: 'flex', justifyContent: 'center', zIndex: 26, opacity: p, transform: `translateY(${(1 - p) * 24}px)` }}>
      <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 999, padding: '18px 50px', display: 'flex', alignItems: 'center', gap: 20 }}>
        ...
        <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 29, color: MV.gold }}>⚡ EP70 · NEW LEGEND EVERY MATCH · worldcup26.world</span>
      </div>
    </div>
  );
}
