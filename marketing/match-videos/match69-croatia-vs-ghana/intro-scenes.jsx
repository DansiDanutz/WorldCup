// intro-scenes.jsx — Ep69 · 15s MYSTIC INTRO (rule #20). A wordless, atmospheric
// face-off between the two nations: mist + embers, the two flags rising from shadow,
// a glowing "VS" igniting between them, the WorldCup26 Legends wordmark, then a
// whoosh-fade into the cold open. Reuses FlagCRO/FlagGHA from match-scenes.jsx.
// Concatenated to the FRONT of the finished 318s body → final ≈ 333s. Full-frame.
...
function IntroBackdrop() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 0, background: 'radial-gradient(ellipse at 50% 48%, #16110a 0%, #05060c 70%)' }}>
...
function SceneIntro() {
  ...
  const drift = Easing.easeInOutCubic(clamp((t - 5.8) / 4.0, 0, 1));     // flags ease inward
  ...
  const sideUnit = (flag, name, x, accent) => (
    <div style={{ position: 'absolute', left: '50%', top: '46%', transform: `translate(-50%,-50%) translateX(${x}px)`, opacity: flagP, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 22, zIndex: 12 }}>
      ...
      <div style={{ opacity: namesP, transform: `translateY(${(1 - namesP) * 16}px)`, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 56, letterSpacing: '0.06em', color: '#fff', textShadow: `0 4px 26px rgba(0,0,0,0.9), 0 0 26px ${accent}66` }}>{name}</div>
    </div>
  );
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      {sideUnit(<FlagCRO w={300} />, 'CROATIA', leftX, '#ff3030')}
      {sideUnit(<FlagGHA w={300} />, 'GHANA', rightX, '#fcd116')}
      ...
      <div style={{ position: 'absolute', bottom: 120, left: 0, right: 0, textAlign: 'center', zIndex: 20, opacity: themeP * (1 - fadeOut) }}>
        <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, letterSpacing: '0.34em', color: '#ffe9a0', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>TWO NATIONS · ONE WILL UNBROKEN</div>
      </div>

      <Vignette strength={0.55} />
      {/* whoosh-fade into the cold open */}
