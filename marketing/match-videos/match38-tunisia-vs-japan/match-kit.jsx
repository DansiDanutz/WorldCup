...
// Japan — white field, a centered red circle (Hinomaru)
function FlagJPN({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ width: h * 0.6, height: h * 0.6, borderRadius: '50%', background: '#bc002d' }} />
    </div>
  );
}

// Tunisia — red field, a centered white circle holding a red crescent + a red five-point star
function FlagTUN({ w = 120 }) {
  const h = w * 2 / 3;
  const disc = h * 0.62;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#e70013', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ width: disc, height: disc, borderRadius: '50%', background: '#fff', position: 'relative', overflow: 'hidden' }}>
        {/* crescent: a red disc partly masked by an offset white disc */}
        <div style={{ position: 'absolute', left: '50%', top: '50%', width: disc * 0.62, height: disc * 0.62, borderRadius: '50%', background: '#e70013', transform: 'translate(-58%,-50%)' }} />
        <div style={{ position: 'absolute', left: '50%', top: '50%', width: disc * 0.52, height: disc * 0.52, borderRadius: '50%', background: '#fff', transform: 'translate(-40%,-50%)' }} />
        {/* star nestled inside the crescent */}
        <span style={{ position: 'absolute', left: '62%', top: '50%', transform: 'translate(-50%,-50%)', color: '#e70013', fontSize: disc * 0.42, lineHeight: 1, fontFamily: SANS }}>★</span>
      </div>
    </div>
  );
}
...
// Match scoreboard chip (top center). Ep38 Japan–Tunisia is NOT yet played —
// the scoreline is OUR PREDICTION, so the badge defaults to "OUR PREDICTION",
// never LIVE / a real minute as fact.
function ScoreBug({ start, jpn = 0, tun = 0, minute, badge = "OUR PREDICTION" }) {
  const t = useTime();
  const local = t - start;
  if (local < 0) return null;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ ...cell, background: MV.jpn }}>JPN</div>
        <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{jpn} — {tun}</div>
        <div style={{ ...cell, background: MV.tun }}>TUN</div>
...
function Confetti({ start, dur, count = 90, zIndex = 24 }) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const colors = [MV.gold, '#fff', MV.jpn, MV.tun];
    pieces.push(
      <div key={i} style={{
