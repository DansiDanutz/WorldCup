// match-scenes.jsx — the twelve scenes of the Ep42 video (300s timeline).
// Egypt vs New Zealand · "The Pharaoh Meets the All Whites" · Group G · Houston.
// MATCH NOT YET PLAYED — predicted Egypt 2–1 (Wood 89' header gives NZ the lead,
// then Salah equaliser & Marmoush 90+ winner).
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#000' }}>
      ...
      <ClipSprite id="glimpse-salah" style={{ filter: 'brightness(0.78) contrast(1.15) saturate(1.2)' }} />
      <ClipSprite id="glimpse-wood" style={{ filter: 'brightness(0.78) contrast(1.15) saturate(1.2)' }} />
      <ClipSprite id="glimpse-nz" style={{ filter: 'brightness(0.78) contrast(1.15) saturate(1.2)' }} />
      {/* ember base so the screen never reads as dead air */}
      <div style={{
        position: 'absolute', inset: 0,
        background: `radial-gradient(ellipse at 50% 78%, rgba(206,17,38,0.14) 0%, transparent 55%)`,
      }} />
      {/* heartbeat glow */}
      <div style={{
        position: 'absolute', inset: 0,
        background: `radial-gradient(ellipse at center, rgba(206,17,38,${(0.48 * beat).toFixed(3)}) 0%, transparent 62%)`,
      }} />
      ...
      {lt > 12.6 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 34, opacity: titleP, zIndex: 22 }}>
          <Kicker size={34}>The King vs The Captain</Kicker>
          <TitleReveal text="ONE SURVIVES" start={13.15} size={150} color={MV.gold} />
        </div>
      )}
      <Letterbox />
    </div>
...
function SceneTitle() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(180deg, #0a0f1c 0%, #11182b 55%, #0a0f1c 100%)` }}>
      ...
      <AmbientParticles start={16.55} dur={12} count={34} />
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 44 }}>
        ...
        <div style={{ opacity: p3, transform: `translateY(${(1 - p3) * 26}px)`, fontFamily: '"Inter",sans-serif', fontWeight: 600, fontSize: 32, color: MV.muted, letterSpacing: '0.08em' }}>
          GROUP G · HOUSTON · A PACKED WORLD CUP NIGHT
        </div>
      </div>
      <Letterbox />
    </div>
    ...
    <div style={{ position: 'absolute', inset: 0, background: '#000' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 0, background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['HOUSTON', 'GROUP G'], ['7×', 'KINGS OF AFRICA'], ['OCEANIA', 'THE ALL WHITES'], ['THE KING', 'vs THE CAPTAIN']].map(([v, l], i) => (
      ...
      <div style={{
        ...
        {stamp && stampP > 0 && (
          <div style={{
            ...
            border: `5px solid ${MV.nzl}`, color: MV.nzlLight, borderRadius: 14, padding: '10px 26px',
...
function SceneHistory() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#05060a' }}>
      ...
      <ClipSprite id="hist-egy" dim={0.78} style={{ filter: 'brightness(0.26) saturate(0.7) contrast(1.1)' }} />
      <ClipSprite id="hist-nzl" dim={0.78} style={{ filter: 'brightness(0.26) saturate(0.7) contrast(1.1)' }} />
      <ClipSprite id="hist-salah" dim={0.78} style={{ filter: 'brightness(0.26) saturate(0.7) contrast(1.1)' }} />
      <ClipSprite id="hist-wood" dim={0.78} style={{ filter: 'brightness(0.26) saturate(0.7) contrast(1.1)' }} />
      ...
      <HistoryPlate start={S + 12} end={S + 23} year="THE PHARAOHS" venue="KINGS OF AFRICA" score="EGYPT" accent={MV.egy} />
      <HistoryPlate start={S + 23} end={S + 33.5} year="THE ALL WHITES" venue="GIANTS OF OCEANIA" score="AOTEAROA" accent={MV.nzlLight} />
      <HistoryPlate start={S + 33.5} end={S + 44} year="SALAH'S LAST DANCE" venue="THE KING OF EGYPT" score="7× AFRICA" accent={MV.gold} stamp="LEGEND" />
      <HistoryPlate start={S + 44} end={S + 54} year="2026" venue="A FAIRY TALE ON THE LINE" score="TONIGHT" accent={MV.gold} />
...
// ── 5. Egypt (98.5–133.5): the Pharaohs & Salah's last dance ─────────────────
function SceneEgypt() {
  const { localTime: lt } = useSprite();
  const S = 98.5;
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#0a0c14' }}>
      <ClipSprite id="egy-bg" dim={0.55} />
      <ClipSprite id="salah" dim={0.12} />
      <ClipSprite id="marmoush" dim={0.12} />
      <ClipSprite id="trezeguet" dim={0.12} />
      <ClipSprite id="elshenawy" dim={0.12} />
      <ClipSprite id="abdelkarim" dim={0.12} />
      <div style={{ position: 'absolute', inset: 0, zIndex: 22, pointerEvents: 'none', background: `linear-gradient(90deg, rgba(206,17,38,0.24) 0%, transparent 30%, transparent 70%, rgba(255,210,74,0.14) 100%)` }} />
      <div style={{ position: 'absolute', top: 108, left: 0, right: 0, display: 'flex', justifyContent: 'center', zIndex: 26, opacity: headerP, transform: `translateY(${(1 - headerP) * -24}px)` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 26, background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 999, padding: '16px 44px' }}>
          <FlagEGY w={74} />
          <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 40, color: MV.text, letterSpacing: '0.10em' }}>THE PHARAOHS</span>
        </div>
      </div>
      <SquadGrid start={S + 0.4} end={S + 7.5} accent={MV.egy} players={[
        { img: 'assets/squad/egy-salah.png', name: 'MOHAMED SALAH', role: 'THE KING' },
        { img: 'assets/squad/egy-marmoush.png', name: 'OMAR MARMOUSH', role: 'THE HEIR' },
        { img: 'assets/squad/egy-trezeguet.png', name: 'TREZEGUET', role: 'THE WARRIOR' },
        { img: 'assets/squad/egy-elshenawy.png', name: 'EL-SHENAWY', role: 'THE GLOVES' },
        { img: 'assets/squad/egy-abdelkarim.png', name: 'ABDELKARIM', role: 'THE WALL' },
      ]} />
      <Sprite start={106.00} end={116.50}>
        <LowerThird start={106.50} name="MOHAMED SALAH" role="The King · Forward" accent={MV.egyLight} />
      </Sprite>
      <Sprite start={116.50} end={125.00}>
        <LowerThird start={116.80} name="OMAR MARMOUSH" role="The Heir · Forward" accent={MV.egyLight} />
      </Sprite>
      <Sprite start={125.00} end={133.50}>
        <LowerThird start={125.30} name="TREZEGUET" role="The Warrior · Winger" accent={MV.egyLight} />
      </Sprite>
...
// ── 6. New Zealand (133.5–168.9): the All Whites ─────────────────────────────
function SceneNewZealand() {
  const { localTime: lt } = useSprite();
  const S = 133.5;
  const headerP = Easing.easeOutCubic(clamp(lt / 0.9, 0, 1));
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#0a0c14' }}>
      <ClipSprite id="nzl-bg" dim={0.55} />
      <ClipSprite id="wood" dim={0.12} />
      <ClipSprite id="barbarouses" dim={0.12} />
      <ClipSprite id="cacace" dim={0.12} />
      <ClipSprite id="stamenic" dim={0.12} />
      <ClipSprite id="singh" dim={0.12} />
      <div style={{ position: 'absolute', inset: 0, zIndex: 22, pointerEvents: 'none', background: `linear-gradient(90deg, rgba(0,36,125,0.26) 0%, transparent 30%, transparent 70%, rgba(255,255,255,0.10) 100%)` }} />
      <div style={{ position: 'absolute', top: 108, left: 0, right: 0, display: 'flex', justifyContent: 'center', zIndex: 26, opacity: headerP, transform: `translateY(${(1 - headerP) * -24}px)` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 26, background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 999, padding: '16px 44px' }}>
          <FlagNZL w={74} />
          <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 40, color: MV.text, letterSpacing: '0.10em' }}>THE ALL WHITES</span>
        </div>
      </div>
      <SquadGrid start={S + 0.4} end={S + 11} accent={MV.nzlLight} players={[
        { img: 'assets/squad/nz-wood.png', name: 'CHRIS WOOD', role: 'THE CAPTAIN' },
        { img: 'assets/squad/nz-barbarouses.png', name: 'BARBAROUSES', role: 'THE VETERAN' },
        { img: 'assets/squad/nz-cacace.png', name: 'L. CACACE', role: 'THE FIRE' },
        { img: 'assets/squad/nz-stamenic.png', name: 'STAMENIC', role: 'THE ENGINE' },
        { img: 'assets/squad/nz-singh.png', name: 'SARPREET SINGH', role: 'THE SPARK' },
      ]} />
      <Sprite start={147.10} end={159.46}>
        <LowerThird start={147.47} name="CHRIS WOOD" role="The Captain · Forward" accent={MV.nzlLight} />
      </Sprite>
      <Sprite start={159.46} end={165.02}>
        <LowerThird start={159.83} name="LIBERATO CACACE" role="The Fire · Defender" accent={MV.nzlLight} />
      </Sprite>
      <Sprite start={165.02} end={174.72}>
        <LowerThird start={165.39} name="KOSTA BARBAROUSES" role="The Veteran · Winger" accent={MV.nzlLight} />
      </Sprite>
      <Vignette strength={0.4} />
      <Letterbox />
    </div>
...
// ── 7. The duel (168.9–186): Salah vs Wood split screen ──────────────────────
function SceneDuel() {
  const { localTime: lt } = useSprite();
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#05060a', transform: `translate(${shake}px, ${-shake}px)` }}>
      <div style={{ position: 'absolute', top: 0, bottom: 0, left: 0, width: '50%', overflow: 'hidden', transform: `translateX(${(1 - slideP) * -100}%)` }}>
        <img src="assets/squad/egy-salah.png" alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'saturate(1.15) brightness(0.92)' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(206,17,38,0.32), transparent 65%)' }} />
        <div style={{ position: 'absolute', bottom: 150, left: 90, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 62, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>
          THE KING
          <div style={{ fontSize: 27, fontWeight: 700, color: MV.gold, letterSpacing: '0.2em', marginTop: 8 }}>MOHAMED SALAH</div>
        </div>
      </div>
      <div style={{ position: 'absolute', top: 0, bottom: 0, right: 0, width: '50%', overflow: 'hidden', transform: `translateX(${(1 - slideP) * 100}%)` }}>
        <img src="assets/squad/nz-wood.png" alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'saturate(1.15) brightness(0.92)' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(270deg, rgba(0,36,125,0.42), transparent 65%)' }} />
        <div style={{ position: 'absolute', bottom: 150, right: 90, textAlign: 'right', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 62, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>
          THE CAPTAIN
          <div style={{ fontSize: 27, fontWeight: 700, color: MV.gold, letterSpacing: '0.2em', marginTop: 8 }}>CHRIS WOOD</div>
        </div>
      </div>
      ...
      <Sprite start={182.85} end={186.00}>
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, top: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 27, background: 'rgba(0,0,0,0.55)' }}>
          <Kicker size={40}>The King vs The Captain</Kicker>
        </div>
      </Sprite>
      <Letterbox />
    </div>
...
// ── 8. Match drama (186–242): Wood 0–1, Salah 1–1, Marmoush 2–1 — OUR PREDICTION
...
function SceneDrama() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#000' }}>
      <ClipSprite id="drama-nz1" dim={0.06} />
      <ClipSprite id="drama-wood" dim={0.1} />
      <ClipSprite id="drama-nz-cel" dim={0.1} />
      <ClipSprite id="drama-salah" dim={0.1} />
      <ClipSprite id="drama-marmoush" dim={0.1} />
      <ClipSprite id="drama-egy-cel" dim={0.05} />
      <ClipSprite id="drama-stad" dim={0.08} />

      {/* 89th minute — predicted: Wood rises at the far post, NZ lead */}
      <GoalFlash at={S + 18.5} />
      <Sprite start={186.00} end={213.00}>
        <ScoreBug start={S + 0.4} egy={0} nzl={1} minute="89'" badge="OUR PREDICTION" />
      </Sprite>

      {/* Egypt reply — Salah's left foot, curling, level */}
      <GoalFlash at={S + 27.5} />
      <Sprite start={213.00} end={231.00}>
        <ScoreBug start={S + 27.4} egy={1} nzl={1} minute="90'" badge="OUR PREDICTION" />
      </Sprite>

      {/* The winner — Marmoush slides it home deep in stoppage time */}
      <GoalFlash at={S + 45.8} />
      <Sprite start={231.00} end={234.00}>
        <ScoreBug start={S + 46.2} egy={2} nzl={1} minute="90+5'" badge="OUR PREDICTION" />
      </Sprite>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 26, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 26, padding: '60px 110px', textAlign: 'center', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 30px 120px rgba(0,0,0,0.7)', position: 'relative' }}>
        ...
        <Kicker size={26}>Our Prediction · The King's Last Dance Goes On</Kicker>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
        <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '52px 84px', minWidth: 900, backdropFilter: 'blur(6px)' }}>
          ...
          <div style={{ marginTop: 26 }}>
            <StatLine start={S + 0.8} delay={0.0} label="EGYPT" value="SURVIVING" accent={MV.egyLight} />
            <StatLine start={S + 0.8} delay={0.25} label="NEW ZEALAND" value="HEARTBROKEN" accent={MV.nzlLight} />
            <StatLine start={S + 0.8} delay={0.5} label="OUR PREDICTION" value="EGY 2 — 1 NZL" accent={MV.text} />
            <StatLine start={S + 0.8} delay={0.75} label="90+ MARMOUSH" value="WINS IT" accent={MV.gold} />
          </div>
        </div>
      </div>
      <Vignette strength={0.5} />
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#02030a' }}>
      ...
      <div style={{
        ...
        background: `radial-gradient(ellipse at ${20 + Math.sin(lt * 0.3) * 14}% 75%, rgba(0,36,125,0.18) 0%, transparent 45%),` +
      ...
      <div style={{ position: 'absolute', top: 118, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: inP }}>
        <Kicker size={26} color="#e8c97a">The Mystery Supporter · Legend No. 042</Kicker>
      </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{
                ...
                <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 40, color: '#3a2706' }}>042</span>
              </div>
              <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 22, color: '#e8c97a', letterSpacing: '0.28em' }}>LEGEND 042</div>
              <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: '#f6f9ff', letterSpacing: '0.01em', marginTop: 8, lineHeight: 1.05, maxWidth: 540 }}>THE NAVIGATOR</div>
              <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 22, color: '#9fb6e0', letterSpacing: '0.16em', marginTop: 14, textTransform: 'uppercase' }}>The Old Wayfinder of the Stars · New Zealand</div>
...
function SceneApp() {
  ...
  const cards = [
    { name: 'EGYPT', mult: '3× PER GOAL', flag: <FlagEGY w={86} /> },
    { name: 'NEW ZEALAND', mult: '2× PER GOAL', flag: <FlagNZL w={86} /> },
...
function SceneCTA() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#07090f' }}>
      ...
      <AmbientParticles start={302.05} dur={12} count={28} />
      ...
      <Sprite start={302.00} end={309.84}>
  ...
  return (
    <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, display: 'flex', justifyContent: 'center', zIndex: 26, opacity: p, transform: `translateY(${(1 - p) * 24}px)` }}>
      <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 999, padding: '20px 54px', display: 'flex', alignItems: 'center', gap: 22 }}>
        ...
        <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 30, color: MV.gold }}>⚡ EP43 · COLLECT LEGEND 042 · worldcup26.world</span>
      </div>
    </div>
  );
}
