...
const MV = {
  ...
  egyLight: '#ef3a52',
  nzl: '#111418',          // New Zealand black (All Whites play black/white)
  nzlLight: '#4b5563',
  green: '#106b4f',
...
// Egypt — horizontal tricolour: red / white / black, the gold eagle of Saladin centred on the white band
function FlagEGY({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <span style={{
        position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)',
        color: '#c09a2e', fontSize: h * 0.30, lineHeight: 1, fontFamily: SANS, fontWeight: 900,
      }}>🦅</span>
    </div>
  );
}

// New Zealand — blue field, a Union Jack in the canton, four red five-point stars (white-bordered) on the fly
function FlagNZL({ w = 120 }) {
  const h = w * 2 / 3;
  const star = (left, top, size) => (
    <span style={{
      position: 'absolute', left, top, transform: 'translate(-50%,-50%)',
      color: '#cc142b', fontSize: size, lineHeight: 1, fontFamily: SANS,
      textShadow: '0 0 0 #fff, 1px 0 0 #fff, -1px 0 0 #fff, 0 1px 0 #fff, 0 -1px 0 #fff',
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#00247d', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      {/* Union Jack canton (upper-left quarter) */}
      <div style={{ position: 'absolute', left: 0, top: 0, width: w * 0.5, height: h * 0.5, overflow: 'hidden', background: '#00247d' }}>
        <div style={{ position: 'absolute', left: '-20%', top: '45%', width: '140%', height: '14%', background: '#fff', transform: 'rotate(26deg)' }} />
        <div style={{ position: 'absolute', left: '-20%', top: '45%', width: '140%', height: '14%', background: '#fff', transform: 'rotate(-26deg)' }} />
        <div style={{ position: 'absolute', left: '-20%', top: '49%', width: '140%', height: '7%', background: '#cc142b', transform: 'rotate(26deg)' }} />
        <div style={{ position: 'absolute', left: '-20%', top: '49%', width: '140%', height: '7%', background: '#cc142b', transform: 'rotate(-26deg)' }} />
        <div style={{ position: 'absolute', left: '43%', top: 0, width: '14%', height: '100%', background: '#fff' }} />
        <div style={{ position: 'absolute', left: 0, top: '43%', width: '100%', height: '14%', background: '#fff' }} />
        <div style={{ position: 'absolute', left: '46.5%', top: 0, width: '7%', height: '100%', background: '#cc142b' }} />
        <div style={{ position: 'absolute', left: 0, top: '46.5%', width: '100%', height: '7%', background: '#cc142b' }} />
      </div>
      {/* Southern Cross — four red stars on the fly */}
      {star(w * 0.80, h * 0.24, h * 0.18)}
      {star(w * 0.68, h * 0.50, h * 0.20)}
      {star(w * 0.84, h * 0.62, h * 0.20)}
      {star(w * 0.78, h * 0.82, h * 0.16)}
    </div>
  );
}
...
function LowerThird({ start, name, role, line, accent = MV.egy }) {
  const t = useTime();
  const local = t - start;
...
// Match scoreboard chip (top center). Ep42 is UNPLAYED — the 2–1 is OUR PREDICTION,
// so the badge defaults to "OUR PREDICTION", never LIVE/FULL TIME stated as fact.
function ScoreBug({ start, egy = 0, nzl = 0, minute, badge = "OUR PREDICTION" }) {
  const t = useTime();
  const local = t - start;
  if (local < 0) return null;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ ...cell, background: MV.egy }}>EGY</div>
        <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{egy} — {nzl}</div>
        <div style={{ ...cell, background: MV.nzl, color: '#fff' }}>NZL</div>
