...
// Match scoreboard chip (top center). Ep40 is UNPLAYED — the 2–1 is OUR PREDICTION,
// so the badge defaults to "OUR PREDICTION", never LIVE/FULL TIME stated as fact.
function ScoreBug({ start, bel = 0, irn = 0, minute, badge = "OUR PREDICTION" }) {
  const t = useTime();
  const local = t - start;
  if (local < 0) return null;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ ...cell, background: MV.bel }}>BEL</div>
        <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{bel} — {irn}</div>
