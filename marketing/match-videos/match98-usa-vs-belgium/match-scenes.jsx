// match-scenes.jsx — Ep98 USA vs Belgium (Play-Offs) — PHOTOREAL + NO-REPEAT/NO-LOOP.
// "THE RED DEVIL AND THE TALISMAN". OUR PREDICTION USA 2-1 Belgium (USA strike first; De Bruyne
// levels it 1-1; late, captain Pulisic — the Talisman — strikes the winner). Mystic (#21): BELGIUM =
// the Red Devils (Rode Duivels / Diables Rouges), a real, documented football nickname first given to
// the Belgian national team by the Belgian press in 1906 for their all-red kit and fiery style of
// play -> Legend 098 = The Red Devil, Belgium's Diables Rouges. USA = the Stars and Stripes, carried
// by the American Eagle (already carded as Legend 086 — referenced here as flavor text only, NOT
// re-carded). Both squads REUSED from prior episodes (USA Ep86, Belgium Ep85, Rule #26) with a
// likeness-audit correction: Pulisic/Weah/McKennie/Adams and Onana re-generated with explicit
// shirt-number prompts; Balogun/De Bruyne/Lukaku/Doku/Tielemans reused as-is (audited correct).
// #23 name<->image SYNC (measured VO onsets). Both sides run 5 named showcases — no squad-accuracy
// omission needed this episode (unlike Ep94/96).

const USA_BLUE = '#0a3161', USA_RED = '#b31942', USA_WHITE = '#f3f4f7';
const BEL_RED = '#ef3340', BEL_BLACK = '#1a1a1a', BEL_GOLD = '#fdda24';
const ACC = '#f0a83c';           // ember-gold accent (the Red Devil's fire)
const GRADE = { filter: 'saturate(1.06) contrast(1.04)' };

function FlagUSA({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, overflow: 'hidden', position: 'relative', boxShadow: '0 6px 18px rgba(0,0,0,0.45)', background: 'repeating-linear-gradient(180deg, #b31942 0, #b31942 9.2%, #f3f4f7 9.2%, #f3f4f7 18.4%)' }}>
      <div style={{ position: 'absolute', left: 0, top: 0, width: '42%', height: '54%', background: USA_BLUE }} />
    </div>
  );
}
function FlagBEL({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, display: 'flex', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ flex: 1, background: BEL_BLACK }} />
      <div style={{ flex: 1, background: BEL_GOLD }} />
...
// The Red Devil card art (slow Ken-Burns) — Belgium's cold-open symbol.
function GoldStill({ start = 0, dur = 5 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', opacity: fade, zIndex: 1 }}>
      <img data-seq src="assets/legend-098-landscape.png" alt="" style={{ position: 'absolute', left: '50%', top: '50%', width: '108%', transform: `translate(-50%,-50%) scale(${scale})`, filter: 'saturate(1.05) contrast(1.03)' }} />
...
function NightField({ o = 0.6 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 50% 42%, rgba(240,168,60,${(0.13 * pulse * o).toFixed(3)}) 0%, transparent 55%)` }} />
    </div>
  );
}

function ScoreBug({ start, usa = 1, bel = 1, note = "88'", badge = 'OUR PREDICTION' }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ ...cell, background: USA_BLUE }}>USA</div>
        <div style={{ ...cell, fontSize: 38, color: ACC }}>{usa} — {bel}</div>
        <div style={{ ...cell, background: BEL_RED }}>BEL</div>
        {note && <div style={{ ...cell, fontSize: 24, color: ACC, borderLeft: `1px solid ${MV.line}` }}>{note}</div>}
      </div>
      <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 16, color: ACC, letterSpacing: '0.22em', background: 'rgba(240,168,60,0.14)', border: '1px solid rgba(240,168,60,0.5)', borderRadius: 999, padding: '4px 16px' }}>{badge}</div>
    </div>
  );
}
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <FS id="be-devil" /><FS id="texture-usa" />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at center, rgba(240,168,60,${(0.20 * beat).toFixed(3)}) 0%, transparent 60%)` }} />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f0dca6', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE RED DEVIL MEETS THE TALISMAN.</div>
        </div>
      )}
      {lt > 15.3 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, opacity: titleP, zIndex: 22 }}>
          <Kicker size={32} color={ACC}>The Red Devil and The Talisman</Kicker>
          <TitleReveal text="THE RED DEVIL" start={16.2} size={64} color={ACC} />
        </div>
      )}
    </div>
  );
...
function SceneTitle() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={1} />
      <AmbientParticles start={23.0} dur={10} count={34} color="240,168,60" />
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 38 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 98 · Play-Offs</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 52, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagUSA w={210} /></Waving><BigTitle size={62} glow={USA_RED}>USA</BigTitle></div>
          <BigTitle size={104} color={ACC}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagBEL w={210} /></Waving><BigTitle size={72} glow={BEL_RED}>BELGIUM</BigTitle></div>
        </div>
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['STARS AND STRIPES', 'USA'], ['RED DEVILS', 'BELGIUM'], ['THE TALISMAN', 'vs THE RED DEVIL'], ['PLAY-OFFS', 'WIN OR GO HOME']].map(([v, l], i) => (
...
function SceneUSA() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={44} end={49}><div style={{ position: 'absolute', inset: 0 }}><FS id="us-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagUSA w={64} />} label="STARS AND STRIPES · USA" accent={USA_BLUE} /></div></Sprite>
      ...
      <Sprite start={54} end={55.5}><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(179,25,66,0.18), #05060c 65%)' }} /></Sprite>
      <PlayerShowcase clipId="us-pulisic" name="CHRISTIAN PULISIC" role="CAPTAIN · THE TALISMAN · 10" accent={USA_RED} start={55.5} end={60.3} />
      <PlayerShowcase clipId="us-weah" name="TIMOTHY WEAH" role="THE WINGER · 21" accent={USA_BLUE} start={60.3} end={65.1} />
      <PlayerShowcase clipId="us-balogun" name="FOLARIN BALOGUN" role="THE FINISHER · 9" accent={USA_RED} start={65.1} end={69.9} />
      <PlayerShowcase clipId="us-mckennie" name="WESTON MCKENNIE" role="THE ENGINE · 8" accent={USA_WHITE} start={69.9} end={74.7} />
      <PlayerShowcase clipId="us-adams" name="TYLER ADAMS" role="THE ANCHOR · 4" accent={USA_BLUE} start={74.7} end={79.5} />
    </div>
  );
}
...
function SceneBelgium() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={79.5} end={84.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="be-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagBEL w={64} />} label="RED DEVILS · BELGIUM" accent={BEL_RED} /></div></Sprite>
      <Sprite start={84.5} end={90.6}><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(120deg, rgba(239,51,64,0.16), #05060c 65%)' }} /></Sprite>
      <PlayerShowcase clipId="be-debruyne" name="KEVIN DE BRUYNE" role="CAPTAIN · THE MAESTRO · 7" accent={BEL_RED} start={90.6} end={95.08} />
      <PlayerShowcase clipId="be-lukaku" name="ROMELU LUKAKU" role="THE HAMMER · 9" accent={BEL_GOLD} start={95.08} end={99.56} />
      <PlayerShowcase clipId="be-doku" name="JÉRÉMY DOKU" role="THE FLASH · 11" accent={BEL_RED} start={99.56} end={104.04} />
      <PlayerShowcase clipId="be-tielemans" name="YOURI TIELEMANS" role="THE CONDUCTOR · 8" accent={BEL_GOLD} start={104.04} end={108.52} />
      <PlayerShowcase clipId="be-onana" name="AMADOU ONANA" role="THE WALL · 4" accent={BEL_RED} start={108.52} end={113.0} />
    </div>
  );
}
...
function SceneRiddle() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={113} end={118}><div style={{ position: 'absolute', inset: 0 }}><FS id="stadium-aerial" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.24)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE RED DEVIL vs THE TALISMAN</div></div></Sprite>
      <Sprite start={118} end={123}><NightField o={0.75} /></Sprite>
      <Sprite start={123} end={132}><BeatCard start={123} end={132} text={<>DOES THE RED DEVIL BURN BRIGHTER,<br />OR DOES THE TALISMAN DELIVER LATE?</>} accent={ACC} big={48} /></Sprite>
    </div>
  );
}
...
function PredictionCard({ start }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(240,168,60,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${USA_BLUE}33 0%, #070b14 38%, #070b14 62%, ${BEL_RED}44 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${USA_BLUE} 0%, ${ACC} 50%, ${BEL_RED} 100%)`, padding: '13px 0', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '36px 70px 28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 46 }}>
            <Badge flag={<FlagUSA w={104} />} name="USA" accent={USA_BLUE} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 116, color: '#fff', lineHeight: 1 }}>
                <span style={{ textShadow: `0 0 36px ${USA_BLUE}cc` }}>2</span>
            ...
            <Badge flag={<FlagBEL w={104} />} name="BELGIUM" accent={BEL_RED} />
          </div>
          <div style={{ textAlign: 'center', marginTop: 16, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 32, color: ACC, letterSpacing: '0.05em', textShadow: `0 0 26px ${ACC}66` }}>THE TALISMAN DELIVERS LATE · 2–1</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 16, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#f3e6c4', background: 'rgba(240,168,60,0.16)', border: '1px solid rgba(240,168,60,0.45)', borderRadius: 999, padding: '7px 18px' }}>🔥 PULISIC — THE LATE WINNER</span>
          </div>
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <Sprite start={132} end={138}><BeatCard start={132} end={138} text={<>USA COME<br />OUT SHARP</>} accent={USA_BLUE} big={62} /></Sprite>
      <Sprite start={138} end={148.21}><div style={{ position: 'absolute', inset: 0 }}><FS id="us-attack" /><ChanceTag start={143} end={148.21} text="THE STARS AND STRIPES SURGE!" sub="THE WALL IS STRETCHED THIN" accent={USA_BLUE} /></div></Sprite>
      <Sprite start={148.21} end={160}><div style={{ position: 'absolute', inset: 0 }}><FS id="us-goal-1" /><GoalFlash at={148.7} /><ChanceTag start={149.2} end={155} text="USA LEAD!" sub="THE YOUNG ATTACK STRIKES" accent={USA_BLUE} /></div></Sprite>
      <Sprite start={160} end={170}><BeatCard clipId="be-attack" start={160} end={170} text={<>DE BRUYNE GOES<br />HUNTING</>} sub="BELGIUM NEED AN ANSWER" accent={ACC} big={60} /></Sprite>
      <Sprite start={170} end={182}><div style={{ position: 'absolute', inset: 0 }}><FS id="be-goal" /><GoalFlash at={170.5} /><ChanceTag start={171} end={177} text="DE BRUYNE — ONE-ONE!" sub="THE MAESTRO DELIVERS" accent={BEL_RED} /></div></Sprite>
      <Sprite start={182} end={192}><BeatCard clipId="us-surge" start={182} end={192} text={<>THE TALISMAN SENSES<br />THE MOMENT</>} accent="#fff" big={58} /></Sprite>
      <Sprite start={192} end={203.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="us-goal-2" /><GoalFlash at={192.4} /><ChanceTag start={192.9} end={197.5} text="PULISIC — 2-1 USA!" sub="THE TALISMAN DELIVERS" accent={ACC} /><ScoreBug start={197.6} usa={2} bel={1} note="PULISIC 88'" badge="OUR PREDICTION" /><PredictionCard start={198.1} /></div></Sprite>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={203.32} end={213.4}><div style={{ position: 'absolute', inset: 0 }}><FS id="vd-handshake" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.25) 0%, transparent 45%, rgba(2,3,8,0.55) 100%)' }} /><div style={{ position: 'absolute', left: 0, right: 0, top: 120, textAlign: 'center', zIndex: 24 }}><Kicker size={28} color={ACC}>The Talisman Delivers Late</Kicker></div></div></Sprite>
      <Sprite start={213.4} end={219}><BeatCard start={213.4} end={219} text={<>BELGIUM LEAVE WITH<br />THEIR HEADS HIGH</>} sub="THE RED DEVILS MADE THEM WORK" accent="#fff" big={54} /></Sprite>
      <Sprite start={219} end={225}><BeatCard start={219} end={225} text={<>USA<br />MARCH ON</>} sub="THE GOLDEN GENERATION DELIVERS" accent={ACC} big={58} /></Sprite>
      <Sprite start={225} end={244}>
        <div style={{ position: 'absolute', inset: 0 }}>
          ...
          <AmbientParticles start={225} dur={19} count={26} color="240,168,60" />
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
            <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 900, backdropFilter: 'blur(6px)', opacity: panelP, transform: `translateY(${(1 - panelP) * 24}px)` }}>
              ...
              <div style={{ marginTop: 24 }}>
                <StatLine start={225.5} delay={0.0} label="USA" value="STARS AND STRIPES" accent={USA_BLUE} />
                <StatLine start={225.5} delay={0.25} label="BELGIUM" value="RED DEVILS" accent={BEL_RED} />
                <StatLine start={225.5} delay={0.5} label="OUR PREDICTION" value="USA 2 — 1 BEL" accent="#fff" />
                <StatLine start={225.5} delay={0.75} label="88' PULISIC" value="THE LATE WINNER" accent={ACC} />
              </div>
            </div>
          </div>
        </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT BELGIUM', sub: 'RED DEVILS', flag: <FlagBEL w={76} />, accent: BEL_RED },
    { label: 'COMMENT USA', sub: 'STARS AND STRIPES', flag: <FlagUSA w={76} />, accent: USA_BLUE },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <AmbientParticles start={244} dur={11} count={30} color="240,168,60" />
      <div style={{ position: 'absolute', inset: 0, background: 'rgba(6,10,18,0.58)' }} />
      <div style={{ position: 'absolute', top: 140, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: headP }}><Kicker size={30}>The Red Devil, Or The Talisman?</Kicker></div>
...
function MiniStrip() {
  const prev = ['legend-094-portrait', 'legend-095-portrait', 'legend-096-portrait', 'legend-097-portrait'];
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
      ...
      <div style={{ width: 78, height: 104, borderRadius: 9, overflow: 'hidden', border: `2px solid ${ACC}`, boxShadow: `0 10px 28px rgba(0,0,0,0.6), 0 0 26px ${ACC}66` }}>
        <img data-seq alt="" src="assets/legend-098-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      </div>
    </div>
  );
}
...
function SceneMystery() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 50% 44%, rgba(240,168,60,${(0.32 * glow).toFixed(3)}) 0%, transparent 55%)` }} />
      <AmbientParticles start={255} dur={26} count={56} color="240,182,90" maxR={4.2} />
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: 90, textAlign: 'center', opacity: teaseP, zIndex: 20 }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f3e0b8', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.7)' }}>THE RED DEVIL</div>
        </div>
      )}
      {lt > 1.6 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 22, opacity: clamp(cardP, 0, 1) }}>
          <div style={{ position: 'relative', transform: `perspective(1500px) rotateY(${tilt}deg) scale(${0.92 + 0.08 * Math.min(cardP, 1)})`, marginTop: -34 }}>
            <img data-seq src="assets/legend-098-portrait.png" alt="" style={{ height: 640, display: 'block', borderRadius: 16, boxShadow: '0 50px 130px rgba(0,0,0,0.85), 0 0 80px rgba(240,168,60,0.5)' }} />
            ...
            <div style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', background: 'rgba(240,168,60,0.95)', color: '#1a1306', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 17, letterSpacing: '0.14em', padding: '5px 16px', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.5)' }}>Nº 098 · ✦✦✦ ULTRA RARE</div>
          </div>
        </div>
      )}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, zIndex: 24 }}>
        <div style={{ textAlign: 'center', opacity: txtP, transform: `translateY(${(1 - txtP) * 16}px)` }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 40, color: ACC, letterSpacing: '0.03em', textShadow: '0 2px 24px rgba(240,168,60,0.5)' }}>THE RED DEVIL · BELGIUM'S DIABLES ROUGES</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 19, color: '#ecd9b0', letterSpacing: '0.18em', marginTop: 6 }}>BELGIUM · LEGEND 098</div>
        </div>
...
function PhoneCollect({ start }) {
  ...
  const filled = ['legend-095-portrait', 'legend-096-portrait', 'legend-097-portrait'];
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 26, opacity: inP }}>
      <div style={{ position: 'relative', width: 300, height: 600, borderRadius: 40, background: 'linear-gradient(160deg,#23262e,#0c0e13)', border: '3px solid #2b2f38', boxShadow: '0 40px 110px rgba(0,0,0,0.8), 0 0 50px rgba(240,168,60,0.25)', padding: 12 }}>
        ...
        <div style={{ width: '100%', height: '100%', borderRadius: 30, background: 'radial-gradient(ellipse at 50% 20%, #1a2030 0%, #06080c 70%)', overflow: 'hidden', position: 'relative', padding: '34px 18px 18px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            ...
            <div style={{ height: 104, borderRadius: 6, overflow: 'hidden', border: snapped ? `2px solid ${ACC}` : '1px dashed rgba(240,168,60,0.6)', position: 'relative', boxShadow: flash > 0 ? `0 0 ${20 * flash}px ${ACC}` : 'none' }}>
              {snapped && <img data-seq alt="" src="assets/legend-098-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
...
function SceneApp() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.8} />
      <AmbientParticles start={282.5} dur={21} count={24} color="240,168,60" />
