// intro-scenes.jsx — Ep98 · 15s MYSTIC INTRO (rule #20). Wordless, atmospheric face-off: the
// Red Devil and the Talisman, the two flags rising from shadow, an igniting "VS", the
// WorldCup26 Legends wordmark, then a whoosh-fade into the cold open. Reuses FlagUSA/FlagBEL from
// match-scenes.jsx.
...
function IntroBackdrop() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 0, background: 'radial-gradient(ellipse at 50% 48%, #0c1020 0%, #05060c 70%)' }}>
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 50% 50%, rgba(240,168,60,${(0.10 * glow).toFixed(3)}) 0%, transparent 55%)` }} />
    </div>
  );
}
...
function SceneIntro() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <IntroBackdrop />
      <AmbientParticles start={0} dur={INTRO_DUR} count={64} color="240,182,90" maxR={3.6} zIndex={6} />
      ...
      {sideUnit(<FlagUSA w={300} />, 'USA', leftX, '#b31942')}
      {sideUnit(<FlagBEL w={300} />, 'BELGIUM', rightX, '#ef3340')}
      {ring > 0 && ring < 1 && <div style={{ position: 'absolute', left: '50%', top: '46%', zIndex: 14, pointerEvents: 'none', width: 620, height: 620, transform: `translate(-50%,-50%) scale(${0.3 + ring * 1.6})`, opacity: (1 - ring) * 0.85, borderRadius: '50%', border: '3px solid rgba(255,225,150,0.75)', boxShadow: '0 0 70px rgba(240,168,60,0.6)' }} />}
      {vsP > 0 && (
        <div style={{ position: 'absolute', left: '50%', top: '46%', transform: `translate(-50%,-50%) scale(${vsP})`, zIndex: 16, opacity: 1 - fadeOut }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 150, color: MV.gold, textShadow: `0 0 ${30 + pulse * 30}px rgba(240,168,60,0.9), 0 6px 30px rgba(0,0,0,0.9)` }}>VS</div>
        </div>
      )}
      ...
      <div style={{ position: 'absolute', bottom: 120, left: 0, right: 0, textAlign: 'center', zIndex: 20, opacity: themeP * (1 - fadeOut) }}>
        <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, letterSpacing: '0.13em', color: '#ffe9a0', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE RED DEVIL AND THE TALISMAN</div>
      </div>
