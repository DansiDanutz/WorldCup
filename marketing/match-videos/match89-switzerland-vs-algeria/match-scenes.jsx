// match-scenes.jsx — Ep89 Switzerland vs Algeria (R32) — PHOTOREAL + NO-REPEAT/NO-LOOP.
// "THE MOUNTAIN AND THE FOX". OUR PREDICTION Switzerland 2-1 Algeria (Mahrez early; Embolo
// equalise; Xhaka late winner). Mystic (#21): SWITZERLAND = the Mountain, Guardian of the Alps;
// ALGERIA = the Desert Foxes (Les Fennecs) -> Legend 089 = the Mountain.
// Algeria REUSED from Ep73 under the SAME names (Rule #26 + #29 likeness QA PASSED: Mahrez #7
// armband verified). Switzerland generated fresh with LIKENESS prompts + real numbers (#29).
// Beat captions over ANONYMOUS/own-nation dimmed footage (#27). Nation-correct only (#28).
// #23 name<->image SYNC (measured VO onsets).

const CH_RED = '#da291c', CH_WHITE = '#f3f4f7';
const DZ_GREEN = '#006233', DZ_RED = '#d21034';
const ACC = '#e0a226';
const GRADE = { filter: 'saturate(1.06) contrast(1.04)' };

function FlagCH({ w = 120 }) {
  const h = w * 2 / 3;
  const bar = h * 0.20, len = h * 0.62;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#da291c', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: bar, height: len, background: '#fff' }} />
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: len, height: bar, background: '#fff' }} />
    </div>
  );
}
function FlagDZ({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', display: 'flex', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-58%,-50%)', width: h * 0.46, height: h * 0.46, borderRadius: '50%', border: `${Math.max(2, h * 0.055)}px solid #d21034`, borderRightColor: 'transparent' }} />
      <div style={{ position: 'absolute', left: '52%', top: '50%', transform: 'translate(-50%,-50%)', color: '#d21034', fontSize: h * 0.30, lineHeight: 1 }}>★</div>
    </div>
  );
}

function FS({ id, style }) { return <ClipSprite id={id} fit="cover" style={{ ...GRADE, ...(style || {}) }} />; }
// Mountain motif from the Legend 089 card art (slow Ken-Burns) — Switzerland's cold-open symbol.
function MountainStill({ start = 0, dur = 5 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', opacity: fade, zIndex: 1 }}>
      <img data-seq src="assets/legend-089-landscape.png" alt="" style={{ position: 'absolute', left: '50%', top: '50%', width: '108%', transform: `translate(-50%,-50%) scale(${scale})`, filter: 'saturate(1.05) contrast(1.03)' }} />
...
function ScoreBug({ start, ch = 2, dz = 1, note = "XHAKA 88'", badge = 'OUR PREDICTION' }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ ...cell, background: CH_RED }}>SUI</div>
        <div style={{ ...cell, fontSize: 38, color: ACC }}>{ch} — {dz}</div>
        <div style={{ ...cell, background: DZ_GREEN }}>ALG</div>
...
// BeatCard (#27): caption over DIMMED footage — never bare text on dark.
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <MountainStill start={0} dur={5} />
      <FS id="sui-mountain" /><FS id="alg-tuareg" /><FS id="alg-tassili" />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f0dca6', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE MOUNTAIN MEETS THE FOX.</div>
        </div>
      )}
      {lt > 19.2 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, opacity: titleP, zIndex: 22 }}>
          <Kicker size={32} color={ACC}>The Mountain and The Fox</Kicker>
          <TitleReveal text="THE MOUNTAIN" start={20.0} size={84} color={ACC} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 38 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 89 · Last 32</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 56, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagCH w={210} /></Waving><BigTitle size={54} glow={CH_RED}>SWITZERLAND</BigTitle></div>
          <BigTitle size={108} color={ACC}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagDZ w={210} /></Waving><BigTitle size={64} glow={DZ_GREEN}>ALGERIA</BigTitle></div>
        </div>
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['KNOCKOUT KINGS', 'SWITZERLAND'], ['DESERT FOXES', 'ALGERIA'], ['THE MOUNTAIN', 'vs THE FOX'], ['LAST 32', 'WIN OR GO HOME']].map(([v, l], i) => (
...
function SceneSwitzerland() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={44} end={49}><div style={{ position: 'absolute', inset: 0 }}><FS id="sui-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagCH w={64} />} label="THE MOUNTAIN · SWITZERLAND" accent={CH_RED} /></div></Sprite>
      <Sprite start={49} end={54}><div style={{ position: 'absolute', inset: 0 }}><FS id="sui-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <Sprite start={54} end={55.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="sui-surge" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(218,41,28,0.30), transparent 55%)' }} /></div></Sprite>
      <PlayerShowcase clipId="sui-xhaka" name="GRANIT XHAKA" role="CAPTAIN · THE GENERAL · 10" accent={CH_RED} start={55.5} end={60.5} />
      <PlayerShowcase clipId="sui-embolo" name="BREEL EMBOLO" role="THE BATTERING RAM · 7" accent={CH_WHITE} start={60.5} end={63.74} />
      <PlayerShowcase clipId="sui-akanji" name="MANUEL AKANJI" role="THE KING OF CALM · 5" accent={CH_RED} start={63.74} end={68.0} />
      <PlayerShowcase clipId="sui-vargas" name="RUBEN VARGAS" role="THE FLYER · 17" accent={CH_WHITE} start={68.0} end={70.99} />
      <PlayerShowcase clipId="sui-sommer" name="YANN SOMMER" role="THE WALL · 1" accent={CH_RED} start={70.99} end={79.5} />
    </div>
  );
}
...
function SceneAlgeria() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={79.5} end={84.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="alg-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagDZ w={64} />} label="LES FENNECS · ALGERIA" accent={DZ_GREEN} /></div></Sprite>
      <Sprite start={84.5} end={90.6}><div style={{ position: 'absolute', inset: 0 }}><FS id="alg-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <PlayerShowcase clipId="alg-mahrez" name="RIYAD MAHREZ" role="CAPTAIN · THE MAGICIAN · 7" accent={DZ_GREEN} start={90.6} end={96.8} />
      <PlayerShowcase clipId="alg-bennacer" name="ISMAËL BENNACER" role="THE METRONOME · 8" accent={DZ_RED} start={96.8} end={99.95} />
      <PlayerShowcase clipId="alg-gouiri" name="AMINE GOUIRI" role="THE SPEARHEAD · 11" accent={DZ_GREEN} start={99.95} end={103.0} />
      <PlayerShowcase clipId="alg-boudaoui" name="HICHAM BOUDAOUI" role="THE ENGINE" accent={DZ_RED} start={103.0} end={105.71} />
      <PlayerShowcase clipId="alg-mandi" name="AÏSSA MANDI" role="THE WALL · 3" accent={DZ_GREEN} start={105.71} end={113.0} />
    </div>
  );
}
...
function SceneRiddle() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={113} end={118}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.24)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE MOUNTAIN vs THE FOX</div></div></Sprite>
      ...
      <Sprite start={123} end={132}><BeatCard clipId="bg-riddle" start={123} end={132} text={<>DOES THE MOUNTAIN STAND,<br />OR THE FOX FIND THE PATH?</>} accent={ACC} big={56} /></Sprite>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(224,162,38,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${CH_RED}44 0%, #070b14 38%, #070b14 62%, ${DZ_GREEN}44 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${CH_RED} 0%, ${ACC} 50%, ${DZ_GREEN} 100%)`, padding: '13px 0', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '36px 70px 28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 46 }}>
            <Badge flag={<FlagCH w={104} />} name="SWITZERLAND" accent={CH_RED} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 116, color: '#fff', lineHeight: 1 }}>
                <span style={{ textShadow: `0 0 36px ${CH_RED}cc` }}>2</span>
                <span style={{ color: ACC, fontSize: 58, transform: 'translateY(-6px)' }}>—</span>
                <span style={{ textShadow: `0 0 36px ${DZ_GREEN}cc` }}>1</span>
              </div>
            ...
            <Badge flag={<FlagDZ w={104} />} name="ALGERIA" accent={DZ_GREEN} />
          </div>
          <div style={{ textAlign: 'center', marginTop: 16, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 32, color: ACC, letterSpacing: '0.05em', textShadow: `0 0 26px ${ACC}66` }}>THE MOUNTAIN STANDS · 2–1</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 16, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#f3e6c4', background: 'rgba(224,162,38,0.16)', border: '1px solid rgba(224,162,38,0.45)', borderRadius: 999, padding: '7px 18px' }}>⛰️ XHAKA — THE LATE WINNER</span>
          </div>
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <Sprite start={132} end={137}><BeatCard clipId="bg-firing" start={132} end={137} text={<>ALGERIA COME<br />OUT RACING</>} accent="#9fe3a0" big={62} /></Sprite>
      <Sprite start={137} end={142}><div style={{ position: 'absolute', inset: 0 }}><FS id="sui-save2" /><ChanceTag start={137.5} end={142} text="SOMMER SAVES THE MOUNTAIN" sub="THE FOX CIRCLES" accent={DZ_GREEN} /></div></Sprite>
      <Sprite start={142} end={152.21}><BeatCard clipId="texture-switzerland" start={142} end={152.21} text={<>THE MOUNTAIN<br />IS SHAKEN</>} accent="#fff" big={66} /></Sprite>
      <Sprite start={152.21} end={159}><div style={{ position: 'absolute', inset: 0 }}><FS id="mahrez-curl" /><GoalFlash at={153.6} /><ChanceTag start={154} end={159} text="MAHREZ — ALGERIA LEAD!" sub="THE FOX DRAWS BLOOD" accent={DZ_GREEN} /></div></Sprite>
      <Sprite start={159} end={172}><BeatCard clipId="bg-rise" start={159} end={172} text={<>BUT MOUNTAINS DO NOT MOVE</>} sub="SWITZERLAND STEADY · THE WEIGHT TELLS" accent={ACC} big={56} /></Sprite>
      <Sprite start={172} end={178.46}><BeatCard clipId="bg-believe" start={172} end={178.46} text={<>THE PRESS<br />TIGHTENS</>} accent="#fff" big={68} /></Sprite>
      <Sprite start={178.46} end={185}><div style={{ position: 'absolute', inset: 0 }}><FS id="sui-goal-1" /><GoalFlash at={179.9} /><ChanceTag start={180.3} end={185} text="EMBOLO — ONE-ONE!" sub="THE MOUNTAIN RUMBLES" accent={CH_RED} /></div></Sprite>
      <Sprite start={185} end={189.58}><BeatCard clipId="bg-roar" start={185} end={189.58} text={<>AND THEN —<br />THE THUNDER</>} accent={ACC} big={68} /></Sprite>
      <Sprite start={189.58} end={203.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="sui-goal-2" /><GoalFlash at={191.0} /><ChanceTag start={191.4} end={196} text="XHAKA — 2-1 SWITZERLAND!" sub="THE MOUNTAIN STANDS" accent={ACC} /><ScoreBug start={196.3} ch={2} dz={1} note="XHAKA 88'" badge="OUR PREDICTION" /><PredictionCard start={196.9} /></div></Sprite>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={203.32} end={208.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="vd-handshake" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.25) 0%, transparent 45%, rgba(2,3,8,0.55) 100%)' }} /><div style={{ position: 'absolute', left: 0, right: 0, top: 120, textAlign: 'center', zIndex: 24 }}><Kicker size={28} color={ACC}>The Mountain Stands</Kicker></div></div></Sprite>
      <Sprite start={208.32} end={213.32}><BeatCard clipId="bg-verdict" start={208.32} end={213.32} text={<>ALGERIA SCALED THE IMPOSSIBLE —<br />THE MOUNTAIN HELD</>} accent="#fff" big={52} /></Sprite>
      <Sprite start={213.32} end={218.32}><BeatCard clipId="bg-winner" start={213.32} end={218.32} text={<>XHAKA</>} sub="THE WINNER · LATE" accent={ACC} big={64} /></Sprite>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
            <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 900, backdropFilter: 'blur(6px)', opacity: panelP, transform: `translateY(${(1 - panelP) * 24}px)` }}>
              ...
              <div style={{ marginTop: 24 }}>
                <StatLine start={219.5} delay={0.0} label="SWITZERLAND" value="THE MOUNTAIN" accent={CH_RED} />
                <StatLine start={219.5} delay={0.25} label="ALGERIA" value="LES FENNECS · THE FOX" accent={DZ_GREEN} />
                <StatLine start={219.5} delay={0.5} label="OUR PREDICTION" value="SUI 2 — 1 ALG" accent="#fff" />
                <StatLine start={219.5} delay={0.75} label="88' XHAKA" value="THE LATE WINNER" accent={ACC} />
              </div>
            </div>
          </div>
        </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT SWITZERLAND', sub: 'THE MOUNTAIN', flag: <FlagCH w={76} />, accent: CH_RED },
    { label: 'COMMENT ALGERIA', sub: 'LES FENNECS', flag: <FlagDZ w={76} />, accent: DZ_GREEN },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 140, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: headP }}><Kicker size={30}>The Mountain, Or The Fox?</Kicker></div>
...
function MiniStrip() {
  const prev = ['legend-084-portrait', 'legend-085-portrait', 'legend-086-portrait', 'legend-087-portrait', 'legend-088-portrait'];
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
      ...
      <div style={{ width: 78, height: 104, borderRadius: 9, overflow: 'hidden', border: `2px solid ${ACC}`, boxShadow: `0 10px 28px rgba(0,0,0,0.6), 0 0 26px ${ACC}66` }}>
        <img data-seq alt="" src="assets/legend-089-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      </div>
    </div>
  );
}
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: 90, textAlign: 'center', opacity: teaseP, zIndex: 20 }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f3e0b8', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.7)' }}>THE GUARDIAN OF THE ALPS</div>
        </div>
      )}
      {lt > 1.6 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 22, opacity: clamp(cardP, 0, 1) }}>
          <div style={{ position: 'relative', transform: `perspective(1500px) rotateY(${tilt}deg) scale(${0.92 + 0.08 * Math.min(cardP, 1)})`, marginTop: -34 }}>
            <img data-seq src="assets/legend-089-portrait.png" alt="" style={{ height: 640, display: 'block', borderRadius: 16, boxShadow: '0 50px 130px rgba(0,0,0,0.85), 0 0 80px rgba(224,162,38,0.5)' }} />
            ...
            <div style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', background: 'rgba(224,162,38,0.95)', color: '#1a1306', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 17, letterSpacing: '0.14em', padding: '5px 16px', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.5)' }}>Nº 089 · ✦✦✦ ULTRA RARE</div>
          </div>
        </div>
      )}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, zIndex: 24 }}>
        <div style={{ textAlign: 'center', opacity: txtP, transform: `translateY(${(1 - txtP) * 16}px)` }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 44, color: ACC, letterSpacing: '0.03em', textShadow: '0 2px 24px rgba(224,162,38,0.5)' }}>THE MOUNTAIN · GUARDIAN OF THE ALPS</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 19, color: '#ecd9b0', letterSpacing: '0.18em', marginTop: 6 }}>SWITZERLAND · LEGEND 089</div>
        </div>
...
function PhoneCollect({ start }) {
  ...
  const filled = ['legend-085-portrait', 'legend-086-portrait', 'legend-087-portrait', 'legend-088-portrait'];
        ...
        <div style={{ width: '100%', height: '100%', borderRadius: 30, background: 'radial-gradient(ellipse at 50% 20%, #1a2030 0%, #06080c 70%)', overflow: 'hidden', position: 'relative', padding: '34px 18px 18px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            ...
            <div style={{ height: 104, borderRadius: 6, overflow: 'hidden', border: snapped ? `2px solid ${ACC}` : '1px dashed rgba(224,162,38,0.6)', position: 'relative', boxShadow: flash > 0 ? `0 0 ${20 * flash}px ${ACC}` : 'none' }}>
              {snapped && <img data-seq alt="" src="assets/legend-089-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 92, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: p }}>
        <Kicker size={30} color={ACC}>Claim the Mountain</Kicker>
