...
"$FF" -y -i music/cue-tense.mp3 -i sfx/braam.mp3 -i sfx/mystic.mp3 -i sfx/whoosh.mp3 \
  ...
  -map "[a]" -c:a aac -b:a 192k -t 15 intro_audio.m4a >/tmp/i89a.log 2>&1
echo "== intro.mp4 =="
"$FF" -y -framerate 30 -start_number 0 -i frames_intro/f_%05d.jpg -i intro_audio.m4a \
  ...
  -map "[v]" -map 1:a -c:v libx264 -pix_fmt yuv420p -crf 25 -preset medium -r 30 -c:a aac -b:a 160k -t 15 -movflags +faststart intro.mp4 >/tmp/i89e.log 2>&1
echo "== concat =="
printf "file 'intro.mp4'\nfile 'body_s.mp4'\n" > concat_list.txt
OUT="WorldCup26_Ep89_Switzerland_Algeria.mp4"
"$FF" -y -f concat -safe 0 -i concat_list.txt -c copy -movflags +faststart "$OUT" >/tmp/c89.log 2>&1 || \
  "$FF" -y -f concat -safe 0 -i concat_list.txt -c:v libx264 -pix_fmt yuv420p -crf 25 -preset medium -c:a aac -b:a 160k -movflags +faststart "$OUT" >/tmp/c89.log 2>&1
