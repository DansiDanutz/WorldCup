// match-scenes.jsx — Ep67 Egypt vs Iran — ANIMATION-FIRST + NO-REPEAT (318s).
// "The Desert Derby" · Group G. OUR PREDICTION EGY 1–0 IRN (Salah cuts inside,
// curls his left foot inside the far post on 71'; Iran press and Taremi goes
// close). Rule #11 no-repeat · #15 no banned wording · #16 story-woven mystery
// (Ney teased "WHAT IS THE BREATH OF AN ANCIENT LAND?" → paid off "the breath of
// the Nile"). Rule #17 — animated HOLO COLLECTIBLE reveal (shine-sweep + 3D tilt
// + UNLOCK beat + growing-collection strip + "COLLECT THEM ALL"). Rule #18 —
// PREMIUM prediction card (branded layered frame, sheen, crest badges, gold seal).

const EGY = '#e3162b', EGY_DK = '#8a0a17', IRN = '#1c9d4e', IRN_RED = '#da0000';
...
function FlagEGY({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <div style={{ position: 'absolute', top: '66.66%', left: 0, right: 0, height: '33.34%', background: '#000' }} />
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', color: '#c8a04a', fontSize: h * 0.30, lineHeight: 1, textShadow: '0 0 2px #8a6a1e' }}>🦅</div>
    </div>
  );
}
function FlagIRN({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '33.33%', background: '#239f40' }} />
      <div style={{ position: 'absolute', top: '33.33%', left: 0, right: 0, height: '33.33%', background: '#fff' }} />
      <div style={{ position: 'absolute', top: '66.66%', left: 0, right: 0, height: '33.34%', background: '#da0000' }} />
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', color: '#da0000', fontSize: h * 0.22, fontWeight: 900, lineHeight: 1 }}>۩</div>
    </div>
  );
}

function ScoreBug({ start, egy = 0, irn = 0, minute, badge = "OUR PREDICTION", note }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ ...cell, background: EGY }}>EGY</div>
        <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{egy} — {irn}</div>
        <div style={{ ...cell, background: IRN }}>IRN</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        {note && <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 16, color: '#fff', letterSpacing: '0.18em', background: EGY, border: `1px solid ${EGY}`, borderRadius: 999, padding: '4px 16px' }}>{note}</div>}
      </div>
    </div>
  );
}
...
function Backdrop() { return <div style={{ position: 'absolute', inset: 0, zIndex: 0, background: 'radial-gradient(ellipse at 50% 45%, #14100a 0%, #05060c 100%)' }} />; }
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <FS id="cg-egyfan" /><FS id="cg-irnfan" /><FS id="cg-tease" /><FS id="cg-stadaerial" />
      ...
      {teaseLabelP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseLabelP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 28, color: '#e8c97a', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>WHAT IS THE BREATH OF AN ANCIENT LAND?</div>
        </div>
      )}
      {lt > 18.0 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 30, opacity: titleP, zIndex: 22 }}>
          <Kicker size={34}>Two Ancient Kings</Kicker>
          <TitleReveal text="THE DESERT DERBY" start={19.8} size={104} color={MV.gold} />
        </div>
      )}
      <Letterbox />
    </div>
...
function SceneTitle() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, #14100a 0%, #1c1408 55%, #14100a 100%)' }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 30%, rgba(227,22,43,0.14) 0%, transparent 55%)' }} />
      <AmbientParticles start={23.48} dur={9.5} count={30} color="200,160,74" />
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 40 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 67</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 64, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagEGY w={220} /></Waving><BigTitle size={84} glow={EGY}>EGYPT</BigTitle></div>
          <BigTitle size={116} color={MV.gold}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagIRN w={220} /></Waving><BigTitle size={88} glow={IRN}>IRAN</BigTitle></div>
        </div>
        <div style={{ opacity: p3, fontFamily: '"Inter",sans-serif', fontWeight: 600, fontSize: 32, color: MV.muted, letterSpacing: '0.08em' }}>GROUP G · THE DESERT DERBY</div>
      </div>
      <Letterbox />
    </div>
  );
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['GROUP G', 'HOUSTON'], ['PHARAOHS', 'vs TEAM MELLI'], ['SALAH', 'vs TAREMI'], ['THE NILE', 'vs PERSIA']].map(([v, l], i) => (
...
function SceneEgypt() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={44} end={55.6}><div style={{ position: 'absolute', inset: 0 }}><FS id="egy-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.32) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagEGY w={66} />} label="THE PHARAOHS · EGYPT" accent={EGY} /></div></Sprite>
      <PlayerShowcase clipId="s-salah" name="MOHAMED SALAH" role="THE KING OF NAGRIG" accent={EGY} start={55.5} end={61.1} />
      <PlayerShowcase clipId="s-marmoush" name="OMAR MARMOUSH" role="THE SPEED" accent={EGY_DK} start={61.1} end={65.1} />
      <PlayerShowcase clipId="s-trezeguet" name="TREZEGUET" role="THE WINGER" accent={EGY} start={65.1} end={68.1} />
      <PlayerShowcase clipId="s-zizo" name="ZIZO" role="THE PLAYMAKER" accent={EGY_DK} start={68.1} end={73.6} />
      <PlayerShowcase clipId="s-elneny" name="MOHAMED ELNENY" role="THE METRONOME" accent={EGY} start={73.6} end={79.5} />
...
function SceneIran() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={79.5} end={90.7}><div style={{ position: 'absolute', inset: 0 }}><FS id="irn-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.32) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagIRN w={66} />} label="TEAM MELLI · IRAN" accent={IRN} /></div></Sprite>
      <PlayerShowcase clipId="s-taremi" name="MEHDI TAREMI" role="THE GHOST" accent={IRN_RED} start={90.6} end={95.2} />
      <PlayerShowcase clipId="s-azmoun" name="SARDAR AZMOUN" role="THE HUNTER" accent={IRN} start={95.2} end={98.2} />
      <PlayerShowcase clipId="s-jahanbakhsh" name="JAHANBAKHSH" role="THE WINGER" accent={IRN_RED} start={98.2} end={100.8} />
      <PlayerShowcase clipId="s-gholizadeh" name="GHOLIZADEH" role="THE SPARK" accent={IRN} start={100.8} end={103} />
      <Vignette strength={0.35} />
      <Letterbox />
    </div>
  );
...
function SceneDuel() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={103} end={113}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-egy" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(138,10,23,0.46), transparent 55%)' }} /><div style={{ position: 'absolute', left: 80, bottom: 130, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE PHARAOH<div style={{ fontSize: 27, fontWeight: 700, color: MV.gold, letterSpacing: '0.2em', marginTop: 8 }}>SALAH'S LEFT FOOT</div></div></div></Sprite>
      <Sprite start={113} end={122.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.25)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 58, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>TWO ANCIENT KINGS, ONE DESERT</div></div></Sprite>
      <Sprite start={122.5} end={132}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-irn" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(270deg, rgba(28,157,78,0.44), transparent 55%)' }} /><div style={{ position: 'absolute', right: 80, bottom: 130, textAlign: 'right', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE GHOST<div style={{ fontSize: 27, fontWeight: 700, color: MV.gold, letterSpacing: '0.2em', marginTop: 8 }}>TAREMI'S RISE</div></div></div></Sprite>
      <Letterbox />
    </div>
  );
}
...
function SceneDrama() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <FS id="dr-irn-attack" /><FS id="dr-irn-chance" /><FS id="dr-egy-buildup" /><FS id="dr-salah-cutin" /><FS id="dr-salah-goal" /><FS id="dr-egy-celeb" /><FS id="dr-taremi-stand" />
      ...
      {/* Iran press & Taremi goes close (denied), then Salah cuts inside and curls the 71' winner */}
      <Sprite start={142.0} end={152.0}><div style={{ position: 'absolute', left: 0, right: 0, bottom: 90, textAlign: 'center', zIndex: 24, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 50, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>TAREMI — INCHES WIDE</div></Sprite>
      <GoalFlash at={S + 46.0} />
      <Sprite start={179.0} end={203.32}><ScoreBug start={S + 47.0} egy={1} irn={0} minute="71'" badge="OUR PREDICTION" note="SALAH" /></Sprite>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.6)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(255,210,74,0.55)' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${EGY}28 0%, #070b14 38%, #070b14 62%, ${IRN}28 100%)` }} />
        ...
        {/* brand ribbon */}
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${EGY} 0%, #11151f 50%, ${IRN} 100%)`, padding: '13px 0', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '40px 70px 30px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 50 }}>
            <Badge flag={<FlagEGY w={104} />} name="EGYPT" accent={EGY} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 132, color: '#fff', lineHeight: 1 }}>
                <span style={{ textShadow: `0 0 36px ${EGY}99` }}>1</span>
                <span style={{ color: MV.gold, fontSize: 64, transform: 'translateY(-6px)' }}>—</span>
                <span style={{ textShadow: `0 0 36px ${IRN}99` }}>0</span>
              </div>
            ...
            <Badge flag={<FlagIRN w={104} />} name="IRAN" accent={IRN} />
          </div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 26, paddingTop: 22, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#ffd4d8', background: 'rgba(227,22,43,0.16)', border: '1px solid rgba(227,22,43,0.45)', borderRadius: 999, padding: '7px 18px' }}>⚽ 71' SALAH</span>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#bff0cf', background: 'rgba(28,157,78,0.16)', border: '1px solid rgba(28,157,78,0.45)', borderRadius: 999, padding: '7px 18px' }}>IRAN PUSHED — DENIED BY AN INCH</span>
          </div>
          <div style={{ textAlign: 'center', marginTop: 22, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 23, color: MV.gold, letterSpacing: '0.16em' }}>★ THE PHARAOH'S SHADOW STRETCHED LONGER ★</div>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, zIndex: 24, background: `linear-gradient(160deg, #14100a 0%, #05060c 100%)`, opacity: clamp((lt - 10.5) / 1.0, 0, 1) }} />
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
        <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 880, backdropFilter: 'blur(6px)', opacity: clamp((lt - 11) / 1.0, 0, 1) }}>
          ...
          <div style={{ marginTop: 24 }}>
            <StatLine start={S + 11.5} delay={0.0} label="EGYPT" value="THE PHARAOHS" accent={EGY} />
            <StatLine start={S + 11.5} delay={0.25} label="IRAN" value="TEAM MELLI" accent={IRN} />
            <StatLine start={S + 11.5} delay={0.5} label="OUR PREDICTION" value="EGY 1 — 0 IRN" accent="#fff" />
            <StatLine start={S + 11.5} delay={0.75} label="71' SALAH · INSIDE THE POST" value="THE PHARAOH'S SHADOW STRETCHED LONGER" accent={MV.gold} />
          </div>
        </div>
      </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT MASR', sub: "THE PHARAOHS", flag: <FlagEGY w={78} />, accent: EGY },
    { label: 'COMMENT IRAN', sub: "TEAM MELLI", flag: <FlagIRN w={78} />, accent: IRN },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <FS id="en-crowd" />
...
function SceneMystery() {
  ...
  const recent = ['legend-062', 'legend-063', 'legend-064', 'legend-065', 'legend-066'];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <ClipSprite id="my-ney" fit="cover" style={{ filter: 'brightness(0.62) saturate(1.12)' }} />
      ...
      <div style={{ position: 'absolute', top: 56, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: inP }}><Kicker size={26} color="#e8c97a">The Breath of the Nile · Legend No. 067</Kicker></div>
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: '50%', top: '46%', zIndex: 24, pointerEvents: 'none', transform: `translate(-50%,-50%) scale(${0.8 + teasePulse * 0.12})`, opacity: teaseP, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
          ...
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 30, color: '#ffe9a0', letterSpacing: '0.2em', textShadow: '0 2px 18px rgba(0,0,0,0.8)' }}>THE BREATH OF THE NILE</div>
        </div>
      )}
        ...
        <div style={{ position: 'absolute', left: '50%', top: '44%', zIndex: 25, opacity: clamp(cardP, 0, 1), perspective: 1700, transform: `translate(-50%,-50%) translateY(${(1 - cardP) * 40 + floatY}px) scale(${0.86 + 0.1 * cardP})` }}>
          ...
          {unlockP > 0 && (
            <div style={{ position: 'absolute', top: -64, left: '50%', transform: `translateX(-50%) scale(${unlockP})`, zIndex: 28, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, whiteSpace: 'nowrap' }}>
              ...
              <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 17, letterSpacing: '0.3em', color: '#f5d016' }}>Nº 067 · ✦✦✦ ULTRA RARE</div>
            </div>
          )}
          <div style={{ position: 'relative', transformStyle: 'preserve-3d', transform: `rotateY(${flipDeg + tiltX}deg)`, borderRadius: 20, overflow: 'hidden', boxShadow: `0 30px 120px rgba(0,0,0,0.8), 0 0 ${50 + settle * 50}px rgba(245,208,22,${0.22 + settle * 0.33})` }}>
            <img data-seq="" src="assets/legend-067-portrait.png" alt="" style={{ height: 760, width: 'auto', display: 'block' }} />
        ...
        <div style={{ position: 'absolute', left: '50%', bottom: 120, zIndex: 26, transform: `translateX(-50%) translateY(${(1 - stripP) * 30}px)`, opacity: stripP, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
            ...
            <div style={{ width: 96, height: 134, borderRadius: 9, overflow: 'hidden', border: `2px solid ${MV.gold}`, boxShadow: `0 8px 26px rgba(0,0,0,0.6), 0 0 ${16 + pulse * 14}px rgba(245,208,22,0.7)`, transform: `scale(${1 + pulse * 0.03})` }}>
              <img data-seq="" src="assets/legend-067-portrait.png" alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </div>
          </div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 22, color: '#fff', letterSpacing: '0.14em' }}>LEGEND <span style={{ color: MV.gold }}>067</span> OF 66 · <span style={{ color: MV.gold }}>COLLECT THEM ALL</span></div>
        </div>
      )}
      {/* sign-up CTA */}
      {settle > 0.2 && (
        <div style={{ position: 'absolute', left: '50%', bottom: 46, zIndex: 26, transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', gap: 16, opacity: settle }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 12, background: 'linear-gradient(100deg, #16a34a, #0e8a3c)', borderRadius: 999, padding: '13px 30px', transform: `scale(${1 + pulse * 0.04})`, boxShadow: `0 8px 30px rgba(22,163,74,${0.35 + pulse * 0.4})` }}><span style={{ fontSize: 20 }}>⚡</span><span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 22, color: '#fff', letterSpacing: '0.03em' }}>SIGN UP FREE — CLAIM LEGEND 067</span></div>
        </div>
      )}
...
function SceneApp() {
  ...
  const pulse = 0.5 + 0.5 * Math.sin(lt * 3.4);
  const cards = [
    { name: 'EGYPT', flag: <FlagEGY w={84} /> },
    { name: 'IRAN', flag: <FlagIRN w={84} /> },
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(160deg, #1c1408 0%, #07090f 70%)` }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 42, opacity: inP }}>
        <Kicker color="#f0c79e" size={30}>The Prediction Game</Kicker>
        <BigTitle size={100} color="#fff" glow={EGY}>worldcup26.world</BigTitle>
        <Kicker color="#f6dccf" size={30}>Sign Up · Pick 3 Of 48</Kicker>
        ...
        <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 27, color: MV.gold, opacity: clamp(btnP, 0, 1) }}>Unlock Legend 067 the moment you sign up · free · no prizes</div>
      </div>
      <Letterbox />
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', left: 0, right: 0, bottom: 140, display: 'flex', justifyContent: 'center', zIndex: 26, opacity: p, transform: `translateY(${(1 - p) * 24}px)` }}>
      <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 999, padding: '18px 50px', display: 'flex', alignItems: 'center', gap: 20 }}>
        ...
        <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 29, color: MV.gold }}>⚡ EP68 · NEW LEGEND EVERY MATCH · worldcup26.world</span>
      </div>
    </div>
  );
}
