// match-scenes.jsx — Ep93 Japan vs South Africa (R16) — PHOTOREAL + NO-REPEAT/NO-LOOP.
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <FS id="jpn-tengu" /><FS id="rsa-impundulu" /><FS id="texture-fuji" />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f0dca6', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE STORM GOD MEETS THE LIGHTNING BIRD.</div>
        </div>
      )}
      {lt > 19.2 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, opacity: titleP, zIndex: 22 }}>
          <Kicker size={32} color={ACC}>The Storm God and The Lightning Bird</Kicker>
          <TitleReveal text="SUSANOO" start={20.0} size={84} color={ACC} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 38 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 93 · Last 16</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 52, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagJPN w={210} /></Waving><BigTitle size={64} glow={JPN_RED}>JAPAN</BigTitle></div>
          <BigTitle size={104} color={ACC}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagRSA w={210} /></Waving><BigTitle size={46} glow={RSA_GOLD}>SOUTH AFRICA</BigTitle></div>
        </div>
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['THE SAMURAI BLUE', 'JAPAN'], ['BAFANA BAFANA', 'SOUTH AFRICA'], ['THE STORM GOD', 'vs THE BIRD'], ['LAST 16', 'WIN OR GO HOME']].map(([v, l], i) => (
...
function SceneJapan() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={44} end={49}><div style={{ position: 'absolute', inset: 0 }}><FS id="jpn-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagJPN w={64} />} label="THE SAMURAI BLUE · JAPAN" accent={JPN_RED} /></div></Sprite>
      <Sprite start={49} end={54}><div style={{ position: 'absolute', inset: 0 }}><FS id="jpn-tifo" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <Sprite start={54} end={55.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="jpn-surge" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(188,0,45,0.30), transparent 55%)' }} /></div></Sprite>
      <PlayerShowcase clipId="jpn-mitoma" name="KAORU MITOMA" role="THE DANCER · 14" accent={JPN_RED} start={55.5} end={60.5} />
      <PlayerShowcase clipId="jpn-kubo" name="TAKEFUSA KUBO" role="THE JEWEL · 11" accent={JPN_LIGHT} start={60.5} end={63.74} />
      <PlayerShowcase clipId="jpn-doan" name="RITSU DŌAN" role="THE STRIKER · 8" accent={JPN_RED} start={63.74} end={68.0} />
      <PlayerShowcase clipId="jpn-endo" name="WATARU ENDŌ" role="CAPTAIN · THE IRON · 6" accent={JPN_LIGHT} start={68.0} end={70.99} />
      <PlayerShowcase clipId="jpn-kamada" name="DAICHI KAMADA" role="THE KEY · 15" accent={JPN_RED} start={70.99} end={79.5} />
    </div>
  );
}
...
function SceneSouthAfrica() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <PlayerShowcase clipId="rsa-williams" name="RONWEN WILLIAMS" role="CAPTAIN · THE WALL · 1" accent={RSA_GOLD} start={90.6} end={96.8} />
      <PlayerShowcase clipId="rsa-foster" name="LYLE FOSTER" role="THE SPEARHEAD · 9" accent={RSA_RED} start={96.8} end={99.95} />
      <PlayerShowcase clipId="rsa-mokoena" name="TEBOHO MOKOENA" role="THE THUNDERCLAP · 8" accent={RSA_GOLD} start={99.95} end={103.0} />
      <PlayerShowcase clipId="rsa-zwane" name="THEMBA ZWANE" role="THE MAESTRO · 10" accent={RSA_RED} start={103.0} end={105.71} />
      <PlayerShowcase clipId="rsa-mofokeng" name="RELEBOHILE MOFOKENG" role="THE FEARLESS · 11" accent={RSA_GOLD} start={105.71} end={113.0} />
    </div>
  );
}
...
function SceneRiddle() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={113} end={118}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.24)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE STORM GOD vs THE LIGHTNING BIRD</div></div></Sprite>
      ...
      <Sprite start={123} end={132}><BeatCard clipId="bg-riddle" start={123} end={132} text={<>DOES THE STORM BREAK THE BIRD,<br />OR DOES THE BIRD RIDE THE STORM?</>} accent={ACC} big={50} /></Sprite>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(244,196,48,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${JPN_RED}33 0%, #070b14 38%, #070b14 62%, ${RSA_GOLD}55 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${JPN_RED} 0%, ${ACC} 50%, ${RSA_GOLD} 100%)`, padding: '13px 0', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '36px 70px 28px' }}>
          ...
          <div style={{ textAlign: 'center', marginTop: 16, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 32, color: ACC, letterSpacing: '0.05em', textShadow: `0 0 26px ${ACC}66` }}>THE STORM GOD RISES · 2–1</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 16, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#f3e6c4', background: 'rgba(244,196,48,0.16)', border: '1px solid rgba(244,196,48,0.45)', borderRadius: 999, padding: '7px 18px' }}>⚡ MITOMA — THE LATE WINNER</span>
          </div>
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <Sprite start={132} end={137}><BeatCard clipId="bg-firing" start={132} end={137} text={<>SOUTH AFRICA<br />COME OUT ROARING</>} accent={RSA_GOLD} big={58} /></Sprite>
      <Sprite start={137} end={142}><div style={{ position: 'absolute', inset: 0 }}><FS id="rsa-surge" /><ChanceTag start={137.5} end={142} text="THE LIGHTNING BIRD STRIKES" sub="GOLD AND FEARLESS" accent={RSA_GOLD} /></div></Sprite>
      <Sprite start={142} end={152.21}><BeatCard clipId="texture-storm" start={142} end={152.21} text={<>JAPAN<br />ARE ROCKED</>} accent="#fff" big={62} /></Sprite>
      <Sprite start={152.21} end={159}><div style={{ position: 'absolute', inset: 0 }}><FS id="rsa-goal-1" /><GoalFlash at={153.6} /><ChanceTag start={154} end={159} text="SOUTH AFRICA LEAD!" sub="THE BIRD IS FLYING" accent={RSA_GOLD} /></div></Sprite>
      <Sprite start={159} end={172}><BeatCard clipId="bg-rise" start={159} end={172} text={<>BUT THE STORM GATHERS</>} sub="THE BALL FINDS KUBO" accent={ACC} big={54} /></Sprite>
      <Sprite start={172} end={178.46}><BeatCard clipId="bg-believe" start={172} end={178.46} text={<>THE JEWEL<br />GOES TO WORK</>} accent="#fff" big={68} /></Sprite>
      <Sprite start={178.46} end={185}><div style={{ position: 'absolute', inset: 0 }}><FS id="kubo-goal" /><GoalFlash at={179.9} /><ChanceTag start={180.3} end={185} text="KUBO — ONE-ONE!" sub="A BOLT FROM THE BLUE" accent={JPN_LIGHT} /></div></Sprite>
      <Sprite start={185} end={189.58}><BeatCard clipId="bg-roar" start={185} end={189.58} text={<>AND THEN —<br />THE TEMPEST</>} accent={ACC} big={68} /></Sprite>
      <Sprite start={189.58} end={203.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="mitoma-goal" /><GoalFlash at={191.0} /><ChanceTag start={191.4} end={196} text="MITOMA — 2-1 JAPAN!" sub="THE STORM GOD RISES" accent={ACC} /><ScoreBug start={196.3} co={2} gh={1} note="88'" badge="OUR PREDICTION" /><PredictionCard start={196.9} /></div></Sprite>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={203.32} end={208.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="vd-handshake" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.25) 0%, transparent 45%, rgba(2,3,8,0.55) 100%)' }} /><div style={{ position: 'absolute', left: 0, right: 0, top: 120, textAlign: 'center', zIndex: 24 }}><Kicker size={28} color={ACC}>The Storm God Rises</Kicker></div></div></Sprite>
      <Sprite start={208.32} end={213.32}><BeatCard clipId="bg-verdict" start={208.32} end={213.32} text={<>SOUTH AFRICA MADE JAPAN SWEAT —<br />BUT THE STORM BROKE THROUGH</>} accent="#fff" big={46} /></Sprite>
      <Sprite start={213.32} end={218.32}><BeatCard clipId="bg-winner" start={213.32} end={218.32} text={<>MITOMA</>} sub="THE LATE WINNER" accent={ACC} big={62} /></Sprite>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
            <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 900, backdropFilter: 'blur(6px)', opacity: panelP, transform: `translateY(${(1 - panelP) * 24}px)` }}>
              ...
              <div style={{ marginTop: 24 }}>
                <StatLine start={219.5} delay={0.0} label="JAPAN" value="THE SAMURAI BLUE" accent={JPN_RED} />
                <StatLine start={219.5} delay={0.25} label="SOUTH AFRICA" value="BAFANA BAFANA" accent={RSA_GOLD} />
                <StatLine start={219.5} delay={0.5} label="OUR PREDICTION" value="JPN 2 — 1 RSA" accent="#fff" />
                <StatLine start={219.5} delay={0.75} label="88' MITOMA" value="THE LATE WINNER" accent={ACC} />
              </div>
            </div>
          </div>
        </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT JAPAN', sub: 'THE SAMURAI BLUE', flag: <FlagJPN w={76} />, accent: JPN_RED },
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 140, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: headP }}><Kicker size={30}>The Storm God, Or The Lightning Bird?</Kicker></div>
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: 90, textAlign: 'center', opacity: teaseP, zIndex: 20 }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f3e0b8', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.7)' }}>THE STORM GOD</div>
        </div>
      )}
      {lt > 1.6 && (
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, zIndex: 24 }}>
        <div style={{ textAlign: 'center', opacity: txtP, transform: `translateY(${(1 - txtP) * 16}px)` }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 44, color: ACC, letterSpacing: '0.03em', textShadow: '0 2px 24px rgba(244,196,48,0.5)' }}>SUSANOO · THE STORM GOD</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 19, color: '#ecd9b0', letterSpacing: '0.18em', marginTop: 6 }}>JAPAN · LEGEND 093</div>
        </div>
