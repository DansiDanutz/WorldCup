...
loudnorm=I=-14:TP=-1.2:LRA=11,alimiter=limit=0.97,apad,atrim=0:15,aformat=channel_layouts=stereo:sample_rates=44100[a]" \
  -map "[a]" -c:a aac -b:a 192k -t 15 intro_audio.m4a >/tmp/i78a.log 2>&1
echo "  ok"
echo "== [4/5] encode intro.mp4 =="
"$FF" -y -framerate 30 -start_number 0 -i frames_intro/f_%05d.jpg -i intro_audio.m4a \
  ...
  -map "[v]" -map 1:a -c:v libx264 -pix_fmt yuv420p -crf 25 -preset medium -r 30 -c:a aac -b:a 160k -t 15 -movflags +faststart intro.mp4 >/tmp/i78e.log 2>&1
echo "  intro.mp4 $(ls -la intro.mp4|awk '{print $5}')"
echo "== [5/5] concat =="
printf "file 'intro.mp4'\nfile 'body_s.mp4'\n" > concat_list.txt
OUT="WorldCup26_Ep78_Brazil_Japan.mp4"
"$FF" -y -f concat -safe 0 -i concat_list.txt -c copy -movflags +faststart "$OUT" >/tmp/c78.log 2>&1 || {
  "$FF" -y -f concat -safe 0 -i concat_list.txt -c:v libx264 -pix_fmt yuv420p -crf 25 -preset medium -c:a aac -b:a 160k -movflags +faststart "$OUT" >/tmp/c78.log 2>&1; }
