// match-scenes.jsx — Ep78 Brazil vs Japan (R32) — PHOTOREAL + NO-REPEAT/NO-LOOP.
// "THE RISING SUN". OUR PREDICTION Japan 2-1 Brazil (Vinicius lead; Mitoma equalise; Kubo late
// winner). Mystic (#21): BRAZIL = the golden Amazon spirit / Christ the Redeemer over Rio;
// JAPAN = the TENGU, winged mountain guardian and master of the sword -> Legend 078 = The Tengu.
// #22 photoreal · #23 name<->image SYNC · #24 footer real mini-cards 073-077 + phone collect
// · #25 NO blank frames.

const BRA_YELLOW = '#f7d417', BRA_GREEN = '#009b3a', BRA_BLUE = '#002776';
const JPN_BLUE = '#13235e', JPN_LIGHT = '#6f8fe0', JPN_RED = '#bc002d', JPN_WHITE = '#f3f4f7';
const ACC = '#e6a017';           // golden rising-sun accent
...
function FlagBRA({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: BRA_GREEN, boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%) rotate(45deg)', width: h * 0.6, height: h * 0.6, background: BRA_YELLOW }} />
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: h * 0.32, height: h * 0.32, borderRadius: '50%', background: BRA_BLUE }} />
    </div>
  );
}
function FlagJPN({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#fff', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: h * 0.56, height: h * 0.56, borderRadius: '50%', background: JPN_RED }} />
    </div>
  );
}
...
function NightField({ o = 0.6 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 44%, #1a140c 0%, #120d08 60%, #05060c 100%)' }} />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 50% 42%, rgba(230,160,23,${(0.13 * pulse * o).toFixed(3)}) 0%, transparent 55%)` }} />
    </div>
  );
}
function SunBeat({ start, dur, count = 28 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 48%, #1d1510 0%, #150f0a 55%, #06080c 100%)' }} />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at ${50 + drift}% 40%, rgba(230,160,23,0.20) 0%, transparent 52%)` }} />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at ${50 - drift}% 64%, rgba(188,0,45,0.18) 0%, transparent 55%)` }} />
      <AmbientParticles start={start} dur={dur} count={count} color="240,200,140" />
    </div>
  );
}

function ScoreBug({ start, bra = 1, jpn = 2, note = "KUBO 87'", badge = 'OUR PREDICTION' }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ ...cell, background: BRA_GREEN }}>BRA</div>
        <div style={{ ...cell, fontSize: 38, color: ACC }}>{bra} — {jpn}</div>
        <div style={{ ...cell, background: JPN_RED }}>JPN</div>
        {note && <div style={{ ...cell, fontSize: 24, color: ACC, borderLeft: `1px solid ${MV.line}` }}>{note}</div>}
      </div>
      <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 16, color: ACC, letterSpacing: '0.22em', background: 'rgba(230,160,23,0.14)', border: '1px solid rgba(230,160,23,0.5)', borderRadius: 999, padding: '4px 16px' }}>{badge}</div>
    </div>
  );
}
...
function Backdrop() { return <div style={{ position: 'absolute', inset: 0, zIndex: 0, background: 'radial-gradient(ellipse at 50% 45%, #151009 0%, #05060c 100%)' }} />; }
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <FS id="texture-redeemer" /><FS id="bra-spirit" /><FS id="texture-fuji" /><FS id="jpn-tengu" />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at center, rgba(230,160,23,${(0.20 * beat).toFixed(3)}) 0%, transparent 60%)` }} />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f0dca6', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE GOLDEN GIANT MEETS THE RISING SUN.</div>
        </div>
      )}
      {lt > 19.2 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, opacity: titleP, zIndex: 22 }}>
          <Kicker size={32} color={ACC}>The Golden Giant vs The Rising Sun</Kicker>
          <TitleReveal text="THE RISING SUN" start={20.0} size={94} color={ACC} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 38 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 78 · Last 32</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 56, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagBRA w={210} /></Waving><BigTitle size={70} glow={BRA_YELLOW}>BRAZIL</BigTitle></div>
          <BigTitle size={108} color={ACC}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagJPN w={210} /></Waving><BigTitle size={74} glow={JPN_RED}>JAPAN</BigTitle></div>
        </div>
        <div style={{ opacity: p3, fontFamily: '"Inter",sans-serif', fontWeight: 600, fontSize: 30, color: MV.muted, letterSpacing: '0.10em' }}>WIN, OR GO HOME</div>
      </div>
    </div>
  );
}
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['5 STARS', 'BRAZIL'], ['THEY BEAT', 'GERMANY & SPAIN'], ['NEVER', 'BEATEN BRAZIL'], ['LAST 32', 'WIN OR GO HOME']].map(([v, l], i) => (
...
function SceneBrazil() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <Sprite start={49} end={54}><div style={{ position: 'absolute', inset: 0 }}><FS id="bra-tifo" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <Sprite start={54} end={55.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="bra-attack" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(247,212,23,0.24), transparent 55%)' }} /></div></Sprite>
      <PlayerShowcase clipId="bra-vinicius" name="VINÍCIUS JR" role="THE LIGHTNING · 7" accent={BRA_YELLOW} start={55.5} end={59.66} />
      <PlayerShowcase clipId="bra-rodrygo" name="RODRYGO" role="THE GHOST · 10" accent={BRA_GREEN} start={59.66} end={62.84} />
      <PlayerShowcase clipId="bra-raphinha" name="RAPHINHA" role="THE MENACE · 11" accent={BRA_YELLOW} start={62.84} end={68.0} />
      <PlayerShowcase clipId="bra-bruno" name="BRUNO GUIMARÃES" role="THE METRONOME · 8" accent={BRA_GREEN} start={68.0} end={71.18} />
      <PlayerShowcase clipId="bra-marquinhos" name="MARQUINHOS" role="CAPTAIN · THE WALL · 4" accent={BRA_YELLOW} start={71.18} end={79.5} />
    </div>
  );
}
...
function SceneJapan() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={79.5} end={84.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="jpn-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagJPN w={64} />} label="SAMURAI BLUE · JAPAN" accent={JPN_RED} /></div></Sprite>
      ...
      <PlayerShowcase clipId="jpn-mitoma" name="KAORU MITOMA" role="THE DANCER · 14" accent={JPN_RED} start={90.6} end={94.61} />
      <PlayerShowcase clipId="jpn-kubo" name="TAKEFUSA KUBO" role="THE JEWEL · 11" accent={JPN_LIGHT} start={94.61} end={97.20} />
      <PlayerShowcase clipId="jpn-doan" name="RITSU DŌAN" role="THE STRIKER · 8" accent={JPN_RED} start={97.20} end={103.0} />
      <PlayerShowcase clipId="jpn-endo" name="WATARU ENDŌ" role="CAPTAIN · THE IRON · 6" accent={JPN_LIGHT} start={103.0} end={107.11} />
      <PlayerShowcase clipId="jpn-kamada" name="DAICHI KAMADA" role="THE KEY · 15" accent={JPN_RED} start={107.11} end={113.0} />
    </div>
  );
}
...
function SceneRiddle() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={113} end={118}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.24)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE BEAUTIFUL vs THE DISCIPLINED</div></div></Sprite>
      ...
      <Sprite start={123} end={132}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={123} dur={9} count={24} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 56, color: ACC, letterSpacing: '0.06em', textAlign: 'center', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>GENIUS —<br />OR THE WARRIORS WHO FEAR NO ONE?</div></div></div></Sprite>
    </div>
  );
}
...
function PredictionCard({ start }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(230,160,23,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${BRA_GREEN}33 0%, #070b14 38%, #070b14 62%, ${JPN_RED}33 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${BRA_GREEN} 0%, ${ACC} 50%, ${JPN_RED} 100%)`, padding: '13px 0', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '38px 70px 30px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 50 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 132, color: '#fff', lineHeight: 1 }}>
                <span style={{ textShadow: `0 0 36px ${BRA_YELLOW}cc` }}>1</span>
                <span style={{ color: ACC, fontSize: 64, transform: 'translateY(-6px)' }}>—</span>
                <span style={{ textShadow: `0 0 36px ${JPN_RED}cc` }}>2</span>
              </div>
            ...
            <Badge flag={<FlagJPN w={104} />} name="JAPAN" accent={JPN_RED} />
          </div>
          <div style={{ textAlign: 'center', marginTop: 16, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 30, color: ACC, letterSpacing: '0.05em', textShadow: `0 0 26px ${ACC}66` }}>JAPAN SLAY THE GIANT · 2–1</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 16, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#f3e6c4', background: 'rgba(230,160,23,0.16)', border: '1px solid rgba(230,160,23,0.45)', borderRadius: 999, padding: '7px 18px' }}>⚔️ KUBO — THE LATE WINNER</span>
          </div>
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <Sprite start={132} end={137}><div style={{ position: 'absolute', inset: 0 }}><FS id="bra-press" /><ChanceTag start={132.5} end={137} text="BRAZIL COME DANCING" sub="VINÍCIUS FLYING · THE YELLOW WAVE" accent={BRA_YELLOW} /></div></Sprite>
      <Sprite start={137} end={144}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={137} dur={7} count={22} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>THE SAMBA<br />SINGS FIRST</div></div></div></Sprite>
      <Sprite start={144} end={152.21}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={144} dur={8.2} count={26} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 74, color: '#fff', letterSpacing: '0.02em', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>THE GENIUS FINDS<br />THE GAP</div></div></div></Sprite>
      <Sprite start={152.21} end={159}><div style={{ position: 'absolute', inset: 0 }}><FS id="vini-goal" /><GoalFlash at={153.6} /><ChanceTag start={154} end={159} text="VINÍCIUS — BRAZIL LEAD!" sub="ELECTRIC · THE GIANT STIRS" accent={BRA_YELLOW} /></div></Sprite>
      <Sprite start={159} end={164.37}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={159} dur={5.4} count={20} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>BUT THE SAMURAI<br />DO NOT BOW</div></div></div></Sprite>
      <Sprite start={164.37} end={172}><div style={{ position: 'absolute', inset: 0 }}><FS id="jpn-surge" /><ChanceTag start={165} end={172} text="THEY ANSWER BACK" sub="SLAYERS OF GIANTS" accent={JPN_RED} /></div></Sprite>
      <Sprite start={172} end={178.46}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={172} dur={6.46} count={22} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 66, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>THE RISING SUN<div style={{ fontSize: 28, fontWeight: 800, color: ACC, letterSpacing: '0.16em', marginTop: 14 }}>BREAKS THROUGH THE CLOUDS</div></div></div></div></Sprite>
      <Sprite start={178.46} end={185}><div style={{ position: 'absolute', inset: 0 }}><FS id="mitoma-goal" /><GoalFlash at={179.9} /><ChanceTag start={180.3} end={185} text="MITOMA — ONE-ONE!" sub="HE SLOTS IT HOME" accent={JPN_RED} /></div></Sprite>
      <Sprite start={185} end={189.58}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={185} dur={4.58} count={18} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 70, color: ACC, textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>AND THEN —<br />THE SWORD FALLS</div></div></div></Sprite>
      <Sprite start={189.58} end={203.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="kubo-goal" /><GoalFlash at={191.0} /><ChanceTag start={191.4} end={196} text="KUBO — 2-1 JAPAN!" sub="THE GIANT IS SLAIN" accent={ACC} /><ScoreBug start={196.3} bra={1} jpn={2} note="KUBO 87'" badge="OUR PREDICTION" /><PredictionCard start={196.9} /></div></Sprite>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={203.32} end={208.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="vd-handshake" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.25) 0%, transparent 45%, rgba(2,3,8,0.55) 100%)' }} /><div style={{ position: 'absolute', left: 0, right: 0, top: 120, textAlign: 'center', zIndex: 24 }}><Kicker size={28} color={ACC}>Genius Undone By Belief</Kicker></div></div></Sprite>
      <Sprite start={208.32} end={213.32}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={208.32} dur={5} count={20} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 60, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>A TEAM THAT REFUSES<br />TO LOSE</div></div></div></Sprite>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
            <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 900, backdropFilter: 'blur(6px)', opacity: panelP, transform: `translateY(${(1 - panelP) * 24}px)` }}>
              ...
              <div style={{ marginTop: 24 }}>
                ...
                <StatLine start={219.5} delay={0.25} label="JAPAN" value="SAMURAI BLUE" accent={JPN_RED} />
                <StatLine start={219.5} delay={0.5} label="OUR PREDICTION" value="BRA 1 — 2 JPN" accent="#fff" />
                <StatLine start={219.5} delay={0.75} label="87' KUBO" value="THE GIANT IS SLAIN" accent={ACC} />
              </div>
            </div>
          </div>
        </div>
...
function SceneEngage() {
  ...
  const cards = [
    ...
    { label: 'COMMENT JAPAN', sub: 'SAMURAI BLUE', flag: <FlagJPN w={76} />, accent: JPN_RED },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 140, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: headP }}><Kicker size={30}>The Golden Giant, Or The Rising Sun?</Kicker></div>
...
function MiniStrip() {
  const prev = ['legend-073-portrait', 'legend-074-portrait', 'legend-075-portrait', 'legend-076-portrait', 'legend-077-portrait'];
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
      ...
      <div style={{ width: 78, height: 104, borderRadius: 9, overflow: 'hidden', border: `2px solid ${ACC}`, boxShadow: `0 10px 28px rgba(0,0,0,0.6), 0 0 26px ${ACC}66` }}>
        <img data-seq alt="" src="assets/legend-078-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      </div>
    </div>
  );
}
...
function SceneMystery() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 46%, #241410 0%, #170d0a 50%, #0a0606 100%)' }} />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 50% 44%, rgba(230,160,23,${(0.32 * glow).toFixed(3)}) 0%, transparent 55%)` }} />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: 90, textAlign: 'center', opacity: teaseP, zIndex: 20 }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f3e0b8', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.7)' }}>THE GUARDIAN OF THE MOUNTAIN</div>
        </div>
      )}
      {lt > 1.6 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 22, opacity: clamp(cardP, 0, 1) }}>
          <div style={{ position: 'relative', transform: `perspective(1500px) rotateY(${tilt}deg) scale(${0.92 + 0.08 * Math.min(cardP, 1)})`, marginTop: -34 }}>
            <img data-seq src="assets/legend-078-portrait.png" alt="" style={{ height: 640, display: 'block', borderRadius: 16, boxShadow: '0 50px 130px rgba(0,0,0,0.85), 0 0 80px rgba(230,160,23,0.5)' }} />
            ...
            <div style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', background: 'rgba(230,160,23,0.95)', color: '#1a1306', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 17, letterSpacing: '0.14em', padding: '5px 16px', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.5)' }}>Nº 078 · ✦✦✦ ULTRA RARE</div>
          </div>
        </div>
      )}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, zIndex: 24 }}>
        <div style={{ textAlign: 'center', opacity: txtP, transform: `translateY(${(1 - txtP) * 16}px)` }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 46, color: ACC, letterSpacing: '0.04em', textShadow: '0 2px 24px rgba(230,160,23,0.5)' }}>THE TENGU · GUARDIAN OF THE MOUNTAIN</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 19, color: '#ecd9b0', letterSpacing: '0.18em', marginTop: 6 }}>JAPAN · LEGEND 078</div>
        </div>
...
function PhoneCollect({ start }) {
  ...
  const filled = ['legend-074-portrait', 'legend-075-portrait', 'legend-076-portrait', 'legend-077-portrait'];
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 26, opacity: inP }}>
      <div style={{ position: 'relative', width: 300, height: 600, borderRadius: 40, background: 'linear-gradient(160deg,#23262e,#0c0e13)', border: '3px solid #2b2f38', boxShadow: '0 40px 110px rgba(0,0,0,0.8), 0 0 50px rgba(230,160,23,0.25)', padding: 12 }}>
        ...
        <div style={{ width: '100%', height: '100%', borderRadius: 30, background: 'radial-gradient(ellipse at 50% 20%, #2a2010 0%, #0c0a06 70%)', overflow: 'hidden', position: 'relative', padding: '34px 18px 18px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            ...
            <div style={{ height: 104, borderRadius: 6, overflow: 'hidden', border: snapped ? `2px solid ${ACC}` : '1px dashed rgba(230,160,23,0.6)', position: 'relative', boxShadow: flash > 0 ? `0 0 ${20 * flash}px ${ACC}` : 'none' }}>
              {snapped && <img data-seq alt="" src="assets/legend-078-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 92, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: p }}>
        <Kicker size={30} color={ACC}>Claim The Tengu</Kicker>
