// match-scenes.jsx — Ep87 Spain vs Austria (R32) — PHOTOREAL + NO-REPEAT/NO-LOOP.
// "THE BULL AND THE EAGLE". OUR PREDICTION Spain 2-1 Austria (Sabitzer early; Yamal equalise;
// Morata late winner). Mystic (#21): SPAIN = El Toro, the Spanish bull; AUSTRIA = the Eagle
// (Austrian black eagle) -> Legend 087 = the Spanish Bull, Soul of La Roja.
// Spain generated fresh; Austria REUSED from Ep73 (Rule #26, same nation). Beat captions over
// ANONYMOUS atmosphere backdrops (#27). Nation-correct clips only (#28). #23 name<->image SYNC.

const ES_RED = '#c60b1e', ES_YELLOW = '#ffc400';
const AT_RED = '#c8102e', AT_WHITE = '#f3f4f7';
const ACC = '#e0a226';           // gold accent
const GRADE = { filter: 'saturate(1.06) contrast(1.04)' };

function FlagES({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <div style={{ flex: 2, background: '#ffc400', position: 'relative' }}>
        <div style={{ position: 'absolute', left: '22%', top: '50%', transform: 'translateY(-50%)', width: h * 0.22, height: h * 0.30, background: '#c60b1e', borderRadius: 3, opacity: 0.9 }} />
      </div>
      <div style={{ flex: 1, background: '#c60b1e' }} />
    </div>
  );
}
function FlagAT({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
...
// Bull motif from the Legend 087 card art (slow Ken-Burns) — Spain's cold-open symbol.
function BullStill({ start = 0, dur = 5 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', opacity: fade, zIndex: 1 }}>
      <img data-seq src="assets/legend-087-landscape.png" alt="" style={{ position: 'absolute', left: '50%', top: '50%', width: '108%', transform: `translate(-50%,-50%) scale(${scale})`, filter: 'saturate(1.05) contrast(1.03)' }} />
...
function ScoreBug({ start, es = 2, at = 1, note = "MORATA 88'", badge = 'OUR PREDICTION' }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ ...cell, background: ES_RED }}>ESP</div>
        <div style={{ ...cell, fontSize: 38, color: ACC }}>{es} — {at}</div>
        <div style={{ ...cell, background: AT_WHITE, color: '#06121a' }}>AUT</div>
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <BullStill start={0} dur={5} />
      <FS id="texture-spain" /><FS id="aut-eagle" /><FS id="texture-austria" />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f0dca6', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE BULL MEETS THE EAGLE.</div>
        </div>
      )}
      {lt > 19.2 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, opacity: titleP, zIndex: 22 }}>
          <Kicker size={32} color={ACC}>The Bull and The Eagle</Kicker>
          <TitleReveal text="THE SPANISH BULL" start={20.0} size={78} color={ACC} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 38 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 87 · Last 32</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 56, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagES w={210} /></Waving><BigTitle size={70} glow={ES_RED}>SPAIN</BigTitle></div>
          <BigTitle size={108} color={ACC}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagAT w={210} /></Waving><BigTitle size={64} glow={AT_RED}>AUSTRIA</BigTitle></div>
        </div>
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['EURO CHAMPIONS', 'SPAIN'], ['THE EAGLE', 'AUSTRIA'], ['THE BULL', 'vs THE EAGLE'], ['LAST 32', 'WIN OR GO HOME']].map(([v, l], i) => (
...
function SceneSpain() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={44} end={49}><div style={{ position: 'absolute', inset: 0 }}><FS id="spa-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagES w={64} />} label="LA ROJA · SPAIN" accent={ES_RED} /></div></Sprite>
      <Sprite start={49} end={54}><div style={{ position: 'absolute', inset: 0 }}><FS id="spa-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <Sprite start={54} end={55.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="spa-surge" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(198,11,30,0.30), transparent 55%)' }} /></div></Sprite>
      <PlayerShowcase clipId="spa-morata" name="ÁLVARO MORATA" role="CAPTAIN · THE FINISHER · 7" accent={ES_RED} start={55.5} end={61.47} />
      <PlayerShowcase clipId="spa-yamal" name="LAMINE YAMAL" role="THE WIZARD · 19" accent={ES_YELLOW} start={61.47} end={65.47} />
      <PlayerShowcase clipId="spa-pedri" name="PEDRI" role="THE CONDUCTOR · 8" accent={ES_RED} start={65.47} end={68.0} />
      <PlayerShowcase clipId="spa-rodri" name="RODRI" role="THE METRONOME · 16" accent={ES_YELLOW} start={68.0} end={74.19} />
      <PlayerShowcase clipId="spa-nicowilliams" name="NICO WILLIAMS" role="THE FLYER · 17" accent={ES_RED} start={74.19} end={79.5} />
    </div>
  );
}
...
function SceneAustria() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={79.5} end={84.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="aut-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagAT w={64} />} label="THE EAGLE · AUSTRIA" accent={AT_RED} /></div></Sprite>
      <Sprite start={84.5} end={90.6}><div style={{ position: 'absolute', inset: 0 }}><FS id="aut-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <PlayerShowcase clipId="aut-alaba" name="DAVID ALABA" role="CAPTAIN · THE GENERAL · 8" accent={AT_RED} start={90.6} end={96.07} />
      <PlayerShowcase clipId="aut-sabitzer" name="MARCEL SABITZER" role="THE ENGINE · 7" accent={AT_WHITE} start={96.07} end={99.52} />
      <PlayerShowcase clipId="aut-arnautovic" name="MARKO ARNAUTOVIĆ" role="THE MENACE · 7" accent={AT_RED} start={99.52} end={103.0} />
      <PlayerShowcase clipId="aut-laimer" name="KONRAD LAIMER" role="THE STORM · 6" accent={AT_WHITE} start={103.0} end={106.36} />
      <PlayerShowcase clipId="aut-baumgartner" name="CHRISTOPH BAUMGARTNER" role="THE POACHER · 19" accent={AT_RED} start={106.36} end={113.0} />
    </div>
  );
}
...
function SceneRiddle() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={113} end={118}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.24)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE BULL vs THE EAGLE</div></div></Sprite>
      ...
      <Sprite start={123} end={132}><BeatCard clipId="bg-riddle" start={123} end={132} text={<>DOES THE BULL CHARGE THROUGH,<br />OR THE EAGLE STRIKE?</>} accent={ACC} big={56} /></Sprite>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(224,162,38,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${ES_RED}44 0%, #070b14 38%, #070b14 62%, ${AT_RED}33 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${ES_RED} 0%, ${ACC} 50%, ${AT_RED} 100%)`, padding: '13px 0', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '36px 70px 28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 46 }}>
            <Badge flag={<FlagES w={104} />} name="SPAIN" accent={ES_RED} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 116, color: '#fff', lineHeight: 1 }}>
                <span style={{ textShadow: `0 0 36px ${ES_RED}cc` }}>2</span>
                <span style={{ color: ACC, fontSize: 58, transform: 'translateY(-6px)' }}>—</span>
                <span style={{ textShadow: `0 0 36px ${AT_RED}cc` }}>1</span>
              </div>
            ...
            <Badge flag={<FlagAT w={104} />} name="AUSTRIA" accent={AT_RED} />
          </div>
          <div style={{ textAlign: 'center', marginTop: 16, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 32, color: ACC, letterSpacing: '0.05em', textShadow: `0 0 26px ${ACC}66` }}>THE BULL CHARGES · 2–1</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 16, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#f3e6c4', background: 'rgba(224,162,38,0.16)', border: '1px solid rgba(224,162,38,0.45)', borderRadius: 999, padding: '7px 18px' }}>🐂 MORATA — THE LATE WINNER</span>
          </div>
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <Sprite start={132} end={137}><BeatCard clipId="bg-firing" start={132} end={137} text={<>AUSTRIA COME<br />OUT FLYING</>} accent="#ff9a9a" big={62} /></Sprite>
      <Sprite start={137} end={142}><div style={{ position: 'absolute', inset: 0 }}><FS id="keeper-save" /><ChanceTag start={137.5} end={142} text="SPAIN RATTLED EARLY" sub="THE EAGLE CIRCLES" accent={AT_RED} /></div></Sprite>
      <Sprite start={142} end={152.21}><BeatCard clipId="bg-shaken" start={142} end={152.21} text={<>THE CHAMPIONS<br />ARE SHAKEN</>} accent="#fff" big={68} /></Sprite>
      <Sprite start={152.21} end={159}><div style={{ position: 'absolute', inset: 0 }}><FS id="aut-goal" /><GoalFlash at={153.6} /><ChanceTag start={154} end={159} text="SABITZER — AUSTRIA LEAD!" sub="THE EAGLE DRAWS BLOOD" accent={AT_RED} /></div></Sprite>
      <Sprite start={159} end={172}><BeatCard clipId="bg-rise" start={159} end={172} text={<>BUT THIS BULL CHARGES BACK</>} sub="SPAIN STEADY · THE BALL SINGS" accent={ACC} big={60} /></Sprite>
      ...
      <Sprite start={178.46} end={185}><div style={{ position: 'absolute', inset: 0 }}><FS id="spa-goal-1" /><GoalFlash at={179.9} /><ChanceTag start={180.3} end={185} text="YAMAL — ONE-ONE!" sub="THE BULL IS CHARGING" accent={ES_RED} /></div></Sprite>
      <Sprite start={185} end={189.58}><BeatCard clipId="bg-roar" start={185} end={189.58} text={<>AND THEN —<br />THE CAPTAIN</>} accent={ACC} big={70} /></Sprite>
      <Sprite start={189.58} end={203.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="spa-goal-2" /><GoalFlash at={191.0} /><ChanceTag start={191.4} end={196} text="MORATA — 2-1 SPAIN!" sub="THE BULL TAKES THE LEAD" accent={ACC} /><ScoreBug start={196.3} es={2} at={1} note="MORATA 88'" badge="OUR PREDICTION" /><PredictionCard start={196.9} /></div></Sprite>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={203.32} end={208.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="vd-handshake" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.25) 0%, transparent 45%, rgba(2,3,8,0.55) 100%)' }} /><div style={{ position: 'absolute', left: 0, right: 0, top: 120, textAlign: 'center', zIndex: 24 }}><Kicker size={28} color={ACC}>The Bull Charges On</Kicker></div></div></Sprite>
      <Sprite start={208.32} end={213.32}><BeatCard clipId="bg-verdict" start={208.32} end={213.32} text={<>AUSTRIA SHOOK THE CHAMPIONS —<br />THE BULL FOUND A WAY</>} accent="#fff" big={54} /></Sprite>
      <Sprite start={213.32} end={218.32}><BeatCard clipId="bg-winner" start={213.32} end={218.32} text={<>MORATA</>} sub="THE WINNER · LATE" accent={ACC} big={64} /></Sprite>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
            <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 900, backdropFilter: 'blur(6px)', opacity: panelP, transform: `translateY(${(1 - panelP) * 24}px)` }}>
              ...
              <div style={{ marginTop: 24 }}>
                <StatLine start={219.5} delay={0.0} label="SPAIN" value="LA ROJA · EL TORO" accent={ES_RED} />
                <StatLine start={219.5} delay={0.25} label="AUSTRIA" value="THE EAGLE" accent={AT_RED} />
                <StatLine start={219.5} delay={0.5} label="OUR PREDICTION" value="ESP 2 — 1 AUT" accent="#fff" />
                <StatLine start={219.5} delay={0.75} label="88' MORATA" value="THE LATE WINNER" accent={ACC} />
              </div>
            </div>
          </div>
        </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT SPAIN', sub: 'LA ROJA', flag: <FlagES w={76} />, accent: ES_RED },
    { label: 'COMMENT AUSTRIA', sub: 'THE EAGLE', flag: <FlagAT w={76} />, accent: AT_RED },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 140, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: headP }}><Kicker size={30}>The Bull, Or The Eagle?</Kicker></div>
...
function MiniStrip() {
  const prev = ['legend-082-portrait', 'legend-083-portrait', 'legend-084-portrait', 'legend-085-portrait', 'legend-086-portrait'];
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
      ...
      <div style={{ width: 78, height: 104, borderRadius: 9, overflow: 'hidden', border: `2px solid ${ACC}`, boxShadow: `0 10px 28px rgba(0,0,0,0.6), 0 0 26px ${ACC}66` }}>
        <img data-seq alt="" src="assets/legend-087-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      </div>
    </div>
  );
}
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: 90, textAlign: 'center', opacity: teaseP, zIndex: 20 }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f3e0b8', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.7)' }}>THE SOUL OF LA ROJA</div>
        </div>
      )}
      {lt > 1.6 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 22, opacity: clamp(cardP, 0, 1) }}>
          <div style={{ position: 'relative', transform: `perspective(1500px) rotateY(${tilt}deg) scale(${0.92 + 0.08 * Math.min(cardP, 1)})`, marginTop: -34 }}>
            <img data-seq src="assets/legend-087-portrait.png" alt="" style={{ height: 640, display: 'block', borderRadius: 16, boxShadow: '0 50px 130px rgba(0,0,0,0.85), 0 0 80px rgba(224,162,38,0.5)' }} />
            ...
            <div style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', background: 'rgba(224,162,38,0.95)', color: '#1a1306', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 17, letterSpacing: '0.14em', padding: '5px 16px', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.5)' }}>Nº 087 · ✦✦✦ ULTRA RARE</div>
          </div>
        </div>
      )}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, zIndex: 24 }}>
        <div style={{ textAlign: 'center', opacity: txtP, transform: `translateY(${(1 - txtP) * 16}px)` }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 44, color: ACC, letterSpacing: '0.03em', textShadow: '0 2px 24px rgba(224,162,38,0.5)' }}>THE SPANISH BULL · EL TORO</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 19, color: '#ecd9b0', letterSpacing: '0.18em', marginTop: 6 }}>SPAIN · LEGEND 087</div>
        </div>
...
function PhoneCollect({ start }) {
  ...
  const filled = ['legend-083-portrait', 'legend-084-portrait', 'legend-085-portrait', 'legend-086-portrait'];
        ...
        <div style={{ width: '100%', height: '100%', borderRadius: 30, background: 'radial-gradient(ellipse at 50% 20%, #1a2030 0%, #06080c 70%)', overflow: 'hidden', position: 'relative', padding: '34px 18px 18px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            ...
            <div style={{ height: 104, borderRadius: 6, overflow: 'hidden', border: snapped ? `2px solid ${ACC}` : '1px dashed rgba(224,162,38,0.6)', position: 'relative', boxShadow: flash > 0 ? `0 0 ${20 * flash}px ${ACC}` : 'none' }}>
              {snapped && <img data-seq alt="" src="assets/legend-087-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 92, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: p }}>
        <Kicker size={30} color={ACC}>Claim the Spanish Bull</Kicker>
