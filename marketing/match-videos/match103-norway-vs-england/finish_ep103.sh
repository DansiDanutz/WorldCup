...
"$FF" -y -i music/cue-tense.mp3 -i sfx/braam.mp3 -i sfx/mystic.mp3 -i sfx/whoosh.mp3 \
  ...
  -map "[a]" -c:a aac -b:a 192k -t 15 intro_audio.m4a >/tmp/i103a.log 2>&1
echo "== intro.mp4 =="
"$FF" -y -framerate 30 -start_number 0 -i "$IF/f_%05d.jpg" -i intro_audio.m4a \
  ...
  -map "[v]" -map 1:a -c:v libx264 -pix_fmt yuv420p -crf 26 -preset medium -r 30 -c:a aac -b:a 160k -t 15 -movflags +faststart intro.mp4 >/tmp/i103e.log 2>&1
echo "== concat =="
printf "file 'intro.mp4'\nfile 'body_s.mp4'\n" > concat_list.txt
OUT="WorldCup26_Ep103_Norway_England.mp4"
"$FF" -y -f concat -safe 0 -i concat_list.txt -c copy -movflags +faststart "$OUT" >/tmp/c103.log 2>&1 || \
  "$FF" -y -f concat -safe 0 -i concat_list.txt -c:v libx264 -pix_fmt yuv420p -crf 26 -preset medium -c:a aac -b:a 160k -movflags +faststart "$OUT" >/tmp/c103.log 2>&1
