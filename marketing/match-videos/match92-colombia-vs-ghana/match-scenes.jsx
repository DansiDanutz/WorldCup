// match-scenes.jsx — Ep92 Colombia vs Ghana (R32) — PHOTOREAL + NO-REPEAT/NO-LOOP.
...
const CO_YELLOW = '#fcd116', CO_BLUE = '#003893', CO_RED = '#ce1126';
const GH_RED = '#ce1126', GH_GOLD = '#fcd116', GH_GREEN = '#006b3f';
const ACC = '#f4c430';
const GRADE = { filter: 'saturate(1.06) contrast(1.04)' };

function FlagCO({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ height: '50%', background: CO_YELLOW }} />
      <div style={{ height: '25%', background: CO_BLUE }} />
      <div style={{ height: '25%', background: CO_RED }} />
    </div>
  );
}
function FlagGH({ w = 120 }) {
  const h = w * 2 / 3;
  return (
...
function GoldStill({ start = 0, dur = 5 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', opacity: fade, zIndex: 1 }}>
      <img data-seq src="assets/legend-092-landscape.png" alt="" style={{ position: 'absolute', left: '50%', top: '50%', width: '108%', transform: `translate(-50%,-50%) scale(${scale})`, filter: 'saturate(1.05) contrast(1.03)' }} />
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ ...cell, background: CO_YELLOW, color: '#06121a' }}>COL</div>
        <div style={{ ...cell, fontSize: 38, color: ACC }}>{co} — {gh}</div>
        <div style={{ ...cell, background: GH_GREEN }}>GHA</div>
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <FS id="co-eldorado" /><FS id="gh-blackstar" /><FS id="texture-ghana" />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f0dca6', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE GOLDEN KING MEETS THE BLACK STAR.</div>
        </div>
      )}
      {lt > 19.2 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, opacity: titleP, zIndex: 22 }}>
          <Kicker size={32} color={ACC}>El Dorado and The Black Star</Kicker>
          <TitleReveal text="EL DORADO" start={20.0} size={84} color={ACC} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 38 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 92 · Last 32</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 52, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagCO w={210} /></Waving><BigTitle size={54} glow={CO_YELLOW}>COLOMBIA</BigTitle></div>
          <BigTitle size={104} color={ACC}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagGH w={210} /></Waving><BigTitle size={64} glow={GH_GREEN}>GHANA</BigTitle></div>
        </div>
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['LOS CAFETEROS', 'COLOMBIA'], ['THE BLACK STARS', 'GHANA'], ['THE GOLDEN KING', 'vs THE STAR'], ['LAST 32', 'WIN OR GO HOME']].map(([v, l], i) => (
...
function SceneColombia() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={44} end={49}><div style={{ position: 'absolute', inset: 0 }}><FS id="co-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagCO w={64} />} label="LOS CAFETEROS · COLOMBIA" accent={CO_YELLOW} /></div></Sprite>
      <Sprite start={49} end={54}><div style={{ position: 'absolute', inset: 0 }}><FS id="co-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <Sprite start={54} end={55.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="co-surge" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(252,209,22,0.30), transparent 55%)' }} /></div></Sprite>
      <PlayerShowcase clipId="co-james" name="JAMES RODRÍGUEZ" role="CAPTAIN · THE ARTIST · 10" accent={CO_YELLOW} start={55.5} end={60.5} />
      <PlayerShowcase clipId="co-diaz" name="LUIS DÍAZ" role="THE LIGHTNING · 7" accent={ACC} start={60.5} end={63.74} />
      <PlayerShowcase clipId="co-duran" name="JHON DURÁN" role="THE CANNON · 9" accent={CO_YELLOW} start={63.74} end={68.0} />
      <PlayerShowcase clipId="co-rios" name="RICHARD RÍOS" role="THE ENGINE · 8" accent={ACC} start={68.0} end={70.99} />
      <PlayerShowcase clipId="co-davinson" name="DÁVINSON SÁNCHEZ" role="THE TOWER · 23" accent={CO_YELLOW} start={70.99} end={79.5} />
    </div>
  );
}
...
function SceneGhana() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={79.5} end={84.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="gh-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagGH w={64} />} label="THE BLACK STARS · GHANA" accent={GH_GOLD} /></div></Sprite>
      <Sprite start={84.5} end={90.6}><div style={{ position: 'absolute', inset: 0 }}><FS id="gh-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <PlayerShowcase clipId="gh-ayew" name="JORDAN AYEW" role="CAPTAIN · THE WARRIOR · 10" accent={GH_GOLD} start={90.6} end={96.8} />
      <PlayerShowcase clipId="gh-kudus" name="MOHAMMED KUDUS" role="THE JEWEL · 20" accent={GH_RED} start={96.8} end={99.95} />
      <PlayerShowcase clipId="gh-partey" name="THOMAS PARTEY" role="THE METRONOME · 5" accent={GH_GOLD} start={99.95} end={103.0} />
      <PlayerShowcase clipId="gh-williams" name="IÑAKI WILLIAMS" role="THE FLASH · 9" accent={GH_RED} start={103.0} end={105.71} />
      <PlayerShowcase clipId="gh-salisu" name="MOHAMMED SALISU" role="THE WALL · 4" accent={GH_GOLD} start={105.71} end={113.0} />
    </div>
  );
}
...
function SceneRiddle() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={113} end={118}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.24)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE GOLDEN KING vs THE BLACK STAR</div></div></Sprite>
      ...
      <Sprite start={123} end={132}><BeatCard clipId="bg-riddle" start={123} end={132} text={<>DOES THE GOLD SHINE TOO BRIGHT,<br />OR DO THE STARS STEAL IT?</>} accent={ACC} big={50} /></Sprite>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(244,196,48,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${CO_YELLOW}33 0%, #070b14 38%, #070b14 62%, ${GH_GREEN}55 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${CO_YELLOW} 0%, ${ACC} 50%, ${GH_GREEN} 100%)`, padding: '13px 0', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '36px 70px 28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 46 }}>
            <Badge flag={<FlagCO w={104} />} name="COLOMBIA" accent={CO_YELLOW} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 116, color: '#fff', lineHeight: 1 }}>
                <span style={{ textShadow: `0 0 36px ${CO_YELLOW}cc` }}>2</span>
                <span style={{ color: ACC, fontSize: 58, transform: 'translateY(-6px)' }}>—</span>
                <span style={{ textShadow: `0 0 36px ${GH_GREEN}cc` }}>1</span>
              </div>
            ...
            <Badge flag={<FlagGH w={104} />} name="GHANA" accent={GH_GOLD} />
          </div>
          <div style={{ textAlign: 'center', marginTop: 16, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 32, color: ACC, letterSpacing: '0.05em', textShadow: `0 0 26px ${ACC}66` }}>THE GOLDEN KING STANDS · 2–1</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 16, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#f3e6c4', background: 'rgba(244,196,48,0.16)', border: '1px solid rgba(244,196,48,0.45)', borderRadius: 999, padding: '7px 18px' }}>⚡ DÍAZ — THE LATE WINNER</span>
          </div>
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <Sprite start={132} end={137}><BeatCard clipId="bg-firing" start={132} end={137} text={<>GHANA COME<br />OUT FLYING</>} accent={GH_GOLD} big={62} /></Sprite>
      <Sprite start={137} end={142}><div style={{ position: 'absolute', inset: 0 }}><FS id="gh-surge" /><ChanceTag start={137.5} end={142} text="THE BLACK STARS PRESS" sub="POWERFUL AND FEARLESS" accent={GH_GOLD} /></div></Sprite>
      <Sprite start={142} end={152.21}><BeatCard clipId="texture-colombia" start={142} end={152.21} text={<>THE GOLDEN MEN<br />ARE ROCKED</>} accent="#fff" big={60} /></Sprite>
      <Sprite start={152.21} end={159}><div style={{ position: 'absolute', inset: 0 }}><FS id="gh-goal-1" /><GoalFlash at={153.6} /><ChanceTag start={154} end={159} text="GHANA LEAD!" sub="THE STARS ARE SHINING" accent={GH_GOLD} /></div></Sprite>
      <Sprite start={159} end={172}><BeatCard clipId="bg-rise" start={159} end={172} text={<>BUT GOLD DOES NOT TARNISH</>} sub="THE BALL FINDS JAMES" accent={ACC} big={54} /></Sprite>
      <Sprite start={172} end={178.46}><BeatCard clipId="bg-believe" start={172} end={178.46} text={<>THE ARTIST<br />GOES TO WORK</>} accent="#fff" big={68} /></Sprite>
      <Sprite start={178.46} end={185}><div style={{ position: 'absolute', inset: 0 }}><FS id="co-goal-1" /><GoalFlash at={179.9} /><ChanceTag start={180.3} end={185} text="JAMES — ONE-ONE!" sub="A TOUCH OF PURE GOLD" accent={CO_YELLOW} /></div></Sprite>
      <Sprite start={185} end={189.58}><BeatCard clipId="bg-roar" start={185} end={189.58} text={<>AND THEN —<br />THE LIGHTNING</>} accent={ACC} big={68} /></Sprite>
      <Sprite start={189.58} end={203.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="diaz-goal" /><GoalFlash at={191.0} /><ChanceTag start={191.4} end={196} text="DÍAZ — 2-1 COLOMBIA!" sub="THE GOLDEN KING STANDS" accent={ACC} /><ScoreBug start={196.3} co={2} gh={1} note="88'" badge="OUR PREDICTION" /><PredictionCard start={196.9} /></div></Sprite>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={203.32} end={208.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="vd-handshake" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.25) 0%, transparent 45%, rgba(2,3,8,0.55) 100%)' }} /><div style={{ position: 'absolute', left: 0, right: 0, top: 120, textAlign: 'center', zIndex: 24 }}><Kicker size={28} color={ACC}>The Golden King Stands</Kicker></div></div></Sprite>
      <Sprite start={208.32} end={213.32}><BeatCard clipId="bg-verdict" start={208.32} end={213.32} text={<>GHANA MADE THE GOLD SWEAT —<br />BUT THE KING SHONE THROUGH</>} accent="#fff" big={46} /></Sprite>
      <Sprite start={213.32} end={218.32}><BeatCard clipId="bg-winner" start={213.32} end={218.32} text={<>DÍAZ</>} sub="THE LATE WINNER" accent={ACC} big={64} /></Sprite>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
            <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 900, backdropFilter: 'blur(6px)', opacity: panelP, transform: `translateY(${(1 - panelP) * 24}px)` }}>
              ...
              <div style={{ marginTop: 24 }}>
                <StatLine start={219.5} delay={0.0} label="COLOMBIA" value="LOS CAFETEROS · EL DORADO" accent={CO_YELLOW} />
                <StatLine start={219.5} delay={0.25} label="GHANA" value="THE BLACK STARS OF AFRICA" accent={GH_GOLD} />
                <StatLine start={219.5} delay={0.5} label="OUR PREDICTION" value="COL 2 — 1 GHA" accent="#fff" />
                <StatLine start={219.5} delay={0.75} label="88' DÍAZ" value="THE LATE WINNER" accent={ACC} />
              </div>
            </div>
          </div>
        </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT COLOMBIA', sub: 'LOS CAFETEROS', flag: <FlagCO w={76} />, accent: CO_YELLOW },
    { label: 'COMMENT GHANA', sub: 'THE BLACK STARS', flag: <FlagGH w={76} />, accent: GH_GREEN },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 140, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: headP }}><Kicker size={30}>The Golden King, Or The Black Star?</Kicker></div>
...
function MiniStrip() {
  const prev = ['legend-087-portrait', 'legend-088-portrait', 'legend-089-portrait', 'legend-090-portrait', 'legend-091-portrait'];
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
      ...
      <div style={{ width: 78, height: 104, borderRadius: 9, overflow: 'hidden', border: `2px solid ${ACC}`, boxShadow: `0 10px 28px rgba(0,0,0,0.6), 0 0 26px ${ACC}66` }}>
        <img data-seq alt="" src="assets/legend-092-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      </div>
    </div>
  );
}
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: 90, textAlign: 'center', opacity: teaseP, zIndex: 20 }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f3e0b8', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.7)' }}>THE KING OF GOLD</div>
        </div>
      )}
      {lt > 1.6 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 22, opacity: clamp(cardP, 0, 1) }}>
          <div style={{ position: 'relative', transform: `perspective(1500px) rotateY(${tilt}deg) scale(${0.92 + 0.08 * Math.min(cardP, 1)})`, marginTop: -34 }}>
            <img data-seq src="assets/legend-092-portrait.png" alt="" style={{ height: 640, display: 'block', borderRadius: 16, boxShadow: '0 50px 130px rgba(0,0,0,0.85), 0 0 80px rgba(244,196,48,0.5)' }} />
            ...
            <div style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', background: 'rgba(244,196,48,0.95)', color: '#1a1306', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 17, letterSpacing: '0.14em', padding: '5px 16px', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.5)' }}>Nº 092 · ✦✦✦ ULTRA RARE</div>
          </div>
        </div>
      )}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, zIndex: 24 }}>
        <div style={{ textAlign: 'center', opacity: txtP, transform: `translateY(${(1 - txtP) * 16}px)` }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 44, color: ACC, letterSpacing: '0.03em', textShadow: '0 2px 24px rgba(244,196,48,0.5)' }}>EL DORADO · THE GOLDEN KING</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 19, color: '#ecd9b0', letterSpacing: '0.18em', marginTop: 6 }}>COLOMBIA · LEGEND 092</div>
        </div>
...
function PhoneCollect({ start }) {
  ...
  const filled = ['legend-088-portrait', 'legend-089-portrait', 'legend-090-portrait', 'legend-091-portrait'];
        ...
        <div style={{ width: '100%', height: '100%', borderRadius: 30, background: 'radial-gradient(ellipse at 50% 20%, #1a2030 0%, #06080c 70%)', overflow: 'hidden', position: 'relative', padding: '34px 18px 18px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            ...
            <div style={{ height: 104, borderRadius: 6, overflow: 'hidden', border: snapped ? `2px solid ${ACC}` : '1px dashed rgba(244,196,48,0.6)', position: 'relative', boxShadow: flash > 0 ? `0 0 ${20 * flash}px ${ACC}` : 'none' }}>
              {snapped && <img data-seq alt="" src="assets/legend-092-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 92, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: p }}>
        <Kicker size={30} color={ACC}>Claim the Gold</Kicker>
...
function SceneCTA() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={303.05} end={308.05}><FS id="co-cta" /></Sprite>
