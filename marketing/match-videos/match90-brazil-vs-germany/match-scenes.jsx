// match-scenes.jsx — Ep90 Brazil vs Germany (R32) — PHOTOREAL + NO-REPEAT/NO-LOOP.
// "THE CANARY AND THE KING". OUR PREDICTION Brazil 2-1 Germany (Havertz early; Vinicius
// equalise; late Brazil winner). Mystic (#21): BRAZIL = the Golden Canary, spirit of the
// beautiful game -> Legend 090 = the Golden Canary. GERMANY = the Barbarossa, the sleeping
// emperor-king (reused canon Ep76/Ep79). Germany REUSED from Ep79 (Rule #26). Brazil generated
// fresh with LIKENESS prompts + real numbers (#29). Beat captions over dimmed footage (#27).
// Nation-correct only (#28). #23 name<->image SYNC (measured VO onsets).

const BR_YELLOW = '#ffdf00', BR_GREEN = '#009c3b', BR_BLUE = '#002776';
const DE_RED = '#dd0000', DE_GOLD = '#ffce00', DE_BLACK = '#141414';
const ACC = '#f4c430';
const GRADE = { filter: 'saturate(1.06) contrast(1.04)' };

function FlagBR({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: BR_GREEN, boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', left: '50%', top: '50%', width: h * 0.92, height: h * 0.92, background: BR_YELLOW, transform: 'translate(-50%,-50%) rotate(45deg)' }} />
      <div style={{ position: 'absolute', left: '50%', top: '50%', width: h * 0.40, height: h * 0.40, borderRadius: '50%', background: BR_BLUE, transform: 'translate(-50%,-50%)' }} />
    </div>
  );
}
function FlagDE({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <div style={{ flex: 1, background: DE_RED }} />
      <div style={{ flex: 1, background: DE_GOLD }} />
    </div>
  );
}

function FS({ id, style }) { return <ClipSprite id={id} fit="cover" style={{ ...GRADE, ...(style || {}) }} />; }
// Golden Canary card art (slow Ken-Burns) — Brazil's cold-open symbol.
function CanaryStill({ start = 0, dur = 5 }) {
...
function ScoreBug({ start, br = 2, de = 1, note = "88'", badge = 'OUR PREDICTION' }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ ...cell, background: BR_GREEN }}>BRA</div>
        <div style={{ ...cell, fontSize: 38, color: ACC }}>{br} — {de}</div>
        <div style={{ ...cell, background: DE_BLACK }}>GER</div>
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <CanaryStill start={0} dur={5} />
      <FS id="ger-barbarossa" /><FS id="texture-ravens" /><FS id="ger-blackforest" />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f0dca6', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE CANARY MEETS THE KING.</div>
        </div>
      )}
      {lt > 19.2 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, opacity: titleP, zIndex: 22 }}>
          <Kicker size={32} color={ACC}>The Canary and The King</Kicker>
          <TitleReveal text="THE CANARY" start={20.0} size={84} color={ACC} />
        </div>
      )}
    </div>
  );
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 38 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 56, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagBR w={210} /></Waving><BigTitle size={64} glow={BR_GREEN}>BRAZIL</BigTitle></div>
          <BigTitle size={108} color={ACC}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagDE w={210} /></Waving><BigTitle size={64} glow={DE_RED}>GERMANY</BigTitle></div>
        </div>
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['5-TIME KINGS', 'BRAZIL'], ['4-TIME KINGS', 'GERMANY'], ['THE CANARY', 'vs THE KING'], ['LAST 32', 'WIN OR GO HOME']].map(([v, l], i) => (
...
function SceneBrazil() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={44} end={49}><div style={{ position: 'absolute', inset: 0 }}><FS id="bra-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagBR w={64} />} label="THE GOLDEN CANARY · BRAZIL" accent={BR_GREEN} /></div></Sprite>
      <Sprite start={49} end={54}><div style={{ position: 'absolute', inset: 0 }}><FS id="bra-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <Sprite start={54} end={55.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="bra-surge" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(0,156,59,0.30), transparent 55%)' }} /></div></Sprite>
      <PlayerShowcase clipId="bra-vinicius" name="VINÍCIUS JR" role="THE LIGHTNING · 7" accent={BR_GREEN} start={55.5} end={60.5} />
      <PlayerShowcase clipId="bra-rodrygo" name="RODRYGO" role="THE GLIDER · 10" accent={ACC} start={60.5} end={63.74} />
      <PlayerShowcase clipId="bra-raphinha" name="RAPHINHA" role="THE FIRE · 11" accent={BR_GREEN} start={63.74} end={68.0} />
      <PlayerShowcase clipId="bra-casemiro" name="CASEMIRO" role="THE WALL · 5" accent={ACC} start={68.0} end={70.99} />
      <PlayerShowcase clipId="bra-marquinhos" name="MARQUINHOS" role="CAPTAIN · THE GENERAL · 4" accent={BR_GREEN} start={70.99} end={79.5} />
    </div>
  );
}
...
function SceneGermany() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={79.5} end={84.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="ger-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagDE w={64} />} label="THE BARBAROSSA · GERMANY" accent={DE_RED} /></div></Sprite>
      <Sprite start={84.5} end={90.6}><div style={{ position: 'absolute', inset: 0 }}><FS id="ger-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <PlayerShowcase clipId="ger-wirtz" name="FLORIAN WIRTZ" role="THE GOLDEN BOY" accent={DE_GOLD} start={90.6} end={96.8} />
      <PlayerShowcase clipId="ger-kimmich" name="JOSHUA KIMMICH" role="THE ENGINE" accent={DE_RED} start={96.8} end={99.95} />
      <PlayerShowcase clipId="ger-musiala" name="JAMAL MUSIALA" role="THE MAGICIAN" accent={DE_GOLD} start={99.95} end={103.0} />
      <PlayerShowcase clipId="ger-havertz" name="KAI HAVERTZ" role="THE SPEARHEAD" accent={DE_RED} start={103.0} end={105.71} />
      <PlayerShowcase clipId="ger-rudiger" name="ANTONIO RÜDIGER" role="THE GRANITE" accent={DE_GOLD} start={105.71} end={113.0} />
    </div>
  );
}
...
function SceneRiddle() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={113} end={118}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.24)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE CANARY vs THE KING</div></div></Sprite>
      ...
      <Sprite start={123} end={132}><BeatCard clipId="bg-riddle" start={123} end={132} text={<>DOES THE CANARY ANSWER THE GHOST,<br />OR THE KING CRUSH THE DREAM?</>} accent={ACC} big={52} /></Sprite>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(244,196,48,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${BR_GREEN}44 0%, #070b14 38%, #070b14 62%, ${DE_RED}44 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${BR_GREEN} 0%, ${ACC} 50%, ${DE_RED} 100%)`, padding: '13px 0', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '36px 70px 28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 46 }}>
            <Badge flag={<FlagBR w={104} />} name="BRAZIL" accent={BR_GREEN} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 116, color: '#fff', lineHeight: 1 }}>
                <span style={{ textShadow: `0 0 36px ${BR_GREEN}cc` }}>2</span>
                <span style={{ color: ACC, fontSize: 58, transform: 'translateY(-6px)' }}>—</span>
                <span style={{ textShadow: `0 0 36px ${DE_RED}cc` }}>1</span>
              </div>
            ...
            <Badge flag={<FlagDE w={104} />} name="GERMANY" accent={DE_RED} />
          </div>
          <div style={{ textAlign: 'center', marginTop: 16, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 32, color: ACC, letterSpacing: '0.05em', textShadow: `0 0 26px ${ACC}66` }}>THE CANARY SINGS AGAIN · 2–1</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 16, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#f3e6c4', background: 'rgba(244,196,48,0.16)', border: '1px solid rgba(244,196,48,0.45)', borderRadius: 999, padding: '7px 18px' }}>🐤 BRAZIL — THE LATE WINNER</span>
          </div>
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <Sprite start={132} end={137}><BeatCard clipId="bg-firing" start={132} end={137} text={<>GERMANY WAKE FIRST</>} accent={DE_GOLD} big={62} /></Sprite>
      <Sprite start={137} end={142}><div style={{ position: 'absolute', inset: 0 }}><FS id="ger-attack2" /><ChanceTag start={137.5} end={142} text="THE KING PILES FORWARD" sub="COLD AND CLINICAL" accent={DE_RED} /></div></Sprite>
      <Sprite start={142} end={152.21}><BeatCard clipId="texture-brazil" start={142} end={152.21} text={<>THE OLD FEAR<br />CREEPS BACK</>} accent="#fff" big={66} /></Sprite>
      <Sprite start={152.21} end={159}><div style={{ position: 'absolute', inset: 0 }}><FS id="havertz-goal" /><GoalFlash at={153.6} /><ChanceTag start={154} end={159} text="HAVERTZ — GERMANY LEAD!" sub="THE GHOST AWAKES" accent={DE_RED} /></div></Sprite>
      <Sprite start={159} end={172}><BeatCard clipId="bg-rise" start={159} end={172} text={<>BUT THE CANARY DOES NOT DIE</>} sub="BRAZIL STEADY · BELIEF RISES" accent={ACC} big={54} /></Sprite>
      <Sprite start={172} end={178.46}><BeatCard clipId="bg-believe" start={172} end={178.46} text={<>THE YELLOW<br />WAVE RISES</>} accent="#fff" big={68} /></Sprite>
      <Sprite start={178.46} end={185}><div style={{ position: 'absolute', inset: 0 }}><FS id="bra-goal-1" /><GoalFlash at={179.9} /><ChanceTag start={180.3} end={185} text="VINÍCIUS — ONE-ONE!" sub="THE MARACANÃ ERUPTS" accent={BR_GREEN} /></div></Sprite>
      <Sprite start={185} end={189.58}><BeatCard clipId="bg-roar" start={185} end={189.58} text={<>AND THEN —<br />THE RELEASE</>} accent={ACC} big={68} /></Sprite>
      <Sprite start={189.58} end={203.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="bra-goal-2" /><GoalFlash at={191.0} /><ChanceTag start={191.4} end={196} text="BRAZIL — 2-1!" sub="TWELVE YEARS, ANSWERED" accent={ACC} /><ScoreBug start={196.3} br={2} de={1} note="90'" badge="OUR PREDICTION" /><PredictionCard start={196.9} /></div></Sprite>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={203.32} end={208.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="vd-handshake" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.25) 0%, transparent 45%, rgba(2,3,8,0.55) 100%)' }} /><div style={{ position: 'absolute', left: 0, right: 0, top: 120, textAlign: 'center', zIndex: 24 }}><Kicker size={28} color={ACC}>The Canary Sings Again</Kicker></div></div></Sprite>
      <Sprite start={208.32} end={213.32}><BeatCard clipId="bg-verdict" start={208.32} end={213.32} text={<>GERMANY WERE KINGS —<br />BUT THE BEAUTIFUL GAME ANSWERED</>} accent="#fff" big={48} /></Sprite>
      <Sprite start={213.32} end={218.32}><BeatCard clipId="bg-winner" start={213.32} end={218.32} text={<>BRAZIL</>} sub="THE LATE WINNER" accent={ACC} big={64} /></Sprite>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
            <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 900, backdropFilter: 'blur(6px)', opacity: panelP, transform: `translateY(${(1 - panelP) * 24}px)` }}>
              ...
              <div style={{ marginTop: 24 }}>
                <StatLine start={219.5} delay={0.0} label="BRAZIL" value="THE GOLDEN CANARY" accent={BR_GREEN} />
                <StatLine start={219.5} delay={0.25} label="GERMANY" value="THE BARBAROSSA · THE KING" accent={DE_RED} />
                <StatLine start={219.5} delay={0.5} label="OUR PREDICTION" value="BRA 2 — 1 GER" accent="#fff" />
                <StatLine start={219.5} delay={0.75} label="LATE WINNER" value="THE MINEIRÃO, ANSWERED" accent={ACC} />
              </div>
            </div>
          </div>
        </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT BRAZIL', sub: 'THE GOLDEN CANARY', flag: <FlagBR w={76} />, accent: BR_GREEN },
    { label: 'COMMENT GERMANY', sub: 'THE BARBAROSSA', flag: <FlagDE w={76} />, accent: DE_RED },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 140, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: headP }}><Kicker size={30}>The Canary, Or The King?</Kicker></div>
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: 90, textAlign: 'center', opacity: teaseP, zIndex: 20 }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f3e0b8', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.7)' }}>THE SPIRIT OF THE BEAUTIFUL GAME</div>
        </div>
      )}
      {lt > 1.6 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 22, opacity: clamp(cardP, 0, 1) }}>
          <div style={{ position: 'relative', transform: `perspective(1500px) rotateY(${tilt}deg) scale(${0.92 + 0.08 * Math.min(cardP, 1)})`, marginTop: -34 }}>
            <img data-seq src="assets/legend-090-portrait.png" alt="" style={{ height: 640, display: 'block', borderRadius: 16, boxShadow: '0 50px 130px rgba(0,0,0,0.85), 0 0 80px rgba(244,196,48,0.5)' }} />
            ...
            <div style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', background: 'rgba(244,196,48,0.95)', color: '#1a1306', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 17, letterSpacing: '0.14em', padding: '5px 16px', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.5)' }}>Nº 090 · ✦✦✦ ULTRA RARE</div>
          </div>
        </div>
      )}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, zIndex: 24 }}>
        <div style={{ textAlign: 'center', opacity: txtP, transform: `translateY(${(1 - txtP) * 16}px)` }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 44, color: ACC, letterSpacing: '0.03em', textShadow: '0 2px 24px rgba(244,196,48,0.5)' }}>THE GOLDEN CANARY · SPIRIT OF THE GAME</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 19, color: '#ecd9b0', letterSpacing: '0.18em', marginTop: 6 }}>BRAZIL · LEGEND 090</div>
        </div>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 92, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: p }}>
        <Kicker size={30} color={ACC}>Claim the Canary</Kicker>
