...
// with goals; never gridiron. Saudi Arabia (green/white) vs Uruguay (sky blue/navy).
...
const MV = {
  ...
  gold: '#ffd24a',
  goldDeep: '#c9942e',
  // Saudi Arabia — the Green Falcons (HOME): green / white
  ksa: '#00a651',        // bright pitch green for type/glow
  ksaDeep: '#006c35',    // flag green
  // Uruguay — La Celeste (AWAY): sky blue / navy
  uru: '#5b9ad5',
  uruNavy: '#10243f',
  uruGold: '#f2c14e',    // the "four stars" / Maracanazo gold accent
  green: '#106b4f',
...
// Saudi Arabia: solid green field with a horizontal white sword bar (an
// abstraction of the national flag — we deliberately do NOT render the Shahada
// script, only the green field + sword motif).
function FlagKSA({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#006c35', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      {/* the sword (white bar with a pointed tip) along the lower third */}
      <div style={{ position: 'absolute', left: '12%', right: '24%', top: '64%', height: h * 0.07, background: '#fff', borderRadius: 2 }} />
      <div style={{ position: 'absolute', right: '14%', top: '60%', width: 0, height: 0,
        borderTop: `${h * 0.075}px solid transparent`, borderBottom: `${h * 0.075}px solid transparent`, borderLeft: `${h * 0.12}px solid #fff` }} />
      {/* a small calligraphic suggestion above the sword (purely abstract dashes) */}
      <div style={{ position: 'absolute', left: '16%', right: '20%', top: '34%', height: h * 0.05, background: 'rgba(255,255,255,0.85)', borderRadius: 3 }} />
    </div>
  );
}

// Uruguay: nine sky-blue & white stripes with a white canton bearing the Sun of May.
function FlagURU({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#fff', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      {/* nine horizontal stripes (5 sky-blue, 4 white) */}
      {Array.from({ length: 9 }).map((_, i) => (
        <div key={i} style={{ position: 'absolute', left: 0, right: 0, top: `${(i / 9) * 100}%`, height: `${100 / 9}%`, background: i % 2 === 0 ? '#5b9ad5' : '#fff' }} />
      ))}
      {/* white canton, top-left */}
      <div style={{ position: 'absolute', left: 0, top: 0, width: h * 0.5, height: h * 0.5, background: '#fff' }} />
      {/* Sun of May — golden disc with rays */}
      <div style={{ position: 'absolute', left: h * 0.25, top: h * 0.25, transform: 'translate(-50%,-50%)', width: h * 0.22, height: h * 0.22, borderRadius: '50%', background: '#f2c14e', boxShadow: '0 0 0 3px rgba(242,193,78,0.5)' }} />
    </div>
  );
}
