...
// Argentina — horizontal triband: sky-blue / white / sky-blue, golden Sun of May centred
function FlagARG({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <span style={{
        position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)',
        color: '#f6b40e', fontSize: h * 0.32, lineHeight: 1, fontFamily: SANS, fontWeight: 900,
        textShadow: '0 0 2px rgba(180,120,0,0.6)',
      }}>☀</span>
    </div>
  );
}

// Austria — horizontal triband: red / white / red
...
// Match scoreboard chip (top center). Ep43 is UNPLAYED — the 2–1 is OUR PREDICTION,
// so the badge defaults to "OUR PREDICTION", never LIVE/FULL TIME stated as fact.
function ScoreBug({ start, arg = 0, aut = 0, minute, badge = "OUR PREDICTION" }) {
  const t = useTime();
  const local = t - start;
  if (local < 0) return null;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ ...cell, background: MV.arg, color: '#0a1a2e' }}>ARG</div>
        <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{arg} — {aut}</div>
