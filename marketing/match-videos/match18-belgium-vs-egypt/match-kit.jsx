// match-kit.jsx — shared primitives for the Match 18 video (loads after animations.jsx)
// Dark cinematic broadcast theme. Episode 18 is IMAGE-BASED: Ken-Burns motion on
...
function FlagBEL({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', display: 'flex', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <div style={{ flex: 1, background: '#f9d616' }} />
      <div style={{ flex: 1, background: '#ef3340' }} />
    </div>
  );
}

// Egypt: horizontal red / white / black, with a central gold eagle-of-Saladin token.
function FlagEGY({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', left: 0, right: 0, top: 0, height: '33.34%', background: '#c8102e' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '33.34%', height: '33.33%', background: '#ffffff' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '66.67%', height: '33.33%', background: '#141414' }} />
      {/* central eagle-of-Saladin token (gold abstraction on the white band) */}
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: h * 0.26, height: h * 0.26, borderRadius: '50%', background: 'rgba(201,148,46,0.95)', border: '2px solid rgba(20,20,20,0.55)' }} />
    </div>
  );
}
...
// Match scoreboard chip (top center). BEL — EGY.
function ScoreBug({ start, bel = 0, egy = 0, minute }) {
  const t = useTime();
  const local = t - start;
  if (local < 0) return null;
  ...
  return (
    <div style={{
      ...
      <div style={{ ...cell, background: MV.bel, color: '#fff' }}>BEL</div>
      <div style={{ ...cell, fontSize: 38, color: MV.belGold }}>{bel} — {egy}</div>
      <div style={{ ...cell, background: MV.egy, color: '#fff' }}>EGY</div>
...
function Confetti({ start, dur, count = 90, zIndex = 24, colors }) {
  ...
  if (local < 0 || local > dur) return null;
  const W = 1920, H = 1080;
  const pal = colors || [MV.gold, '#fff', MV.bel, MV.belGold];
  const pieces = [];
