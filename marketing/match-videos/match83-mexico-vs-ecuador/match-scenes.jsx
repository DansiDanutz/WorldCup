// match-scenes.jsx — Ep83 Mexico vs Ecuador (R32) — PHOTOREAL + NO-REPEAT/NO-LOOP.
// "THE SERPENT AND THE CONDOR". OUR PREDICTION Ecuador 2-1 Mexico (Gimenez early; Valencia
// equalise; Paez late winner). Mystic (#21): MEXICO = Quetzalcoatl, the feathered serpent god;
// ECUADOR = the Andean Condor, the eternal guardian of the heights -> Legend 083 = the Andean
// Condor, Guardian of the Heights.
// #22 photoreal · #23 name<->image SYNC · #24 footer real mini-cards 078-082 + phone collect
// · #25 NO blank frames.

const MX_GREEN = '#1f7a4d', MX_WHITE = '#f3f4f7', MX_RED = '#ce1126';
const EC_YELLOW = '#ffd100', EC_BLUE = '#0072ce', EC_RED = '#ef3340';
const ACC = '#e0a226';           // condor-gold accent
...
function FlagEC({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ height: '50%', background: '#ffd100' }} />
      <div style={{ height: '25%', background: '#0072ce' }} />
      <div style={{ height: '25%', background: '#ef3340' }} />
    </div>
  );
}
...
function SunBeat({ start, dur, count = 28 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at ${50 + drift}% 40%, rgba(31,122,77,0.20) 0%, transparent 52%)` }} />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at ${50 - drift}% 64%, rgba(255,209,0,0.16) 0%, transparent 55%)` }} />
      <AmbientParticles start={start} dur={dur} count={count} color="240,200,140" />
    </div>
  );
}

function ScoreBug({ start, mex = 1, ecu = 2, note = "PÁEZ 88'", badge = 'OUR PREDICTION' }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        ...
        <div style={{ ...cell, fontSize: 38, color: ACC }}>{mex} — {ecu}</div>
        <div style={{ ...cell, background: EC_YELLOW, color: '#06121a' }}>ECU</div>
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <FS id="ec-condor" /><FS id="texture-ecuador" /><FS id="mx-quetzal" /><FS id="texture-mexico" />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f0dca6', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE SERPENT MEETS THE CONDOR.</div>
        </div>
      )}
      {lt > 19.2 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, opacity: titleP, zIndex: 22 }}>
          <Kicker size={32} color={ACC}>The Serpent and The Condor</Kicker>
          <TitleReveal text="THE ANDEAN CONDOR" start={20.0} size={80} color={ACC} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 38 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 83 · Last 32</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 56, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagEC w={210} /></Waving><BigTitle size={70} glow={EC_YELLOW}>ECUADOR</BigTitle></div>
        </div>
        <div style={{ opacity: p3, fontFamily: '"Inter",sans-serif', fontWeight: 600, fontSize: 30, color: MV.muted, letterSpacing: '0.10em' }}>WIN, OR YOUR TOURNAMENT DIES</div>
      </div>
    </div>
  );
}
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['CO-HOSTS', 'MEXICO'], ['CAICEDO & PÁEZ', 'ECUADOR'], ['THE SERPENT', 'vs THE CONDOR'], ['LAST 32', 'WIN OR GO HOME']].map(([v, l], i) => (
...
function SceneMexico() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <Sprite start={49} end={54}><div style={{ position: 'absolute', inset: 0 }}><FS id="mx-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <Sprite start={54} end={55.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="mx-surge" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(31,122,77,0.30), transparent 55%)' }} /></div></Sprite>
      <PlayerShowcase clipId="mx-gimenez" name="SANTIAGO GIMÉNEZ" role="THE FINISHER · 9" accent={MX_GREEN} start={55.5} end={59.0} />
      <PlayerShowcase clipId="mx-lozano" name="HIRVING LOZANO" role="CHUCKY · THE SPARK · 22" accent={MX_RED} start={59.0} end={62.4} />
      <PlayerShowcase clipId="mx-alvarez" name="EDSON ÁLVAREZ" role="CAPTAIN · THE ENGINE · 4" accent={MX_WHITE} start={62.4} end={68.0} />
      <PlayerShowcase clipId="mx-vega" name="ALEXIS VEGA" role="THE KEY · 10" accent={MX_GREEN} start={68.0} end={73.9} />
      <PlayerShowcase clipId="mx-montes" name="CÉSAR MONTES" role="THE TOWER · 3" accent={MX_RED} start={73.9} end={79.5} />
    </div>
  );
}
...
function SceneEcuador() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={79.5} end={84.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="ec-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagEC w={64} />} label="LA TRI · ECUADOR" accent={EC_YELLOW} /></div></Sprite>
      <Sprite start={84.5} end={90.6}><div style={{ position: 'absolute', inset: 0 }}><FS id="ec-press" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <PlayerShowcase clipId="ec-valencia" name="ENNER VALENCIA" role="THE TALISMAN · 13" accent={EC_YELLOW} start={90.6} end={94.5} />
      <PlayerShowcase clipId="ec-caicedo" name="MOISÉS CAICEDO" role="THE ENGINE · 23" accent={EC_BLUE} start={94.5} end={98.5} />
      <PlayerShowcase clipId="ec-paez" name="KENDRY PÁEZ" role="THE JEWEL · 10" accent={EC_YELLOW} start={98.5} end={103.0} />
      <PlayerShowcase clipId="ec-estupinan" name="PERVIS ESTUPIÑÁN" role="THE RAIDER · 7" accent={EC_BLUE} start={103.0} end={106.7} />
      <PlayerShowcase clipId="ec-hincapie" name="PIERO HINCAPIÉ" role="CAPTAIN · THE WALL · 3" accent={EC_YELLOW} start={106.7} end={113.0} />
    </div>
  );
}
...
function SceneRiddle() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={113} end={118}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.24)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE SERPENT vs THE CONDOR</div></div></Sprite>
      ...
      <Sprite start={123} end={132}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={123} dur={9} count={24} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 56, color: ACC, letterSpacing: '0.06em', textAlign: 'center', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>DOES THE SERPENT RULE,<br />OR THE CONDOR RISE?</div></div></div></Sprite>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(224,162,38,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${MX_GREEN}33 0%, #070b14 38%, #070b14 62%, ${EC_YELLOW}33 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${MX_GREEN} 0%, ${ACC} 50%, ${EC_YELLOW} 100%)`, padding: '13px 0', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 46 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 116, color: '#fff', lineHeight: 1 }}>
                ...
                <span style={{ textShadow: `0 0 36px ${EC_YELLOW}cc` }}>2</span>
              </div>
            ...
            <Badge flag={<FlagEC w={104} />} name="ECUADOR" accent={EC_YELLOW} />
          </div>
          <div style={{ textAlign: 'center', marginTop: 16, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 32, color: ACC, letterSpacing: '0.05em', textShadow: `0 0 26px ${ACC}66` }}>THE CONDOR RISES · 2–1</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 16, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#f3e6c4', background: 'rgba(224,162,38,0.16)', border: '1px solid rgba(224,162,38,0.45)', borderRadius: 999, padding: '7px 18px' }}>🦅 PÁEZ — THE LATE WINNER, AT 17</span>
          </div>
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <Sprite start={132} end={137}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={132} dur={5} count={20} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 62, color: '#3fae74', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>MEXICO COME OUT<br />ROARING</div></div></div></Sprite>
      <Sprite start={137} end={142}><div style={{ position: 'absolute', inset: 0 }}><FS id="keeper-save" /><ChanceTag start={137.5} end={142} text="ECUADOR HANG ON" sub="THE GREEN WALL ROARS" accent={EC_YELLOW} /></div></Sprite>
      <Sprite start={142} end={152.21}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={142} dur={10.21} count={22} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 70, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>THE STADIUM<br />SHAKES GREEN</div></div></div></Sprite>
      <Sprite start={152.21} end={159}><div style={{ position: 'absolute', inset: 0 }}><FS id="mx-goal" /><GoalFlash at={153.6} /><ChanceTag start={154} end={159} text="GIMÉNEZ — MEXICO LEAD!" sub="THE SERPENT COILS TIGHT" accent={MX_GREEN} /></div></Sprite>
      <Sprite start={159} end={164.37}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={159} dur={5.4} count={20} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 58, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>BUT YOU CANNOT<br />CAGE A CONDOR</div></div></div></Sprite>
      <Sprite start={164.37} end={172}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={164.37} dur={7.63} count={22} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: ACC, textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>ECUADOR RISE<div style={{ fontSize: 28, fontWeight: 800, color: '#fff', letterSpacing: '0.16em', marginTop: 14 }}>CAICEDO SEIZES THE MIDFIELD</div></div></div></div></Sprite>
      <Sprite start={172} end={178.46}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={172} dur={6.46} count={22} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 70, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>THE CONDOR<br />CLIMBS</div></div></div></Sprite>
      <Sprite start={178.46} end={185}><div style={{ position: 'absolute', inset: 0 }}><FS id="ec-goal-1" /><GoalFlash at={179.9} /><ChanceTag start={180.3} end={185} text="VALENCIA — ONE-ONE!" sub="THE CONDOR SPREADS ITS WINGS" accent={EC_YELLOW} /></div></Sprite>
      <Sprite start={185} end={189.58}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={185} dur={4.58} count={18} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 70, color: ACC, textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>AND THEN —<br />THE BOY</div></div></div></Sprite>
      <Sprite start={189.58} end={203.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="ec-goal-2" /><GoalFlash at={191.0} /><ChanceTag start={191.4} end={196} text="PÁEZ — 2-1 ECUADOR!" sub="THE CONDOR SOARS OVER A GIANT" accent={ACC} /><ScoreBug start={196.3} mex={1} ecu={2} note="PÁEZ 88'" badge="OUR PREDICTION" /><PredictionCard start={196.9} /></div></Sprite>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={203.32} end={208.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="vd-handshake" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.25) 0%, transparent 45%, rgba(2,3,8,0.55) 100%)' }} /><div style={{ position: 'absolute', left: 0, right: 0, top: 120, textAlign: 'center', zIndex: 24 }}><Kicker size={28} color={ACC}>The Condor Soars Again</Kicker></div></div></Sprite>
      <Sprite start={208.32} end={213.32}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={208.32} dur={5} count={20} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 56, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>MEXICO RULED THE NIGHT —<br />THE BOY FLEW HIGHER</div></div></div></Sprite>
      <Sprite start={213.32} end={218.32}><div style={{ position: 'absolute', inset: 0 }}><NightField o={0.8} /><AmbientParticles start={213.32} dur={5} count={20} color="240,200,140" /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: ACC, textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>KENDRY PÁEZ<div style={{ fontSize: 28, fontWeight: 800, color: '#fff', letterSpacing: '0.18em', marginTop: 12 }}>THE BOY · LATE</div></div></div></div></Sprite>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
            <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 900, backdropFilter: 'blur(6px)', opacity: panelP, transform: `translateY(${(1 - panelP) * 24}px)` }}>
              ...
              <div style={{ marginTop: 24 }}>
                <StatLine start={219.5} delay={0.0} label="MEXICO" value="EL TRI" accent={MX_GREEN} />
                <StatLine start={219.5} delay={0.25} label="ECUADOR" value="LA TRI" accent={EC_YELLOW} />
                <StatLine start={219.5} delay={0.5} label="OUR PREDICTION" value="MEX 1 — 2 ECU" accent="#fff" />
                <StatLine start={219.5} delay={0.75} label="88' PÁEZ" value="THE LATE WINNER" accent={ACC} />
              </div>
            </div>
          </div>
        </div>
...
function SceneEngage() {
  ...
  const cards = [
    ...
    { label: 'COMMENT ECUADOR', sub: 'THE CONDOR', flag: <FlagEC w={76} />, accent: EC_YELLOW },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 140, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: headP }}><Kicker size={30}>The Serpent, Or The Condor?</Kicker></div>
...
function MiniStrip() {
  const prev = ['legend-078-portrait', 'legend-079-portrait', 'legend-080-portrait', 'legend-081-portrait', 'legend-082-portrait'];
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
      ...
      <div style={{ width: 78, height: 104, borderRadius: 9, overflow: 'hidden', border: `2px solid ${ACC}`, boxShadow: `0 10px 28px rgba(0,0,0,0.6), 0 0 26px ${ACC}66` }}>
        <img data-seq alt="" src="assets/legend-083-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      </div>
    </div>
  );
}
...
function SceneMystery() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 46%, #1a2230 0%, #0f161e 50%, #06080c 100%)' }} />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: 90, textAlign: 'center', opacity: teaseP, zIndex: 20 }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f3e0b8', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.7)' }}>THE ONE WHO BOWS TO NO PEAK</div>
        </div>
      )}
      {lt > 1.6 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 22, opacity: clamp(cardP, 0, 1) }}>
          <div style={{ position: 'relative', transform: `perspective(1500px) rotateY(${tilt}deg) scale(${0.92 + 0.08 * Math.min(cardP, 1)})`, marginTop: -34 }}>
            <img data-seq src="assets/legend-083-portrait.png" alt="" style={{ height: 640, display: 'block', borderRadius: 16, boxShadow: '0 50px 130px rgba(0,0,0,0.85), 0 0 80px rgba(224,162,38,0.5)' }} />
            ...
            <div style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', background: 'rgba(224,162,38,0.95)', color: '#1a1306', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 17, letterSpacing: '0.14em', padding: '5px 16px', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.5)' }}>Nº 083 · ✦✦✦ ULTRA RARE</div>
          </div>
        </div>
      )}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, zIndex: 24 }}>
        <div style={{ textAlign: 'center', opacity: txtP, transform: `translateY(${(1 - txtP) * 16}px)` }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 42, color: ACC, letterSpacing: '0.03em', textShadow: '0 2px 24px rgba(224,162,38,0.5)' }}>THE ANDEAN CONDOR · GUARDIAN OF THE HEIGHTS</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 19, color: '#ecd9b0', letterSpacing: '0.18em', marginTop: 6 }}>ECUADOR · LEGEND 083</div>
        </div>
...
function PhoneCollect({ start }) {
  ...
  const filled = ['legend-079-portrait', 'legend-080-portrait', 'legend-081-portrait', 'legend-082-portrait'];
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 26, opacity: inP }}>
      <div style={{ position: 'relative', width: 300, height: 600, borderRadius: 40, background: 'linear-gradient(160deg,#23262e,#0c0e13)', border: '3px solid #2b2f38', boxShadow: '0 40px 110px rgba(0,0,0,0.8), 0 0 50px rgba(224,162,38,0.25)', padding: 12 }}>
        ...
        <div style={{ width: '100%', height: '100%', borderRadius: 30, background: 'radial-gradient(ellipse at 50% 20%, #14202c 0%, #06080c 70%)', overflow: 'hidden', position: 'relative', padding: '34px 18px 18px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            ...
            <div style={{ height: 104, borderRadius: 6, overflow: 'hidden', border: snapped ? `2px solid ${ACC}` : '1px dashed rgba(224,162,38,0.6)', position: 'relative', boxShadow: flash > 0 ? `0 0 ${20 * flash}px ${ACC}` : 'none' }}>
              {snapped && <img data-seq alt="" src="assets/legend-083-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 92, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: p }}>
        <Kicker size={30} color={ACC}>Claim the Andean Condor</Kicker>
