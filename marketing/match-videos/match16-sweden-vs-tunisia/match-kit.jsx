// match-kit.jsx — shared primitives for the Match 16 video (loads after animations.jsx)
// Dark cinematic broadcast theme. Episode 16 is IMAGE-BASED: Ken-Burns motion on
...
const MV = {
  ...
  gold: '#ffd24a',
  goldDeep: '#c9942e',
  // Sweden — Blagult: blue / yellow (the frost)
  swe: '#fecc00',        // Sweden yellow (primary accent)
  sweBlue: '#006aa7',    // Sweden blue
  // Tunisia — Eagles of Carthage: red / white (the fire)
  tun: '#e70013',        // Tunisia red
  tunWhite: '#ffffff',
  fire: '#ff5a1f',       // ember/fire accent for Tunisia
  frost: '#9ad8ff',      // frost accent for Sweden
  green: '#106b4f',
...
function FlagSWE({ w = 120 }) {
  const h = w * 5 / 8;
  const barT = h * 0.16;       // cross arm thickness
  const crossX = w * 0.34;     // vertical bar offset from left
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#006aa7', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      {/* horizontal arm */}
      <div style={{ position: 'absolute', left: 0, right: 0, top: '50%', height: barT, marginTop: -barT / 2, background: '#fecc00' }} />
      {/* vertical arm (off-centre, Nordic) */}
      <div style={{ position: 'absolute', top: 0, bottom: 0, left: crossX, width: barT, marginLeft: -barT / 2, background: '#fecc00' }} />
    </div>
  );
}

// Tunisia: red field with a white central disc, a red crescent and a red star.
function FlagTUN({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#e70013', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      {/* white central disc */}
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: disc, height: disc, borderRadius: '50%', background: '#fff' }} />
      {/* crescent: a red disc overlapping a white disc to carve the moon */}
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-38%,-50%)', width: disc * 0.74, height: disc * 0.74, borderRadius: '50%', background: '#fff' }} />
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-22%,-50%)', width: disc * 0.74, height: disc * 0.74, borderRadius: '50%', background: '#e70013' }} />
      {/* five-point star token */}
      <div style={{ position: 'absolute', left: '56%', top: '50%', transform: 'translate(-50%,-50%)', color: '#e70013', fontSize: disc * 0.46, lineHeight: 1, fontFamily: 'sans-serif' }}>★</div>
    </div>
  );
}
...
function LowerThird({ start, name, role, line, accent = MV.swe }) {
  const t = useTime();
  const local = t - start;
...
// Match scoreboard chip (top center). SWE — TUN.
function ScoreBug({ start, swe = 0, tun = 0, minute }) {
  const t = useTime();
  const local = t - start;
  if (local < 0) return null;
  ...
  return (
    <div style={{
      ...
      <div style={{ ...cell, background: MV.sweBlue, color: '#fff' }}>SWE</div>
      <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{swe} — {tun}</div>
      <div style={{ ...cell, background: MV.tun, color: '#fff' }}>TUN</div>
...
function Confetti({ start, dur, count = 90, zIndex = 24, colors }) {
  ...
  if (local < 0 || local > dur) return null;
  const W = 1920, H = 1080;
  const pal = colors || [MV.gold, '#fff', MV.swe, MV.sweBlue];
  const pieces = [];
