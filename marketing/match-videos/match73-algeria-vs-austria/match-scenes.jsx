// match-scenes.jsx — Ep73 Algeria vs Austria — PHOTOREAL + NO-REPEAT/NO-LOOP.
// "The Battle of the Midfields / The Reckoning" · Group J. OUR PREDICTION ALG 0–0 AUT —
// the climax is the 73rd-minute Mahrez left-foot curl DEFLECTED WIDE by Alaba (NO goal);
// the ending is a deliberate riddle ("who blinks first"). REAL fact (bridge): the 1982
// Disgrace of Gijon. Mystic (#21): ALGERIA = Tuareg 'Blue Men' + Tassili rock art (cold-open
// motif); AUSTRIA = the Krampus (Alpine folklore) = Legend 073.
// #22 photoreal · #23 player name↔image SYNC (windows match build_clips) ·
// #24 footer = REAL previous-5 mini-cards (068–072) + animated phone "collect in the app" ·
// #25 NO blank frames (every text beat over a clip/dimmed clip/rich motion graphics) ·
// #10 short labels only · #19 full-frame · #20 15s mystic intro.

const ALG_GREEN = '#0f7a3a', ALG_RED = '#d4213c', ALG_WHITE = '#eef5ef';
const AUT_RED = '#e4002b', AUT_WHITE = '#eef3f8';
const ACC = '#e6b94f';           // amber-gold — desert heat / Krampus embers / the reckoning
...
function FlagALG({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', display: 'flex', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ width: '50%', height: '100%', background: ALG_GREEN }} />
      <div style={{ width: '50%', height: '100%', background: '#fff' }} />
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', color: ALG_RED, fontSize: h * 0.5, lineHeight: 1 }}>☪</div>
    </div>
  );
}
function FlagAUT({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ height: '33.34%', background: AUT_RED }} />
      <div style={{ height: '33.33%', background: '#fff' }} />
      <div style={{ height: '33.33%', background: AUT_RED }} />
    </div>
  );
}

function FS({ id, style }) { return <ClipSprite id={id} fit="cover" style={{ ...GRADE, ...(style || {}) }} />; }
// Dimmed clip backdrop for text beats (rule #25 — imagery under every interstitial).
...
function NightField({ o = 0.6 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 44%, #11241a 0%, #0a1410 60%, #05060c 100%)' }} />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 50% 42%, rgba(230,185,74,${(0.13 * pulse * o).toFixed(3)}) 0%, transparent 55%)` }} />
    </div>
  );
}
// Rich warm motion-graphic backdrop for pure-narration beats (never near-black, #25).
function AuroraBeat({ start, dur, count = 30 }) {
  const { localTime: lt } = useSprite();
  const drift = Math.sin(lt * 0.5) * 6;
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 48%, #15331f 0%, #0c1a14 55%, #060a0c 100%)' }} />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at ${50 + drift}% 40%, rgba(230,185,74,0.20) 0%, transparent 52%)` }} />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at ${50 - drift}% 64%, rgba(15,122,58,0.22) 0%, transparent 55%)` }} />
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <FS id="alg-tuareg" /><FS id="aut-krampus" /><FS id="alg-tassili" /><FS id="aut-alps" /><FS id="texture-embers" />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f0dca6', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>GIJÓN, 1982 — THE DESERT NEVER FORGOT</div>
        </div>
      )}
      {lt > 19.2 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, opacity: titleP, zIndex: 22 }}>
          <Kicker size={32} color={ACC}>Forty-Four Years Later</Kicker>
          <TitleReveal text="THE RECKONING" start={20.0} size={104} color={ACC} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 38 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 73</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 56, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagALG w={210} /></Waving><BigTitle size={70} glow={ALG_GREEN}>ALGERIA</BigTitle></div>
          <BigTitle size={108} color={ACC}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagAUT w={210} /></Waving><BigTitle size={70} glow={AUT_RED}>AUSTRIA</BigTitle></div>
        </div>
        <div style={{ opacity: p3, fontFamily: '"Inter",sans-serif', fontWeight: 600, fontSize: 30, color: MV.muted, letterSpacing: '0.10em' }}>GROUP J · THE BATTLE OF THE MIDFIELDS</div>
      </div>
    </div>
  );
}
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['GROUP J', 'THE RECKONING'], ['FOXES', 'vs ALPINE WARRIORS'], ['MIDFIELD', 'THE BATTLEGROUND'], ['WHO BLINKS', 'FIRST?']].map(([v, l], i) => (
...
function SceneAlgeria() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={44} end={49}><div style={{ position: 'absolute', inset: 0 }}><FS id="alg-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagALG w={64} />} label="LES FENNECS · ALGERIA" accent={ALG_GREEN} /></div></Sprite>
...
function SceneAustria() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={79.5} end={84.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="aut-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagAUT w={64} />} label="DAS TEAM · AUSTRIA" accent={AUT_RED} /></div></Sprite>
...
function SceneRiddle() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={113} end={118}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.24)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>METRONOME vs ENGINE</div></div></Sprite>
      <FSDim id="duel-mahrez-alaba" dim={0.5}><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>NAMESAKE vs MAGICIAN</div></FSDim>
      ...
      <Sprite start={128} end={132}><FSDim id="texture-alpine-mist" dim={0.5}><div style={{ position: 'absolute', left: 0, right: 0, top: '40%', textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 60, color: ACC, letterSpacing: '0.12em', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>WHO BLINKS FIRST?</div></FSDim></Sprite>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(230,185,74,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${ALG_GREEN}30 0%, #070b14 38%, #070b14 62%, ${AUT_RED}33 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${ALG_GREEN} 0%, #11151f 50%, ${AUT_RED} 100%)`, padding: '13px 0', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '40px 70px 30px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 50 }}>
            <Badge flag={<FlagALG w={104} />} name="ALGERIA" accent={ALG_GREEN} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 132, color: '#fff', lineHeight: 1 }}>
                <span style={{ textShadow: `0 0 36px ${ALG_GREEN}99` }}>0</span>
                <span style={{ color: ACC, fontSize: 64, transform: 'translateY(-6px)' }}>—</span>
                <span style={{ textShadow: `0 0 36px ${AUT_RED}cc` }}>0</span>
              </div>
            ...
            <Badge flag={<FlagAUT w={104} />} name="AUSTRIA" accent={AUT_RED} />
          </div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 26, paddingTop: 22, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#f3e6c4', background: 'rgba(230,185,74,0.16)', border: '1px solid rgba(230,185,74,0.45)', borderRadius: 999, padding: '7px 18px' }}>73' MAHREZ CURL — ALABA DEFLECTS WIDE</span>
          </div>
          <div style={{ textAlign: 'center', marginTop: 22, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 23, color: ACC, letterSpacing: '0.16em' }}>★ DECIDED BY WHO WANTS IT MORE ★</div>
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <Sprite start={132} end={137}><FSDim id="sabitzer-shot" dim={0.5}><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 58, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>TWO ENGINES,<br />RUNNING HOT</div></div></FSDim></Sprite>
      <Sprite start={137} end={142}><FSDim id="texture-desert-heat" dim={0.5}><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 50, color: ACC, letterSpacing: '0.08em', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>DARBUKA vs<br />ALPINE DRUM</div></div></FSDim></Sprite>
      <Sprite start={142} end={147}><div style={{ position: 'absolute', inset: 0 }}><FS id="keeper-save" /><ChanceTag start={142.5} end={147} text="SABITZER" sub="CLAWED AWAY" accent="#f0dca6" /></div></Sprite>
      <Sprite start={147} end={152.21}><FSDim id="gouiri-goal" dim={0.5}><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 56, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>NEITHER GIVES<br />AN INCH</div></div></FSDim></Sprite>
      <Sprite start={152.21} end={164.37}><div style={{ position: 'absolute', inset: 0 }}><AuroraBeat start={152.21} dur={12.16} count={30} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 84, color: '#fff', letterSpacing: '0.03em', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>THE 73<span style={{ fontSize: 44 }}>RD</span> MINUTE<div style={{ fontSize: 26, fontWeight: 700, color: ACC, letterSpacing: '0.22em', marginTop: 14 }}>BENNACER TURNS UNDER LAIMER</div></div></div></div></Sprite>
      <Sprite start={164.37} end={169.37}><div style={{ position: 'absolute', inset: 0 }}><FS id="mahrez-curl" /><ChanceTag start={164.8} end={169.37} text="MAHREZ — THE CURL" sub="TOWARD THE TOP CORNER" accent={ALG_WHITE} /></div></Sprite>
      <Sprite start={169.37} end={178.46}><div style={{ position: 'absolute', inset: 0 }}><AuroraBeat start={169.37} dur={9.09} count={22} /><div style={{ position: 'absolute', left: 0, right: 0, top: '42%', textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 76, color: '#fff', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>THE STADIUM RISES</div></div></Sprite>
      <Sprite start={178.46} end={185}><div style={{ position: 'absolute', inset: 0 }}><FS id="alaba-block" /><ChanceTag start={179.0} end={185} text="ALABA — DEFLECTED WIDE!" sub="A HELD BREATH · NO GOAL" accent={ACC} /></div></Sprite>
      <Sprite start={185} end={194}><div style={{ position: 'absolute', inset: 0 }}><AuroraBeat start={185} dur={9} count={26} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 60, color: ACC, letterSpacing: '0.05em', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>NOT THE FAMOUS —<br />THE BURNING</div></div></div></Sprite>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
            <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 900, backdropFilter: 'blur(6px)', opacity: panelP, transform: `translateY(${(1 - panelP) * 24}px)` }}>
              ...
              <div style={{ marginTop: 24 }}>
                <StatLine start={219.5} delay={0.0} label="ALGERIA" value="LES FENNECS" accent={ALG_GREEN} />
                <StatLine start={219.5} delay={0.25} label="AUSTRIA" value="DAS TEAM" accent={AUT_RED} />
                <StatLine start={219.5} delay={0.5} label="OUR PREDICTION" value="ALG 0 — 0 AUT" accent="#fff" />
                <StatLine start={219.5} delay={0.75} label="73' THE DEFLECTION" value="NEITHER WARRIOR BLINKED" accent={ACC} />
              </div>
            </div>
          </div>
        </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT ALGERIA', sub: 'LES FENNECS', flag: <FlagALG w={76} />, accent: ALG_GREEN },
    { label: 'COMMENT AUSTRIA', sub: 'DAS TEAM', flag: <FlagAUT w={76} />, accent: AUT_RED },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 140, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: headP }}><Kicker size={30}>Who Wanted It More?</Kicker></div>
...
function MiniStrip() {
  const prev = ['legend-068-portrait', 'legend-069-portrait', 'legend-070-portrait', 'legend-071-portrait', 'legend-072-portrait'];
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
      ...
      <div style={{ width: 78, height: 104, borderRadius: 9, overflow: 'hidden', border: `2px solid ${ACC}`, boxShadow: `0 10px 28px rgba(0,0,0,0.6), 0 0 26px ${ACC}66` }}>
        <img data-seq alt="" src="assets/legend-073-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      </div>
    </div>
  );
}
...
function SceneMystery() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 46%, #4a3414 0%, #261a0a 50%, #0c0a06 100%)' }} />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: 90, textAlign: 'center', opacity: teaseP, zIndex: 20 }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f3e6c4', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.7)' }}>THE ONE WHO NEVER FORGETS A DEBT</div>
        </div>
      )}
      {lt > 1.6 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 22, opacity: clamp(cardP, 0, 1) }}>
          <div style={{ position: 'relative', transform: `perspective(1500px) rotateY(${tilt}deg) scale(${0.92 + 0.08 * Math.min(cardP, 1)})`, marginTop: -34 }}>
            <img data-seq src="assets/legend-073-portrait.png" alt="" style={{ height: 640, display: 'block', borderRadius: 16, boxShadow: '0 50px 130px rgba(0,0,0,0.85), 0 0 80px rgba(230,185,74,0.5)' }} />
            ...
            <div style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', background: 'rgba(230,185,74,0.95)', color: '#1a1306', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 17, letterSpacing: '0.14em', padding: '5px 16px', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.5)' }}>Nº 073 · ✦✦✦ ULTRA RARE</div>
          </div>
        </div>
      )}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, zIndex: 24 }}>
        <div style={{ textAlign: 'center', opacity: txtP, transform: `translateY(${(1 - txtP) * 16}px)` }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 48, color: ACC, letterSpacing: '0.04em', textShadow: '0 2px 24px rgba(230,185,74,0.5)' }}>THE KRAMPUS OF THE ALPS</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 19, color: '#ecdcb4', letterSpacing: '0.18em', marginTop: 6 }}>AUSTRIA · LEGEND 073</div>
        </div>
...
function PhoneCollect({ start }) {
  ...
  const filled = ['legend-069-portrait', 'legend-070-portrait', 'legend-071-portrait', 'legend-072-portrait'];
        ...
        <div style={{ width: '100%', height: '100%', borderRadius: 30, background: 'radial-gradient(ellipse at 50% 20%, #2a2010 0%, #0c0a06 70%)', overflow: 'hidden', position: 'relative', padding: '34px 18px 18px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            ...
            <div style={{ height: 104, borderRadius: 6, overflow: 'hidden', border: snapped ? `2px solid ${ACC}` : '1px dashed rgba(230,185,74,0.6)', position: 'relative', boxShadow: flash > 0 ? `0 0 ${20 * flash}px ${ACC}` : 'none' }}>
              {snapped && <img data-seq alt="" src="assets/legend-073-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 92, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: p }}>
        <Kicker size={30} color={ACC}>Claim The Krampus</Kicker>
