...
// Spain — horizontal red / yellow (double height) / red
...
// Saudi Arabia — green field, a white horizontal sword and a white Arabic-style script mark
function FlagKSA({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#006c35', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      {/* Arabic-style script mark (the shahada, stylised) */}
      <div style={{
        position: 'absolute', left: '50%', top: h * 0.34, transform: 'translate(-50%,-50%)',
        width: w * 0.62, height: h * 0.16, display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      }}>
        {[0.16, 0.10, 0.13, 0.09, 0.14, 0.10].map((bw, i) => (
          <div key={i} style={{ width: `${bw * 100}%`, height: '100%', background: '#fff', borderRadius: h * 0.04, transform: `skewX(${i % 2 ? -12 : 12}deg)` }} />
        ))}
      </div>
      {/* horizontal sword: blade + hilt */}
      <div style={{ position: 'absolute', left: w * 0.16, right: w * 0.22, top: h * 0.66, height: h * 0.05, background: '#fff', borderRadius: h * 0.03 }} />
      <div style={{
        position: 'absolute', left: w * 0.10, top: h * 0.605, width: 0, height: 0,
        borderTop: `${h * 0.075}px solid transparent`, borderBottom: `${h * 0.075}px solid transparent`,
        borderRight: `${w * 0.07}px solid #fff`,
      }} />
      <div style={{ position: 'absolute', left: w * 0.78, top: h * 0.575, width: w * 0.03, height: h * 0.21, background: '#fff', borderRadius: h * 0.02 }} />
    </div>
  );
}
...
// Match scoreboard chip (top center). Ep39 is UNPLAYED — the scoreline is a
// PREDICTION, so the badge defaults to "OUR PREDICTION", never "LIVE"/"FULL TIME".
function ScoreBug({ start, esp = 0, ksa = 0, minute, badge = "OUR PREDICTION" }) {
  const t = useTime();
  const local = t - start;
  if (local < 0) return null;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ ...cell, background: MV.esp }}>ESP</div>
        <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{esp} — {ksa}</div>
        <div style={{ ...cell, background: MV.ksa }}>KSA</div>
...
// Golden goal flash + text burst used at the goal moments (accent colours the burst).
function GoalFlash({ at, accent = MV.gold }) {
  const t = useTime();
  const local = t - at;
  ...
  return (<>
    ...
    <div style={{
      ...
      <div style={{
        fontFamily: SANS, fontWeight: 900, fontSize: 230, color: accent, letterSpacing: '0.04em',
        textShadow: `0 0 80px ${accent}aa, 0 10px 40px rgba(0,0,0,0.8)`, WebkitTextStroke: '4px rgba(0,0,0,0.45)',
      }}>GOAL!</div>
    </div>
  </>);
}
...
function Confetti({ start, dur, count = 90, zIndex = 24 }) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const colors = [MV.gold, '#fff', MV.espLight, MV.ksaLight];
    pieces.push(
      <div key={i} style={{
