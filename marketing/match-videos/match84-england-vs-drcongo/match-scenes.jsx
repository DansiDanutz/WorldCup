// match-scenes.jsx — Ep84 England vs DR Congo (R32) — PHOTOREAL + NO-REPEAT/NO-LOOP.
// "THE LIONS AND THE LEOPARD". OUR PREDICTION England 2-1 DR Congo (Bakambu early; Kane
// equalise; Bellingham late winner). Mystic (#21): ENGLAND = the Three Lions of the crown;
// DR CONGO = the Leopard (Les Leopards) -> Legend 084 = the Three Lions, Keepers of the Crown.
// DR Congo assets REUSED from Ep72 (Rule #26).
// #22 photoreal · #23 name<->image SYNC · #24 footer real mini-cards 079-083 + phone collect
// · #25 NO blank frames.

const ENG_WHITE = '#f3f4f7', ENG_NAVY = '#27348b', ENG_RED = '#ce1124';
const CD_RED = '#cf0921', CD_BLUE = '#1f8fff', CD_YELLOW = '#f7d618';
...
function FlagCD({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#1f8fff', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, transparent 36%, #cf0921 36%, #cf0921 64%, transparent 64%)' }} />
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, transparent 42%, #f7d618 42%, #f7d618 58%, transparent 58%)' }} />
      <div style={{ position: 'absolute', top: '8%', left: '8%', color: '#f7d618', fontSize: h * 0.3, lineHeight: 1 }}>★</div>
    </div>
  );
}
...
function SunBeat({ start, dur, count = 28 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at ${50 + drift}% 40%, rgba(39,52,139,0.20) 0%, transparent 52%)` }} />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at ${50 - drift}% 64%, rgba(207,9,33,0.16) 0%, transparent 55%)` }} />
      <AmbientParticles start={start} dur={dur} count={count} color="240,200,140" />
    </div>
  );
}

function ScoreBug({ start, eng = 2, drc = 1, note = "BELLINGHAM 88'", badge = 'OUR PREDICTION' }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ ...cell, background: ENG_WHITE, color: '#06121a' }}>ENG</div>
        <div style={{ ...cell, fontSize: 38, color: ACC }}>{eng} — {drc}</div>
        <div style={{ ...cell, background: CD_RED }}>COD</div>
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <FS id="eng-lions" /><FS id="texture-england" /><FS id="cod-leopard" /><FS id="cod-kinshasa" />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f0dca6', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE LIONS MEET THE LEOPARD.</div>
        </div>
      )}
      {lt > 19.2 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, opacity: titleP, zIndex: 22 }}>
          <Kicker size={32} color={ACC}>The Lions and The Leopard</Kicker>
          <TitleReveal text="THE THREE LIONS" start={20.0} size={84} color={ACC} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 38 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 84 · Last 32</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 56, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagENG w={210} /></Waving><BigTitle size={70} glow={ENG_NAVY}>ENGLAND</BigTitle></div>
          <BigTitle size={108} color={ACC}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagCD w={210} /></Waving><BigTitle size={62} glow={CD_RED}>DR CONGO</BigTitle></div>
        </div>
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['HOME OF FOOTBALL', 'ENGLAND'], ['THE LEOPARDS', 'DR CONGO'], ['THE LIONS', 'vs THE LEOPARD'], ['LAST 32', 'WIN OR GO HOME']].map(([v, l], i) => (
...
function SceneEngland() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={44} end={49}><div style={{ position: 'absolute', inset: 0 }}><FS id="eng-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagENG w={64} />} label="THREE LIONS · ENGLAND" accent={ENG_NAVY} /></div></Sprite>
      <Sprite start={49} end={54}><div style={{ position: 'absolute', inset: 0 }}><FS id="eng-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <Sprite start={54} end={55.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="eng-surge" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(39,52,139,0.30), transparent 55%)' }} /></div></Sprite>
      <PlayerShowcase clipId="eng-kane" name="HARRY KANE" role="CAPTAIN · THE FINISHER · 9" accent={ENG_NAVY} start={55.5} end={61.1} />
      <PlayerShowcase clipId="eng-bellingham" name="JUDE BELLINGHAM" role="THE BIG-GAME KING · 10" accent={ENG_RED} start={61.1} end={65.7} />
      <PlayerShowcase clipId="eng-saka" name="BUKAYO SAKA" role="THE STAR BOY · 7" accent={ENG_NAVY} start={65.7} end={68.0} />
      <PlayerShowcase clipId="eng-foden" name="PHIL FODEN" role="THE MAGICIAN · 11" accent={ENG_WHITE} start={68.0} end={71.8} />
      <PlayerShowcase clipId="eng-rice" name="DECLAN RICE" role="THE ENGINE · 4" accent={ENG_NAVY} start={71.8} end={79.5} />
    </div>
  );
}

function SceneDRCongo() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={79.5} end={84.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="cod-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagCD w={64} />} label="LES LÉOPARDS · DR CONGO" accent={CD_RED} /></div></Sprite>
      <Sprite start={84.5} end={90.6}><div style={{ position: 'absolute', inset: 0 }}><FS id="cod-attack" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <PlayerShowcase clipId="dc-wissa" name="YOANE WISSA" role="THE RUNNER · 11" accent={CD_RED} start={90.6} end={93.6} />
      <PlayerShowcase clipId="dc-silas" name="SILAS" role="THE BOLT · 7" accent={CD_BLUE} start={93.6} end={96.6} />
      <PlayerShowcase clipId="dc-mbemba" name="CHANCEL MBEMBA" role="CAPTAIN · THE ROCK · 5" accent={CD_RED} start={96.6} end={103.0} />
      <PlayerShowcase clipId="dc-bakambu" name="CÉDRIC BAKAMBU" role="THE POACHER · 13" accent={CD_YELLOW} start={103.0} end={106.6} />
      <PlayerShowcase clipId="dc-wanbissaka" name="AARON WAN-BISSAKA" role="THE TACKLER · 29" accent={CD_RED} start={106.6} end={113.0} />
    </div>
  );
}
...
function SceneRiddle() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={113} end={118}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.24)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE LIONS vs THE LEOPARD</div></div></Sprite>
      ...
      <Sprite start={123} end={132}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={123} dur={9} count={24} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 56, color: ACC, letterSpacing: '0.06em', textAlign: 'center', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>DO THE LIONS ROAR,<br />OR THE LEOPARD POUNCE?</div></div></div></Sprite>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(224,162,38,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${ENG_NAVY}33 0%, #070b14 38%, #070b14 62%, ${CD_RED}33 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${ENG_NAVY} 0%, ${ACC} 50%, ${CD_RED} 100%)`, padding: '13px 0', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 46 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 116, color: '#fff', lineHeight: 1 }}>
                ...
                <span style={{ textShadow: `0 0 36px ${CD_RED}cc` }}>1</span>
              </div>
            ...
            <Badge flag={<FlagCD w={104} />} name="DR CONGO" accent={CD_RED} />
          </div>
          <div style={{ textAlign: 'center', marginTop: 16, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 32, color: ACC, letterSpacing: '0.05em', textShadow: `0 0 26px ${ACC}66` }}>THE LIONS SURVIVE · 2–1</div>
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <Sprite start={132} end={137}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={132} dur={5} count={20} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 62, color: '#ff5a6e', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>DR CONGO COME<br />OUT FEARLESS</div></div></div></Sprite>
      <Sprite start={137} end={142}><div style={{ position: 'absolute', inset: 0 }}><FS id="keeper-save" /><ChanceTag start={137.5} end={142} text="ENGLAND RATTLED" sub="THE LEOPARD CIRCLES" accent={CD_RED} /></div></Sprite>
      <Sprite start={142} end={152.21}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={142} dur={10.21} count={22} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 70, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>THE GIANT<br />IS SHAKEN</div></div></div></Sprite>
      <Sprite start={152.21} end={159}><div style={{ position: 'absolute', inset: 0 }}><FS id="cd-goal" /><GoalFlash at={153.6} /><ChanceTag start={154} end={159} text="BAKAMBU — DR CONGO LEAD!" sub="THE LEOPARD DRAWS BLOOD" accent={CD_RED} /></div></Sprite>
      ...
      <Sprite start={164.37} end={172}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={164.37} dur={7.63} count={22} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: ACC, textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>ENGLAND STEADY<div style={{ fontSize: 28, fontWeight: 800, color: '#fff', letterSpacing: '0.16em', marginTop: 14 }}>FODEN PULLS THE STRINGS</div></div></div></div></Sprite>
      <Sprite start={172} end={178.46}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={172} dur={6.46} count={22} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 70, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>THE QUALITY<br />RISES</div></div></div></Sprite>
      <Sprite start={178.46} end={185}><div style={{ position: 'absolute', inset: 0 }}><FS id="eng-goal-1" /><GoalFlash at={179.9} /><ChanceTag start={180.3} end={185} text="KANE — ONE-ONE!" sub="THE LIONS ARE ROARING" accent={ENG_NAVY} /></div></Sprite>
      <Sprite start={185} end={189.58}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={185} dur={4.58} count={18} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 70, color: ACC, textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>AND THEN —<br />THE BELIEF</div></div></div></Sprite>
      <Sprite start={189.58} end={203.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="eng-goal-2" /><GoalFlash at={191.0} /><ChanceTag start={191.4} end={196} text="BELLINGHAM — 2-1 ENGLAND!" sub="THE LIONS SURVIVE THE SCARE" accent={ACC} /><ScoreBug start={196.3} eng={2} drc={1} note="BELLINGHAM 88'" badge="OUR PREDICTION" /><PredictionCard start={196.9} /></div></Sprite>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <Sprite start={208.32} end={213.32}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={208.32} dur={5} count={20} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>DR CONGO TERRIFIED A GIANT —<br />THE LIONS FOUND A WAY</div></div></div></Sprite>
      <Sprite start={213.32} end={218.32}><div style={{ position: 'absolute', inset: 0 }}><NightField o={0.8} /><AmbientParticles start={213.32} dur={5} count={20} color="240,200,140" /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: ACC, textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>BELLINGHAM<div style={{ fontSize: 28, fontWeight: 800, color: '#fff', letterSpacing: '0.18em', marginTop: 12 }}>THE HEADER · LATE</div></div></div></div></Sprite>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
            <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 900, backdropFilter: 'blur(6px)', opacity: panelP, transform: `translateY(${(1 - panelP) * 24}px)` }}>
              ...
              <div style={{ marginTop: 24 }}>
                <StatLine start={219.5} delay={0.0} label="ENGLAND" value="THE THREE LIONS" accent={ENG_NAVY} />
                <StatLine start={219.5} delay={0.25} label="DR CONGO" value="LES LÉOPARDS" accent={CD_RED} />
                <StatLine start={219.5} delay={0.5} label="OUR PREDICTION" value="ENG 2 — 1 COD" accent="#fff" />
                <StatLine start={219.5} delay={0.75} label="88' BELLINGHAM" value="THE LATE WINNER" accent={ACC} />
              </div>
            </div>
          </div>
        </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT ENGLAND', sub: 'THE THREE LIONS', flag: <FlagENG w={76} />, accent: ENG_NAVY },
    { label: 'COMMENT CONGO', sub: 'THE LEOPARDS', flag: <FlagCD w={76} />, accent: CD_RED },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 140, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: headP }}><Kicker size={30}>The Lions, Or The Leopard?</Kicker></div>
...
function MiniStrip() {
  const prev = ['legend-079-portrait', 'legend-080-portrait', 'legend-081-portrait', 'legend-082-portrait', 'legend-083-portrait'];
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
      ...
      <div style={{ width: 78, height: 104, borderRadius: 9, overflow: 'hidden', border: `2px solid ${ACC}`, boxShadow: `0 10px 28px rgba(0,0,0,0.6), 0 0 26px ${ACC}66` }}>
        <img data-seq alt="" src="assets/legend-084-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      </div>
    </div>
  );
}
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: 90, textAlign: 'center', opacity: teaseP, zIndex: 20 }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f3e0b8', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.7)' }}>THE GUARDIANS OF A KINGDOM</div>
        </div>
      )}
      {lt > 1.6 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 22, opacity: clamp(cardP, 0, 1) }}>
          <div style={{ position: 'relative', transform: `perspective(1500px) rotateY(${tilt}deg) scale(${0.92 + 0.08 * Math.min(cardP, 1)})`, marginTop: -34 }}>
            <img data-seq src="assets/legend-084-portrait.png" alt="" style={{ height: 640, display: 'block', borderRadius: 16, boxShadow: '0 50px 130px rgba(0,0,0,0.85), 0 0 80px rgba(224,162,38,0.5)' }} />
            ...
            <div style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', background: 'rgba(224,162,38,0.95)', color: '#1a1306', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 17, letterSpacing: '0.14em', padding: '5px 16px', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.5)' }}>Nº 084 · ✦✦✦ ULTRA RARE</div>
          </div>
        </div>
      )}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, zIndex: 24 }}>
        <div style={{ textAlign: 'center', opacity: txtP, transform: `translateY(${(1 - txtP) * 16}px)` }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 44, color: ACC, letterSpacing: '0.03em', textShadow: '0 2px 24px rgba(224,162,38,0.5)' }}>THE THREE LIONS · KEEPERS OF THE CROWN</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 19, color: '#ecd9b0', letterSpacing: '0.18em', marginTop: 6 }}>ENGLAND · LEGEND 084</div>
        </div>
...
function PhoneCollect({ start }) {
  ...
  const filled = ['legend-080-portrait', 'legend-081-portrait', 'legend-082-portrait', 'legend-083-portrait'];
        ...
        <div style={{ width: '100%', height: '100%', borderRadius: 30, background: 'radial-gradient(ellipse at 50% 20%, #1a2030 0%, #06080c 70%)', overflow: 'hidden', position: 'relative', padding: '34px 18px 18px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            ...
            <div style={{ height: 104, borderRadius: 6, overflow: 'hidden', border: snapped ? `2px solid ${ACC}` : '1px dashed rgba(224,162,38,0.6)', position: 'relative', boxShadow: flash > 0 ? `0 0 ${20 * flash}px ${ACC}` : 'none' }}>
              {snapped && <img data-seq alt="" src="assets/legend-084-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 92, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: p }}>
        <Kicker size={30} color={ACC}>Claim the Three Lions</Kicker>
