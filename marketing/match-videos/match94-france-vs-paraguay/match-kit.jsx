...
const MV = {
  ...
  curLight: '#2a55b8',
  bel: '#C8102E',          // Belgian red
  belLight: '#ef3a52',
  irn: '#239F40',          // Iran green
  irnLight: '#3ec85f',
  uru: '#3A8DDE',          // Uruguay sky blue (celeste)
  uruLight: '#6db4ee',
...
// IMAGE-SEQUENCE playback. Headless Chromium cannot seek-decode <video> frames
// (it freezes on frame 0 for both screenshot AND canvas drawImage), so every clip
// is pre-extracted to JPGs by extract_frames.mjs and we display the exact frame for
// the current timeline time. Images always render in headless → true full motion.
function VideoSprite({ src, start, dur, fit = 'cover', style = {}, dim = 0 }) {
  const t = useTime();
  const local = t - start;
  if (local < 0 || local >= dur) return null;
  const base = String(src).replace(/^assets\//, '').replace(/\.mp4$/, '');
  const seq = (window.MV_SEQ || {})[base];
  if (!seq || !seq.n) {
    // No extracted sequence → fall back to a single still (never black).
    return <img src={src} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: fit, filter: dim ? `brightness(${1 - dim})` : 'none', ...style }} />;
  }
  const fps = seq.fps || 30;
  const idx = ((Math.floor(local * fps) % seq.n) + seq.n) % seq.n;   // loop the source
  const name = String(idx + 1).padStart(5, '0');
  const url = `assets/seq/${base}/${name}.jpg`;
  return (
    <img
      data-seq=""
      src={url}
...
// ── Cinematic dressing ───────────────────────────────────────────────────────
// Letterbox: DISABLED (rule #19). YouTube is already 16:9, so cinematic black bars
// only waste vertical space and look like a rendering gap. Full-frame 1920x1080.
function Letterbox() { return null; }
...
// France — vertical tricolore: blue / white / red
...
// Senegal — vertical green / yellow / red, green five-point star centred on the yellow
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', display: 'flex', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <div style={{
        ...
        width: 0, height: 0, color: '#00853f', fontSize: h * 0.42, lineHeight: 1,
      }}>
        <span style={{ position: 'absolute', transform: 'translate(-50%,-50%)', fontFamily: SANS, color: '#00853f' }}>★</span>
      </div>
    </div>
  );
}
...
// Iran — horizontal tricolour: green / white / red, a red central emblem on the white band
function FlagIRN({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <span style={{
        position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)',
        color: '#da0000', fontSize: h * 0.26, lineHeight: 1, fontFamily: SANS, fontWeight: 900,
      }}>۞</span>
    </div>
  );
}

// Uruguay — white field with 9 horizontal sky-blue stripes and a golden Sun of May
// in the top-left (hoist) canton.
function FlagURU({ w = 120 }) {
  const h = w * 2 / 3;
  // 9 stripes total (5 blue, 4 white) over the full height.
  const stripes = [];
  for (let i = 0; i < 9; i++) {
    stripes.push(
      <div key={i} style={{ position: 'absolute', left: 0, right: 0, top: `${(i * 100) / 9}%`, height: `${100 / 9}%`, background: i % 2 === 0 ? '#7BB2E0' : '#fff' }} />
    );
  }
  const canton = h * 4 / 9; // top-left white canton spans 4 stripes high
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#fff', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      {stripes}
      {/* white canton covering the top-left quadrant */}
      <div style={{ position: 'absolute', left: 0, top: 0, width: canton, height: canton, background: '#fff' }} />
      {/* golden Sun of May */}
      <span style={{
        position: 'absolute', left: canton / 2, top: canton / 2, transform: 'translate(-50%,-50%)',
        color: '#f5c518', fontSize: canton * 0.78, lineHeight: 1, fontFamily: SANS, fontWeight: 900,
        textShadow: '0 0 4px rgba(180,120,0,0.45)',
      }}>☀</span>
    </div>
  );
}

// Cape Verde — blue field, a white-red-white horizontal band across the lower third,
// and a circle of 10 yellow stars on the left.
function FlagCPV({ w = 120 }) {
  const h = w * 2 / 3;
  const stars = [];
  const cx = w * 0.38, cy = h * 0.5, R = h * 0.30;
  for (let i = 0; i < 10; i++) {
    const a = (i / 10) * Math.PI * 2 - Math.PI / 2;
    stars.push(
      <span key={i} style={{
        position: 'absolute', left: cx + Math.cos(a) * R, top: cy + Math.sin(a) * R,
        transform: 'translate(-50%,-50%)', color: '#f9d616', fontSize: h * 0.14, lineHeight: 1, fontFamily: SANS,
      }}>★</span>
    );
  }
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', background: '#003893', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, top: '58%', height: '7%', background: '#fff' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '65%', height: '12%', background: '#cf2027' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '77%', height: '7%', background: '#fff' }} />
      {stars}
    </div>
  );
}
...
// Match scoreboard chip (top center). Ep41 is UNPLAYED — the 2–1 is OUR PREDICTION,
// so the badge defaults to "OUR PREDICTION", never LIVE/FULL TIME stated as fact.
function ScoreBug({ start, uru = 0, cpv = 0, minute, badge = "OUR PREDICTION" }) {
  const t = useTime();
  const local = t - start;
  if (local < 0) return null;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ ...cell, background: MV.uru }}>URU</div>
        <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{uru} — {cpv}</div>
        <div style={{ ...cell, background: MV.cpv, color: '#fff' }}>CPV</div>
...
function GoalFlash({ at }) {
  ...
  return (<>
    ...
    <div style={{
      ...
      <div style={{
        fontFamily: SANS, fontWeight: 900, fontSize: 230, color: MV.gold, letterSpacing: '0.04em',
        textShadow: `0 0 80px ${MV.gold}aa, 0 10px 40px rgba(0,0,0,0.8)`, WebkitTextStroke: '4px rgba(120,70,0,0.55)',
      }}>GOAL!</div>
    </div>
  </>);
}
