// match-scenes.jsx — Ep80 Netherlands vs Morocco (R32) — PHOTOREAL + NO-REPEAT/NO-LOOP.
// "THE GHOST SHIP vs THE ATLAS LIONS". OUR PREDICTION Morocco 2-1 Netherlands (Depay lead;
// En-Nesyri equalise; Hakimi late winner). Mystic (#21): NETHERLANDS = the Flying Dutchman, the
// cursed ghost ship that can never reach port; MOROCCO = the Atlas Lion -> Legend 080 = The Atlas
// Lion, Guardian of the Mountains.
// #22 photoreal · #23 name<->image SYNC · #24 footer real mini-cards 075-079 + phone collect
// · #25 NO blank frames.

const NL_ORANGE = '#f3701b', NL_WHITE = '#f3f4f7', NL_BLUE = '#1d3a8f';
...
function SunBeat({ start, dur, count = 28 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at ${50 + drift}% 40%, rgba(224,162,38,0.20) 0%, transparent 52%)` }} />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at ${50 - drift}% 64%, rgba(193,39,45,0.18) 0%, transparent 55%)` }} />
      <AmbientParticles start={start} dur={dur} count={count} color="240,200,140" />
    </div>
  );
}

function ScoreBug({ start, ned = 1, mar = 2, note = "HAKIMI 88'", badge = 'OUR PREDICTION' }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 92, left: '50%', zIndex: 30, transform: `translateX(-50%) scale(${inP})`, opacity: clamp(inP, 0, 1), display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 14, boxShadow: '0 10px 36px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
        <div style={{ ...cell, background: NL_ORANGE, color: '#06121a' }}>NED</div>
        <div style={{ ...cell, fontSize: 38, color: ACC }}>{ned} — {mar}</div>
        <div style={{ ...cell, background: MA_RED }}>MAR</div>
...
function SceneColdOpen() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <FS id="nl-dutchman" /><FS id="texture-sea" /><FS id="ma-atlaslion" /><FS id="texture-atlas" />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 150, textAlign: 'center', zIndex: 23, opacity: teaseP }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f0dca6', letterSpacing: '0.22em', textShadow: '0 2px 18px rgba(0,0,0,0.85)' }}>THE GHOST SHIP MEETS THE LION.</div>
        </div>
      )}
      {lt > 19.2 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 26, opacity: titleP, zIndex: 22 }}>
          <Kicker size={32} color={ACC}>The Ghost Ship vs The Atlas Lions</Kicker>
          <TitleReveal text="THE ATLAS LIONS" start={20.0} size={88} color={ACC} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 38 }}>
        <div style={{ opacity: p1, transform: `translateY(${(1 - p1) * -30}px)` }}><Kicker>WorldCup26 Legends · Episode 80 · Last 32</Kicker></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 56, opacity: clamp(p2, 0, 1), transform: `scale(${p2})` }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving><FlagNL w={210} /></Waving><BigTitle size={60} glow={NL_ORANGE}>NETHERLANDS</BigTitle></div>
          <BigTitle size={108} color={ACC}>VS</BigTitle>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}><Waving speed={1.9}><FlagMA w={210} /></Waving><BigTitle size={70} glow={MA_RED}>MOROCCO</BigTitle></div>
        </div>
        <div style={{ opacity: p3, fontFamily: '"Inter",sans-serif', fontWeight: 600, fontSize: 30, color: MV.muted, letterSpacing: '0.10em' }}>WIN, OR SAIL ON INTO THE DARK</div>
      </div>
    </div>
  );
}
    ...
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 120, display: 'flex', justifyContent: 'center', zIndex: 25, opacity: stripP, transform: `translateY(${(1 - stripP) * 30}px)` }}>
        <div style={{ display: 'flex', background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(6px)' }}>
          {[['3x FINALISTS', 'NETHERLANDS'], ['SEMI-FINAL', 'MOROCCO 2022'], ['TOTAL FOOTBALL', 'vs THE LIONS'], ['LAST 32', 'WIN OR GO HOME']].map(([v, l], i) => (
...
function SceneNetherlands() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={44} end={49}><div style={{ position: 'absolute', inset: 0 }}><FS id="nl-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagNL w={64} />} label="ORANJE · NETHERLANDS" accent={NL_ORANGE} /></div></Sprite>
      <Sprite start={49} end={54}><div style={{ position: 'absolute', inset: 0 }}><FS id="nl-tifo" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(2,3,8,0.32)' }} /></div></Sprite>
      <Sprite start={54} end={55.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="nl-attack" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(243,112,27,0.26), transparent 55%)' }} /></div></Sprite>
      <PlayerShowcase clipId="nl-depay" name="MEMPHIS DEPAY" role="THE TALISMAN · 10" accent={NL_ORANGE} start={55.5} end={60.5} />
      <PlayerShowcase clipId="nl-gakpo" name="CODY GAKPO" role="THE POWER · 11" accent={NL_WHITE} start={60.5} end={64.8} />
      <PlayerShowcase clipId="nl-xavisimons" name="XAVI SIMONS" role="THE SPARK · 7" accent={NL_ORANGE} start={64.8} end={68.0} />
      <PlayerShowcase clipId="nl-dejong" name="FRENKIE DE JONG" role="THE CONDUCTOR · 21" accent={NL_WHITE} start={68.0} end={71.2} />
      <PlayerShowcase clipId="nl-vandijk" name="VIRGIL VAN DIJK" role="CAPTAIN · THE WALL · 4" accent={NL_ORANGE} start={71.2} end={79.5} />
    </div>
  );
}
...
function SceneMorocco() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={79.5} end={84.5}><div style={{ position: 'absolute', inset: 0 }}><FS id="ma-crowd" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.30) 0%, transparent 40%, rgba(2,3,8,0.55) 100%)' }} /><TeamBanner flag={<FlagMA w={64} />} label="ATLAS LIONS · MOROCCO" accent={MA_RED} /></div></Sprite>
      ...
      <PlayerShowcase clipId="ma-hakimi" name="ACHRAF HAKIMI" role="THE ROCKET · 2" accent={MA_RED} start={90.6} end={95.0} />
      <PlayerShowcase clipId="ma-ziyech" name="HAKIM ZIYECH" role="THE WIZARD · 7" accent={MA_LIGHT} start={95.0} end={99.0} />
      <PlayerShowcase clipId="ma-ennesyri" name="YOUSSEF EN-NESYRI" role="THE TOWER · 19" accent={MA_RED} start={99.0} end={103.0} />
      <PlayerShowcase clipId="ma-amrabat" name="SOFYAN AMRABAT" role="THE SHIELD · 4" accent={MA_GREEN} start={103.0} end={106.5} />
      <PlayerShowcase clipId="ma-brahim" name="BRAHIM DÍAZ" role="THE KEY · 10" accent={MA_RED} start={106.5} end={113.0} />
    </div>
  );
}
...
function SceneRiddle() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <NightField o={0.5} />
      <Sprite start={113} end={118}><div style={{ position: 'absolute', inset: 0 }}><FS id="duel-mid" /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.24)' }} /><div style={{ position: 'absolute', left: 0, right: 0, bottom: 70, textAlign: 'center', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 54, color: '#fff', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>THE BEAUTIFUL GAME vs THE WALL</div></div></Sprite>
      ...
      <Sprite start={123} end={132}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={123} dur={9} count={24} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 56, color: ACC, letterSpacing: '0.06em', textAlign: 'center', textShadow: '0 4px 26px rgba(0,0,0,0.9)' }}>DO THE ATLAS LIONS<br />ROAR ONCE MORE?</div></div></div></Sprite>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,3,6,0.62)', opacity: p }}>
      <div style={{ width: 1010, borderRadius: 28, overflow: 'hidden', transform: `scale(${0.92 + 0.08 * p})`, boxShadow: '0 36px 130px rgba(0,0,0,0.8)', position: 'relative', border: '2px solid rgba(224,162,38,0.55)' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(105deg, ${NL_ORANGE}33 0%, #070b14 38%, #070b14 62%, ${MA_RED}33 100%)` }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, background: `linear-gradient(100deg, ${NL_ORANGE} 0%, ${ACC} 50%, ${MA_RED} 100%)`, padding: '13px 0', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '36px 70px 28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 46 }}>
            <Badge flag={<FlagNL w={104} />} name="NETHERLANDS" accent={NL_ORANGE} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 116, color: '#fff', lineHeight: 1 }}>
                <span style={{ textShadow: `0 0 36px ${NL_ORANGE}cc` }}>1</span>
                <span style={{ color: ACC, fontSize: 58, transform: 'translateY(-6px)' }}>—</span>
                <span style={{ textShadow: `0 0 36px ${MA_RED}cc` }}>2</span>
              </div>
            ...
            <Badge flag={<FlagMA w={104} />} name="MOROCCO" accent={MA_RED} />
          </div>
          <div style={{ textAlign: 'center', marginTop: 16, fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 32, color: ACC, letterSpacing: '0.05em', textShadow: `0 0 26px ${ACC}66` }}>THE ATLAS LIONS ROAR · 2–1</div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 16, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 21, color: '#f3e6c4', background: 'rgba(224,162,38,0.16)', border: '1px solid rgba(224,162,38,0.45)', borderRadius: 999, padding: '7px 18px' }}>🦁 HAKIMI — THE LATE WINNER</span>
          </div>
...
function SceneDrama() {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Backdrop />
      <Sprite start={132} end={137}><div style={{ position: 'absolute', inset: 0 }}><FS id="nl-press" /><ChanceTag start={132.5} end={137} text="NETHERLANDS COME FLOWING" sub="DEPAY PROWLING · GAKPO FLYING" accent={NL_ORANGE} /></div></Sprite>
      <Sprite start={137} end={142}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={137} dur={5} count={20} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>AN HOUR<br />OF ORANGE</div></div></div></Sprite>
      <Sprite start={142} end={147}><div style={{ position: 'absolute', inset: 0 }}><FS id="bono-save" /><ChanceTag start={142.5} end={147} text="BONO — A WALL" sub="MOROCCO HOLD" accent={MA_GREEN} /></div></Sprite>
      <Sprite start={147} end={152.21}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={147} dur={5.2} count={20} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 70, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>THE PRESSURE<br />BUILDS</div></div></div></Sprite>
      <Sprite start={152.21} end={159}><div style={{ position: 'absolute', inset: 0 }}><FS id="nl-goal" /><GoalFlash at={153.6} /><ChanceTag start={154} end={159} text="DEPAY — NETHERLANDS LEAD!" sub="THE GHOST SHIP SAILS ON" accent={NL_ORANGE} /></div></Sprite>
      <Sprite start={159} end={164.37}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={159} dur={5.4} count={20} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 64, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>BUT THE LIONS<br />DO NOT FOLD</div></div></div></Sprite>
      <Sprite start={164.37} end={172}><div style={{ position: 'absolute', inset: 0 }}><FS id="ma-surge" /><ChanceTag start={165} end={172} text="THEY ANSWER BACK" sub="A WHOLE CONTINENT RISES" accent={MA_RED} /></div></Sprite>
      <Sprite start={172} end={178.46}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={172} dur={6.46} count={22} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 70, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>THE LION<div style={{ fontSize: 28, fontWeight: 800, color: ACC, letterSpacing: '0.16em', marginTop: 14 }}>STIRS</div></div></div></div></Sprite>
      <Sprite start={178.46} end={185}><div style={{ position: 'absolute', inset: 0 }}><FS id="ma-goal-1" /><GoalFlash at={179.9} /><ChanceTag start={180.3} end={185} text="EN-NESYRI — ONE-ONE!" sub="HE CLIMBS ABOVE THEM ALL" accent={MA_RED} /></div></Sprite>
      <Sprite start={185} end={189.58}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={185} dur={4.58} count={18} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 72, color: ACC, textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>AND THEN —<br />THE MAGIC</div></div></div></Sprite>
      <Sprite start={189.58} end={203.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="ma-goal-2" /><GoalFlash at={191.0} /><ChanceTag start={191.4} end={196} text="HAKIMI — 2-1 MOROCCO!" sub="ANOTHER GIANT FALLS" accent={ACC} /><ScoreBug start={196.3} ned={1} mar={2} note="HAKIMI 88'" badge="OUR PREDICTION" /><PredictionCard start={196.9} /></div></Sprite>
...
function SceneVerdict() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Sprite start={203.32} end={208.32}><div style={{ position: 'absolute', inset: 0 }}><FS id="vd-handshake" /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(2,3,8,0.25) 0%, transparent 45%, rgba(2,3,8,0.55) 100%)' }} /><div style={{ position: 'absolute', left: 0, right: 0, top: 120, textAlign: 'center', zIndex: 24 }}><Kicker size={28} color={ACC}>The Lions Roar Again</Kicker></div></div></Sprite>
      <Sprite start={208.32} end={213.32}><div style={{ position: 'absolute', inset: 0 }}><SunBeat start={208.32} dur={5} count={20} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 60, color: '#fff', textAlign: 'center', textShadow: '0 4px 30px rgba(0,0,0,0.9)' }}>BELIEF CAN TOPPLE<br />ANYTHING</div></div></div></Sprite>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 25 }}>
            <div style={{ background: MV.panel, border: `1px solid ${MV.line}`, borderRadius: 24, padding: '48px 80px', minWidth: 900, backdropFilter: 'blur(6px)', opacity: panelP, transform: `translateY(${(1 - panelP) * 24}px)` }}>
              ...
              <div style={{ marginTop: 24 }}>
                <StatLine start={219.5} delay={0.0} label="NETHERLANDS" value="ORANJE" accent={NL_ORANGE} />
                <StatLine start={219.5} delay={0.25} label="MOROCCO" value="THE ATLAS LIONS" accent={MA_RED} />
                <StatLine start={219.5} delay={0.5} label="OUR PREDICTION" value="NED 1 — 2 MAR" accent="#fff" />
                <StatLine start={219.5} delay={0.75} label="88' HAKIMI" value="THE LATE WINNER" accent={ACC} />
              </div>
            </div>
          </div>
        </div>
...
function SceneEngage() {
  ...
  const cards = [
    { label: 'COMMENT NETHERLANDS', sub: 'ORANJE', flag: <FlagNL w={76} />, accent: NL_ORANGE },
    { label: 'COMMENT MOROCCO', sub: 'THE ATLAS LIONS', flag: <FlagMA w={76} />, accent: MA_RED },
  ];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 140, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: headP }}><Kicker size={30}>The Ghost Ship, Or The Lion?</Kicker></div>
...
function MiniStrip() {
  const prev = ['legend-075-portrait', 'legend-076-portrait', 'legend-077-portrait', 'legend-078-portrait', 'legend-079-portrait'];
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12 }}>
      ...
      <div style={{ width: 78, height: 104, borderRadius: 9, overflow: 'hidden', border: `2px solid ${ACC}`, boxShadow: `0 10px 28px rgba(0,0,0,0.6), 0 0 26px ${ACC}66` }}>
        <img data-seq alt="" src="assets/legend-080-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      </div>
    </div>
  );
}
...
function SceneMystery() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 46%, #241810 0%, #170f0a 50%, #0a0706 100%)' }} />
      ...
      {teaseP > 0.01 && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: 90, textAlign: 'center', opacity: teaseP, zIndex: 20 }}>
          <div style={{ display: 'inline-block', fontFamily: '"Inter",sans-serif', fontWeight: 800, fontSize: 30, color: '#f3e0b8', letterSpacing: '0.24em', textShadow: '0 2px 18px rgba(0,0,0,0.7)' }}>THE ONE WHO BOWS TO NO GIANT</div>
        </div>
      )}
      {lt > 1.6 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 22, opacity: clamp(cardP, 0, 1) }}>
          <div style={{ position: 'relative', transform: `perspective(1500px) rotateY(${tilt}deg) scale(${0.92 + 0.08 * Math.min(cardP, 1)})`, marginTop: -34 }}>
            <img data-seq src="assets/legend-080-portrait.png" alt="" style={{ height: 640, display: 'block', borderRadius: 16, boxShadow: '0 50px 130px rgba(0,0,0,0.85), 0 0 80px rgba(224,162,38,0.5)' }} />
            ...
            <div style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', background: 'rgba(224,162,38,0.95)', color: '#1a1306', fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 17, letterSpacing: '0.14em', padding: '5px 16px', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.5)' }}>Nº 080 · ✦✦✦ ULTRA RARE</div>
          </div>
        </div>
      )}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, zIndex: 24 }}>
        <div style={{ textAlign: 'center', opacity: txtP, transform: `translateY(${(1 - txtP) * 16}px)` }}>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 900, fontSize: 46, color: ACC, letterSpacing: '0.04em', textShadow: '0 2px 24px rgba(224,162,38,0.5)' }}>THE ATLAS LION · GUARDIAN OF THE MOUNTAINS</div>
          <div style={{ fontFamily: '"Inter",sans-serif', fontWeight: 700, fontSize: 19, color: '#ecd9b0', letterSpacing: '0.18em', marginTop: 6 }}>MOROCCO · LEGEND 080</div>
        </div>
...
function PhoneCollect({ start }) {
  ...
  const filled = ['legend-076-portrait', 'legend-077-portrait', 'legend-078-portrait', 'legend-079-portrait'];
        ...
        <div style={{ width: '100%', height: '100%', borderRadius: 30, background: 'radial-gradient(ellipse at 50% 20%, #2a2010 0%, #0c0a06 70%)', overflow: 'hidden', position: 'relative', padding: '34px 18px 18px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            ...
            <div style={{ height: 104, borderRadius: 6, overflow: 'hidden', border: snapped ? `2px solid ${ACC}` : '1px dashed rgba(224,162,38,0.6)', position: 'relative', boxShadow: flash > 0 ? `0 0 ${20 * flash}px ${ACC}` : 'none' }}>
              {snapped && <img data-seq alt="" src="assets/legend-080-portrait.png" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 92, left: 0, right: 0, textAlign: 'center', zIndex: 25, opacity: p }}>
        <Kicker size={30} color={ACC}>Claim The Atlas Lion</Kicker>
