// match-kit.jsx — shared primitives for the Match 23 video (loads after animations.jsx)
// Dark cinematic broadcast theme. Episode 23 is IMAGE-BASED: Ken-Burns motion on
...
const MV = {
  ...
  gold: '#ffd24a',
  goldDeep: '#c9942e',
  // Germany: red + gold (team A = aut/autSoft slots)
  aut: '#DD0000',
  autSoft: '#FFCE00',
  // Ivory Coast: orange + green (team B = jor/jorGreen slots)
  jor: '#F77F00',
  jorGreen: '#009E60',
  jorRed: '#F77F00',
  green: '#009E60',
...
function FlagUSA({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.04, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      {[...Array(13)].map((_, i) => (<div key={i} style={{ position: 'absolute', left: 0, right: 0, top: `${(i / 13) * 100}%`, height: `${100 / 13}%`, background: i % 2 ? '#fff' : '#b22234' }} />))}
...
function FlagAUS({ w = 120 }) {
  const h = w * 0.5;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.04, position: 'relative', overflow: 'hidden', background: '#00247d', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', left: w * 0.06, bottom: h * 0.12, color: '#fff', fontSize: h * 0.26, lineHeight: 1 }}>✦</div>
      <div style={{ position: 'absolute', right: w * 0.1, top: h * 0.16, color: '#fff', fontSize: h * 0.16 }}>✦</div>
      <div style={{ position: 'absolute', right: w * 0.22, top: h * 0.42, color: '#fff', fontSize: h * 0.13 }}>✦</div>
      <div style={{ position: 'absolute', right: w * 0.06, top: h * 0.5, color: '#fff', fontSize: h * 0.13 }}>✦</div>
      <div style={{ position: 'absolute', right: w * 0.16, bottom: h * 0.12, color: '#fff', fontSize: h * 0.13 }}>✦</div>
    </div>
  );
}

// Jordan: three horizontal bands black / white / green, with a red hoist triangle
// carrying a white seven-pointed star (abstracted to a clean star token).
function FlagJOR({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.05, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', left: 0, right: 0, top: 0, height: '33.34%', background: '#000' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '33.33%', height: '33.34%', background: '#fff' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '66.66%', height: '33.34%', background: '#007a3d' }} />
      {/* red hoist triangle */}
...
function FlagNED({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.04, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', left: 0, right: 0, top: 0, height: '33.34%', background: '#AE1C28' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '33.33%', height: '33.34%', background: '#fff' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '66.66%', height: '33.34%', background: '#21468B' }} />
    </div>
  );
}
// Sweden: blue field with an off-centre yellow Nordic cross.
function FlagSWE({ w = 120 }) {
  const h = w * 0.625;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.04, position: 'relative', overflow: 'hidden', background: '#006AA7', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', top: 0, bottom: 0, left: w * 0.30, width: h * 0.2, background: '#FECC00' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '40%', height: h * 0.2, background: '#FECC00' }} />
    </div>
  );
}
...
function FlagGER({ w = 120 }) {
  ...
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.04, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', left: 0, right: 0, top: 0, height: '33.34%', background: '#000' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '33.33%', height: '33.34%', background: '#DD0000' }} />
      <div style={{ position: 'absolute', left: 0, right: 0, top: '66.66%', height: '33.34%', background: '#FFCE00' }} />
    </div>
  );
}
// Ivory Coast: three vertical bands orange / white / green.
function FlagCIV({ w = 120 }) {
  const h = w * 2 / 3;
  return (
    <div style={{ width: w, height: h, borderRadius: w * 0.04, position: 'relative', overflow: 'hidden', boxShadow: '0 6px 18px rgba(0,0,0,0.45)' }}>
      <div style={{ position: 'absolute', top: 0, bottom: 0, left: 0, width: '33.34%', background: '#F77F00' }} />
      <div style={{ position: 'absolute', top: 0, bottom: 0, left: '33.33%', width: '33.34%', background: '#fff' }} />
      <div style={{ position: 'absolute', top: 0, bottom: 0, left: '66.66%', width: '33.34%', background: '#009E60' }} />
    </div>
  );
}
...
// Match scoreboard chip (top center). AUT — JOR.
function ScoreBug({ start, aut = 0, jor = 0, minute }) {
  const t = useTime();
  const local = t - start;
  if (local < 0) return null;
  ...
  return (
    <div style={{
      ...
      <div style={{ ...cell, background: MV.aut, color: '#fff' }}>GER</div>
      <div style={{ ...cell, fontSize: 38, color: MV.gold }}>{aut} — {jor}</div>
      <div style={{ ...cell, background: MV.jor, color: '#fff' }}>CIV</div>
