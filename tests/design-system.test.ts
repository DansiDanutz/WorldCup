...
describe("WorldCup design system integration", () => {
  ...
  it("turns Legends episode assets into collectible cards unlocked from YouTube", () => {
    ...
    assert.match(legendCardCollection, /selectEnglishStoryVoice\(window\.speechSynthesis\.getVoices\(\)\)/);
    assert.match(legendCardCollection, /utterance\.lang = storyVoice\?\.lang \?\? storyVoiceLanguage/);
    assert.match(legendCardCollection, /utterance\.voice = storyVoice/);
    assert.match(legendCardCollection, /Stories play in English with Brian when available/);
