...
describe("WorldCup design system integration", () => {
  ...
  it("lets dense dashboard cards minimize and maximize without losing full data access", () => {
    ...
    assert.match(dashboard, /Show full rules/);
    assert.match(dashboard, /Show all \$\{matches\.length\} matches/);
    assert.match(dashboard, /Show all \$\{publicLeaderboard\.length\}/);
    assert.doesNotMatch(dashboard, /panel-size-toggle/);
    assert.doesNotMatch(dashboard, /panel-count-chip/);
    assert.doesNotMatch(dashboard, /Maximize .* card/);
    assert.doesNotMatch(dashboard, /Minimize .* card/);
    ...
    assert.match(globalsCss, /\.card-disclosure\s*{/);
    assert.match(globalsCss, /\.card-disclosure__button\s*{/);
  ...
  it("keeps the three-team pick workflow guided on mobile", () => {
    ...
    assert.match(dashboard, /Show all \$\{filteredTeams\.length\} teams/);
    assert.match(dashboard, /Show first \$\{compactTeamCount\} teams/);
    ...
    assert.match(dashboard, /className="card-disclosure card-disclosure--teams"/);
    ...
    assert.match(globalsCss, /\.card-disclosure--teams\s*{/);
