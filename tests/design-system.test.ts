...
describe("WorldCup design system integration", () => {
  ...
  it("lets dense dashboard cards minimize and maximize without losing full data access", () => {
    ...
    assert.match(dashboard, /compactText=\{`First \$\{Math\.min\(compactMatchCount, matches\.length\)\} of \$\{matches\.length\} matches`\}/);
    assert.match(dashboard, /expandedText=\{`All \$\{matches\.length\} matches`\}/);
  ...
  it("turns Legends episode assets into collectible cards unlocked from YouTube", () => {
    ...
    assert.match(legendCardCollection, /new SpeechSynthesisUtterance\(createLegendCardVoiceText\(card\)\)/);
    assert.match(legendCardCollection, /Brian voice enabled\. Stories use ElevenLabs Brian/);
    ...
    assert.match(legendCardVoiceRoute, /X-WorldCup26-Voice/);
    assert.match(storyVoice, /storyVoiceLanguage = "en-GB"/);
    assert.match(storyVoice, /elevenLabsBrianVoiceName = "Brian"/);
    assert.match(storyVoice, /selectElevenLabsBrianVoice/);
