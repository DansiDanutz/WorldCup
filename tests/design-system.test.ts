...
const storyVoice = readFileSync("src/lib/story-voice.ts", "utf8");
...
const legendCardStoryUnlockMigration = readFileSync(
  "supabase/migrations/20260628090000_worldcup_legend_card_story_unlocks.sql",
  "utf8",
);
...
describe("WorldCup design system integration", () => {
  ...
  it("makes the collectible card album the homepage entry point", () => {
    ...
    assert.match(legendCardsPromo, /bonusCount/);
    assert.match(legendCardsPromoCss, /\.legend-promo__stats\s*{[\s\S]*?grid-template-columns:\s*repeat\(4,\s*minmax\(0,\s*1fr\)\);/);
    assert.match(legendCardsPromoCss, /\.legend-promo__steps\s*{[\s\S]*?display:\s*grid;/);
    assert.match(legendCardsPromoCss, /\.legend-promo__card\s*{[\s\S]*?border-radius:\s*8px;/);
    assert.match(legendCardsPromoCss, /@media \(max-width:\s*760px\)/);
  });
  ...
  it("lets dense dashboard cards minimize and maximize without losing full data access", () => {
    ...
    assert.match(dashboard, /matchScheduleExpanded, setMatchScheduleExpanded\] = useState\(true\)/);
    assert.match(dashboard, /matchDateFilter, setMatchDateFilter/);
    assert.match(dashboard, /matchDateOptions = useMemo/);
    assert.match(dashboard, /filteredMatches = useMemo/);
    ...
    assert.match(dashboard, /import \{ CardViewControl \} from "@\/components\/card-view-control";/);
    assert.equal(dashboard.match(/<CardViewControl/g)?.length, 4);
    assert.match(cardViewControl, /className=\{`card-view-control \$\{expanded \? "is-expanded" : "is-compact"\}`\}/);
    assert.match(cardViewControl, /expanded \? "Full view" : "Compact view"/);
    assert.match(cardViewControl, /aria-label=\{`\$\{expanded \? "Minimize" : "Maximize"\} \$\{label\}`\}/);
    assert.match(cardViewControl, /aria-haspopup=\{hasPopup\}/);
    assert.match(cardViewControl, /\{expanded \? "Minimize" : "Maximize"\}/);
    assert.match(dashboard, /compactText="Quick rules view"/);
    assert.match(dashboard, /expandedText="Full rules guide"/);
    assert.match(dashboard, /compactText=\{`\$\{Math\.min\(compactMatchCount, filteredMatches\.length\)\} of \$\{filteredMatches\.length\} matches visible`\}/);
    assert.match(dashboard, /expandedText=\{`All \$\{filteredMatches\.length\} matches visible`\}/);
    assert.match(dashboard, /compactText=\{`Top \$\{Math\.min\(compactLeaderboardCount, publicLeaderboard\.length\)\} of \$\{publicLeaderboard\.length\} entries`\}/);
    assert.match(dashboard, /expandedText=\{`All \$\{publicLeaderboard\.length\} entries`\}/);
    assert.doesNotMatch(dashboard, /panel-size-toggle/);
    assert.doesNotMatch(dashboard, /panel-count-chip/);
    ...
    assert.match(dashboard, /visibleMatches = matchScheduleExpanded[\s\S]*?\? filteredMatches[\s\S]*?: filteredMatches\.slice\(0, compactMatchCount\)/);
    assert.doesNotMatch(dashboard, /First 24 matches shown/);
    assert.match(dashboard, /\{visibleMatches\.length\} of \{filteredMatches\.length\} matches shown/);
    assert.match(dashboard, /id="dashboard-match-date-filter"/);
    assert.match(dashboard, /formatMatchDateLabel\(option\.date\)/);
    assert.match(dashboard, /getMatchCalendarDate\(match\.kickoff_at\)/);
    assert.match(dashboard, /Scores and points update automatically after full-time/);
    assert.match(globalsCss, /\.card-view-control\s*{/);
    assert.match(globalsCss, /\.card-view-control__button\s*{/);
    assert.match(globalsCss, /#leaderboard \.card-view-control,[\s\S]*?\.rules-panel \.card-view-control/);
    assert.doesNotMatch(globalsCss, /\.panel-size-toggle\s*{/);
    assert.doesNotMatch(globalsCss, /\.panel-count-chip\s*{/);
  ...
  it("turns Legends episode assets into collectible cards unlocked from YouTube", () => {
    ...
    assert.match(youtubeLegendEpisodes, /bSFDrewxg9s/);
    ...
    assert.match(youtubeLegendEpisodes, /qND33AsNhr0/);
    assert.match(youtubeLegendEpisodes, /DJX6iFxc_8g/);
    assert.match(youtubeLegendEpisodes, /Uqmr0LU-agU/);
    assert.match(youtubeLegendEpisodes, /4hufXmyJzn0/);
    assert.match(youtubeLegendEpisodes, /cbbX6EJ6S_w/);
    assert.match(youtubeLegendEpisodes, /GzGpgwLR5C8/);
    assert.match(youtubeLegendEpisodes, /MuGCI3AI7Rs/);
    assert.match(youtubeLegendEpisodes, /GD0n8FvInKg/);
    assert.match(youtubeLegendEpisodes, /wzCu-vitgeM/);
    assert.match(youtubeLegendEpisodes, /rcDLZV_hhWs/);
    assert.match(youtubeLegendEpisodes, /w1GZnvQgw_0/);
    assert.match(youtubeLegendEpisodes, /04MKt0r5UXw/);
    assert.match(youtubeLegendEpisodes, /QXIKEV5qxYo/);
    assert.match(youtubeLegendEpisodes, /qP2qU-JAr6A/);
    assert.match(youtubeLegendEpisodes, /BrxftWhayzU/);
    ...
    assert.doesNotMatch(legendCardRegistry, /supporterCards/);
    assert.doesNotMatch(legendCardRegistry, /createSupporterCard/);
    assert.doesNotMatch(legendCardRegistry, /kind:\s*"supporter-card"/);
    ...
    assert.match(legendCardRegistry, /isSeriesVideo/);
    assert.match(legendCardRegistry, /channelSeriesCards/);
    assert.match(legendCardRegistry, /didYouKnowShortCards/);
    assert.match(legendCardRegistry, /kind:\s*"did-you-know-short"/);
    assert.match(legendCardRegistry, /youtube:\s*short\.youtube/);
    assert.match(legendCardRegistry, /alternateYoutube/);
    ...
    assert.doesNotMatch(legendCards, /getSupporterCardImagePath/);
    ...
    assert.match(legendCardCollection, /worldcup_legend_listened_cards/);
    assert.match(legendCardCollection, /worldcup_legend_pulse_reads/);
    assert.match(legendCardCollection, /worldcup_legend_pulse_preview/);
    assert.match(legendCardCollection, /worldcup_legend_notifications/);
    assert.match(legendCardCollection, /pulsePreviewDurationMs = 60_000/);
    assert.match(legendCardCollection, /unlockableCardIds/);
    assert.match(legendCardCollection, /const legendPulseItems: LegendPulseItem\[\]/);
    ...
    assert.doesNotMatch(legendCardCollection, /\| "wallpapers"/);
    assert.doesNotMatch(legendCardCollection, /\| "supporters"/);
    assert.match(legendCardCollection, /\| "legends"/);
    ...
    assert.doesNotMatch(legendCardCollection, /Supporters/);
    assert.match(legendCardCollection, /Did You Know Legends/);
    assert.match(legendCardCollection, /legend-filter-button--legends/);
    assert.match(legendCardCollection, /Need story/);
    ...
    assert.doesNotMatch(legendCardCollection, /supporterCount/);
    assert.match(legendCardCollection, /didYouKnowCount/);
    ...
    assert.match(legendCardCollection, /!unlockedIds\.has\(card\.id\) && !listenedIds\.has\(card\.id\)/);
    assert.match(
      legendCardCollection,
      /card\.youtube && !unlockedIds\.has\(card\.id\) && listenedIds\.has\(card\.id\) && !watchedIds\.has\(card\.id\)/,
    );
    assert.match(legendCardCollection, /function canCollectLegendCard/);
    ...
    assert.match(legendCardCollection, /readyFocusCard \?\?[\s\S]*?youtubeFocusCard \?\?[\s\S]*?storyFocusCard/);
    assert.match(legendCardCollection, /const focusCardAction = focusCard/);
    assert.match(legendCardCollection, /Ready to collect/);
    assert.match(legendCardCollection, /Next best action/);
    assert.match(legendCardCollection, /YouTube next/);
    ...
    assert.match(legendCardCollection, /const watchProofStorageKey = "worldcup_legend_youtube_watch_proofs"/);
    assert.match(legendCardCollection, /const didYouKnowWatchMinimumMs = 20_000/);
    assert.match(legendCardCollection, /const storyWatchMinimumMs = 45_000/);
    assert.match(legendCardCollection, /function createWatchProof\(card: LegendCard/);
    assert.match(legendCardCollection, /function getWatchSecondsRemaining/);
    assert.match(legendCardCollection, /const markCardWatchReady = useCallback/);
    assert.match(legendCardCollection, /const syncReadyWatchProofs = useCallback/);
    assert.match(legendCardCollection, /function startCardWatchProof\(card: LegendCard\)/);
    assert.match(legendCardCollection, /Keep the YouTube story open/);
    assert.match(legendCardCollection, /const hasListenedStory = listenedIds\.has\(card\.id\)/);
    assert.match(legendCardCollection, /const watchSecondsRemaining = getWatchSecondsRemaining\(watchProof, watchTimerTick\)/);
    assert.match(legendCardCollection, /const isWatchPending = Boolean\(card\.youtube && !hasWatchedEpisode && watchSecondsRemaining > 0\)/);
    ...
    assert.match(legendCardCollection, /card\.youtube[\s\S]*?hasWatchedEpisode[\s\S]*?isWatchPending[\s\S]*?hasListenedStory/);
    assert.match(legendCardCollection, /card\.youtube \? "YouTube" : "Own story"/);
    assert.match(legendCardCollection, /isUnlocked \? "is-done" : canUnlock \? "is-next" : ""/);
    assert.match(globalsCss, /\.legend-card__watch-note\s*{/);
    ...
    assert.match(legendCardCollection, /preview unlocked for 60 seconds/);
    assert.match(legendCardCollection, /id="news" className="legend-pulse"/);
    assert.match(legendCardCollection, /News \/ Pulse/);
    assert.match(legendCardCollection, /Read the Pulse, preview a card/);
    assert.match(legendCardCollection, /Short daily story drops open a temporary 60-second card preview/);
    assert.match(legendCardCollection, /className="legend-pulse__feed"/);
    assert.match(legendCardCollection, /legend-notification-button/);
    assert.match(legendCardCollection, /Sign in for saved cards/);
    assert.match(legendCardCollection, /Watch to collect/);
    assert.match(legendCardCollection, /Read for \${pulsePreviewDurationMs \/ 1000}s preview/);
    assert.match(legendCardCollection, /id="collector-quest" className="legend-quest"/);
    assert.match(legendCardCollection, /Today&apos;s Collector Quest/);
    assert.match(legendCardCollection, /Finish the card loop/);
    assert.match(legendCardCollection, /Read the Pulse, listen to the story, open the YouTube episode, then collect the card/);
    ...
    assert.match(legendCardCollection, /legend-feature-card__guidance/);
    ...
    assert.match(legendCardCollection, /<dt>Legends<\/dt>/);
    ...
    assert.match(globalsCss, /\.legend-quest__grid\s*{[\s\S]*?grid-template-columns:\s*minmax\(220px,\s*0\.72fr\)\s*minmax\(0,\s*1\.28fr\);/);
    assert.match(globalsCss, /\.legend-quest-step\s*{[\s\S]*?grid-template-columns:\s*auto minmax\(0,\s*1fr\) auto;/);
    ...
    assert.match(globalsCss, /\.legend-pulse__preview\.is-open\s*{[\s\S]*?border-color:\s*rgba\(255,\s*207,\s*102,\s*0\.7\);/);
    assert.match(globalsCss, /\.legend-pulse-item\.is-active button\s*{[\s\S]*?rgba\(255,\s*207,\s*102,\s*0\.12\)/);
    assert.match(globalsCss, /\.legend-card\.is-previewing \.legend-card__image img\s*{[\s\S]*?filter:\s*none;/);
    assert.match(globalsCss, /\.legend-card\.is-previewing \.legend-card__lock\s*{[\s\S]*?text-transform:\s*none;/);
    ...
    assert.match(globalsCss, /\.legend-feature-card__image\s*{[\s\S]*?width:\s*100%;[\s\S]*?min-width:\s*0;/);
    assert.match(globalsCss, /@media \(min-width:\s*641px\)\s*{[\s\S]*?\.legend-feature-card__image\s*{[\s\S]*?aspect-ratio:\s*auto;/);
    assert.match(globalsCss, /\.legend-feature-card\.is-locked \.legend-feature-card__image img\s*{[\s\S]*?filter:\s*blur\(5px\)/);
    assert.match(globalsCss, /\.legend-feature-card__guidance\s*{[\s\S]*?max-width:\s*640px;/);
    assert.match(globalsCss, /\.legend-feature-card__actions--guided\s*{[\s\S]*?grid-template-columns:\s*minmax\(0,\s*1\.25fr\)\s*minmax\(0,\s*0\.75fr\);/);
    assert.match(globalsCss, /\.legend-feature-card__primary-action\s*{[\s\S]*?min-height:\s*44px;/);
    ...
    assert.match(globalsCss, /@media \(max-width:\s*640px\)\s*{[\s\S]*?\.legend-pulse__preview,[\s\S]*?\.legend-pulse-item\s*{[\s\S]*?grid-template-columns:\s*minmax\(0,\s*1fr\);/);
    ...
    assert.match(legendCardCollection, /\/api\/legend-cards\/voice/);
    assert.match(legendCardCollection, /ElevenLabs Brian/);
    assert.match(legendCardCollection, /NEXT_PUBLIC_ALLOW_BROWSER_STORY_VOICE_FALLBACK/);
    assert.match(legendCardCollection, /allowBrowserStoryVoiceFallback && playBrowserBrianFallback/);
    assert.match(legendCardCollection, /selectBrowserBrianStoryVoice\(window\.speechSynthesis\.getVoices\(\)\)/);
    assert.doesNotMatch(legendCardCollection, /selectEnglishStoryVoice\(window\.speechSynthesis\.getVoices\(\)\)/);
    assert.match(legendCardVoiceRoute, /ELEVENLABS_API_KEY/);
    assert.match(legendCardVoiceRoute, /ELEVENLABS_BRIAN_VOICE_ID/);
    assert.match(legendCardVoiceRoute, /ELEVENLABS_VOICE_ID/);
    assert.match(legendCardVoiceRoute, /selectElevenLabsBrianVoice/);
    assert.match(legendCardVoiceRoute, /text-to-speech/);
    assert.match(storyVoice, /storyVoiceLanguage = "en-GB"/);
    assert.match(storyVoice, /elevenLabsBrianVoiceName = "Brian"/);
    ...
    assert.match(dashboard, /<strong>Collect Cards<\/strong>/);
    assert.match(dashboard, /<small>Main journey<\/small>/);
    assert.match(dashboard, /<strong>\{showPickWorkflow \? "Pick Teams" : "Account"\}<\/strong>/);
    assert.match(dashboard, /<small>\{showPickWorkflow \? "Game entry" : "Your entry"\}<\/small>/);
    assert.match(dashboard, /News & cards/);
    assert.match(dashboard, /hash: "collector-quest"/);
    assert.match(dashboard, /href="\/predictions#collector-quest"/);
    assert.match(dashboard, /Start quest/);
    assert.match(dashboard, /Read, listen, open YouTube, and finish today&apos;s card quest/);
    ...
    assert.doesNotMatch(legendCardCollection, /Phone portrait cards/);
    assert.doesNotMatch(legendCardCollection, /1080 x 1920 portraits/);
    assert.doesNotMatch(legendCardCollection, /Browse wallpapers/);
    assert.doesNotMatch(legendCardCollection, /getSupporterWallpaperHref/);
    assert.doesNotMatch(legendCardCollection, /getSupporterWallpaperDownloadHref/);
    assert.doesNotMatch(legendCardCollection, /getWallpaperGalleryCards/);
    assert.doesNotMatch(legendCardCollection, /cardFilter === "wallpapers"/);
    assert.doesNotMatch(legendCardCollection, /chooseCardFilter\("wallpapers"\)/);
    assert.doesNotMatch(legendCardCollection, /wallpapers:\s*wallpaperCount/);
    assert.doesNotMatch(legendCardCollection, /unlockedWallpaperHrefs/);
    assert.doesNotMatch(legendCardCollection, /unlockedWallpaperCount/);
    ...
    assert.doesNotMatch(legendCardCollection, /legend-card--supporter/);
    assert.match(legendCardCollection, /legend-card--wide-asset/);
    assert.doesNotMatch(legendCardCollection, /canDownloadWallpaper/);
    assert.doesNotMatch(legendCardCollection, /unlockedWallpaperHrefs\.has\(downloadHref\)/);
    assert.doesNotMatch(legendCardCollection, /Download HD/);
    assert.doesNotMatch(legendCardCollection, /Collect one to download/);
    assert.doesNotMatch(legendCardCollection, /\/api\/supporter-cards\/download\/\$\{encodeURIComponent\(fileName\)\}/);
    assert.doesNotMatch(legendCardCollection, /download=\{downloadFileName\}/);
    assert.match(globalsCss, /\.legend-card__watch,\s*[\s\S]*?\.legend-card__disabled\s*{[\s\S]*?width:\s*100%;/);
  ...
  it("persists collected Legend cards as owner-readable account data", () => {
    ...
    assert.match(legendCardsRoute, /event === "youtube_opened" \|\| event === "pulse_read"/);
    assert.match(legendCardsRoute, /unlock_source:\s*card\.youtube \? "youtube" : "story"/);
    assert.match(legendCardStoryUnlockMigration, /unlock_source in \('youtube', 'story'\)/);
  ...
  it("adds a smart browse hub so users can jump between grouped app cards", () => {
    ...
    assert.match(dashboard, /Start, collect, track, and check fixtures from one place\./);
    ...
    assert.equal(dashboard.match(/className="browse-tile(?:\s|")/g)?.length, 9);
    assert.match(dashboard, /browse-tile--cards/);
    assert.match(dashboard, /Main journey: listen, watch, collect/);
    assert.match(dashboard, /Exact episode artefacts to collect/);
    assert.match(dashboard, /YouTube episodes linked to cards/);
    assert.match(dashboard, /hash:\s*"collector-quest"/);
    assert.match(dashboard, /hash:\s*"legend-card-grid"/);
    assert.match(dashboard, /hash:\s*"episode-library-title"/);
    ...
    assert.match(globalsCss, /\.browse-hub__grid\s*{[\s\S]*?grid-template-columns:\s*repeat\(4,\s*minmax\(0,\s*1fr\)\);/);
    assert.match(globalsCss, /\.browse-tile\s*{[\s\S]*?grid-template-columns:\s*auto minmax\(0,\s*1fr\) auto auto;/);
    assert.match(globalsCss, /\.browse-tile--cards\s*{[\s\S]*?rgba\(84,\s*224,\s*179,\s*0\.14\)/);
  ...
  it("adds a route menu and collection shortcuts to the Legends page", () => {
    assert.match(predictionsPage, /className="app-shell predictions-shell"/);
    assert.match(predictionsPage, /className="topbar predictions-topbar"/);
    assert.match(predictionsPage, /aria-label="Legends navigation"/);
    assert.match(predictionsPage, /href="#collector-quest"/);
    assert.match(predictionsPage, /href="#legend-card-grid"/);
    assert.match(predictionsPage, /href="#matches"/);
    ...
    assert.doesNotMatch(legendCardCollection, /id="legend-wallpapers"/);
    assert.match(globalsCss, /\.predictions-topbar\s*{[\s\S]*?position:\s*sticky;/);
    assert.match(globalsCss, /\.legend-section-menu\s*{[\s\S]*?grid-template-columns:\s*repeat\(6,\s*minmax\(0,\s*1fr\)\);/);
    assert.match(globalsCss, /\.legend-section-menu__item\.is-primary\s*{[\s\S]*?linear-gradient\(180deg,\s*#ffe29a,\s*#ffcf66\)/);
    assert.doesNotMatch(globalsCss, /\.legend-wallpaper\s*{[\s\S]*?scroll-margin-top:\s*120px;/);
    assert.match(globalsCss, /@media \(max-width:\s*980px\)\s*{[\s\S]*?\.legend-filter-bar,[\s\S]*?\.legend-section-menu\s*{[\s\S]*?grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*1fr\)\);/);
    assert.match(globalsCss, /@media \(max-width:\s*640px\)\s*{[\s\S]*?\.legend-section-menu,[\s\S]*?\.legend-filter-bar,[\s\S]*?grid-template-columns:\s*minmax\(0,\s*1fr\);/);
  });
  ...
  it("uses the active Supabase session for logged-in navigation and wallet state", () => {
    ...
    assert.match(
      dashboard,
      /className="nav-item nav-item--primary" href=\{\{ pathname: "\/predictions", hash: "collector-quest" \}\}/,
    );
    assert.match(dashboard, /className="nav-item" href=\{showPickWorkflow \? "#pick" : "#me"\}/);
    assert.match(dashboard, /showPickWorkflow \? "Pick Teams" : "Account"/);
    assert.match(dashboard, /showPickWorkflow \? "Game entry" : "Your entry"/);
  ...
  it("keeps the first poster installable as a PWA without hiding the play CTA", () => {
    ...
    assert.match(globalsCss, /@media \(max-width:\s*640px\)\s*{[\s\S]*?\.hero-swiper__slide:first-child \.hero-cta-row\s*{[\s\S]*?grid-template-columns:\s*1fr;/);
  ...
  it("keeps the three-team pick workflow guided on mobile", () => {
    ...
    assert.match(dashboard, /showTeamDisclosure/);
    assert.match(dashboard, /compactText=\{`\$\{Math\.min\(compactTeamCount, filteredTeams\.length\)\} of \$\{filteredTeams\.length\} teams visible`\}/);
    assert.match(dashboard, /expandedText=\{`All \$\{filteredTeams\.length\} teams visible`\}/);
    assert.match(dashboard, /label="team list"/);
    ...
    assert.match(globalsCss, /\.team-search-count\s*{/);
    assert.match(globalsCss, /\.card-view-control\s*{[\s\S]*?border-radius:\s*8px;/);
  ...
  it("keeps the signed-in account cards in the app's colored card system", () => {
    ...
    assert.match(myStanding, /standing-card-icon-badge standing-card-icon-badge--gold/);
    assert.equal(myStanding.match(/<CardViewControl/g)?.length, 3);
    assert.match(myStanding, /controlsId="standing-leaderboard-modal"/);
    assert.match(myStanding, /compactText="Rank and points summary"/);
    assert.match(myStanding, /expandedText="Leaderboard details open"/);
    assert.match(myStanding, /hasPopup="dialog"/);
    ...
    assert.match(myStanding, /standing-card-icon-badge standing-card-icon-badge--green/);
    assert.match(myStanding, /compactText=\{`\$\{referralCount\} invited summary`\}/);
    assert.match(myStanding, /expandedText=\{`\$\{referralCount\} referral position/);
    assert.match(myStanding, /controlsId="standing-referrals-list"/);
    ...
    assert.match(myStanding, /controlsId="standing-agent-code"/);
    ...
    assert.match(globalsCss, /\.standing-card-icon-badge\s*{[\s\S]*?linear-gradient\(180deg,\s*#ffe29a/);
    assert.match(globalsCss, /\.standing-card-icon-badge--green\s*{[\s\S]*?linear-gradient\(180deg,\s*#dfffe9/);
    assert.match(globalsCss, /\.standing-card \.card-view-control\s*{/);
    assert.match(globalsCss, /\.standing-card \.card-view-control__button\s*{/);
