...
describe("WorldCup design system integration", () => {
  ...
  it("lets dense dashboard cards minimize and maximize without losing full data access", () => {
    ...
    assert.match(dashboard, /function CardViewControl\(/);
    assert.equal(dashboard.match(/<CardViewControl/g)?.length, 4);
    assert.match(dashboard, /className=\{`card-view-control \$\{expanded \? "is-expanded" : "is-compact"\}`\}/);
    assert.match(dashboard, /expanded \? "Full view" : "Compact view"/);
    assert.match(dashboard, /aria-label=\{`\$\{expanded \? "Minimize" : "Maximize"\} \$\{label\}`\}/);
    assert.match(dashboard, /\{expanded \? "Minimize" : "Maximize"\}/);
  ...
  it("keeps the signed-in account cards in the app's colored card system", () => {
    ...
    assert.match(myStanding, /Open leaderboard details/);
    ...
    assert.match(myStanding, /Show referral summary/);
    assert.match(myStanding, /Show \$\{referralCount\} referral position/);
    ...
    assert.match(myStanding, /className="card-disclosure card-disclosure--standing"/);
    ...
    assert.match(globalsCss, /\.card-disclosure--standing \.card-disclosure__button\s*{/);
