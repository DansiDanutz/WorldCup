...
## ⭐ Standard episode thumbnail (current pipeline)

Every match episode's `thumbnail.jpg` is generated by the **thumbnail kit**
(`marketing/match-videos/_thumbnail-kit/`) so the series stays consistent on the
formula that wins on our channel:

> **Star face + ≤4-word hook + a phone showing the leaderboard climb (Pick 3 · Free).**

This fuses the proven episode formula (a marketable face + a curiosity/stakes
headline — Ep6 Messi breakout) with our best-performing app thumbnails
("Pick 3 + climb" and "phone + leaderboard"). Generate with
`cd marketing/match-videos/_thumbnail-kit && node gen-thumbnails.mjs --install`;
add a new episode by appending to `thumbnails.config.json` (see the kit README).

**Research-first rule:** before changing this template, study our own YouTube
Studio CTR/impressions and the fastest-growing football / faceless-narration
channels, and copy what works. Findings live in
[`THUMBNAIL_RESEARCH.md`](./THUMBNAIL_RESEARCH.md) — keep it updated.

The templates below remain the reference library for one-off / strategy videos.
